...
async function renderTrack(file, fixture, frame, trackId) {
  const image = {
    ...
    data: Buffer.alloc(fixture.canvas.width * fixture.canvas.height * 4)
  };
...
function landmarks(frame, fixture) {
  ...
  return {
    root: { ...fixture.canvas.pivot },
    baseline: fixture.canvas.baseline,
    sockets,
...
export async function runClockworkCourier(root) {
  ...
  const frames = fixture.clips.flatMap((clip) =>
    clip.frames.map((frame) => ({ ...frame, clipId: clip.id, loopMode: clip.loopMode }))
  );
  const outputs = [];
  for (const frame of frames)
    for (const trackId of frame.tracks) {
  ...
  const contractDocument = {
    ...
    scale: fixture.scale,
    palette: {
      rgba: fixture.palette,
      sha256: stableHash(fixture.palette),
      snapperPaletteHex: fixture.palette.slice(1).map((color) =>
        color
          .slice(0, 3)
  ...
  const receipt = await writeSnapReceipt({
    projectDir: root,
    run: { id: 'fixture-production', outputDir: snapped, manifestSha256: stableHash({ fixture: 1 }) },
    contract,
    inputs: outputs,
    outputs,
    args: ['16', '--palette', contractDocument.palette.snapperPaletteHex.join(',')],
    identity: {
  ...
  const approval = await writeFrameApproval({
    ...
    contract,
    snapReceipt: receipt,
    frames: outputs.map((file, index) => ({
      frameId: frames.flatMap((frame) => frame.tracks.map(() => frame.id))[index],
      trackId: frames.flatMap((frame) => frame.tracks)[index],
      path: path.basename(file),
      sha256: receipt.document.payload.outputs[index].sha256
    })),
    approvals: frames.map((frame) => ({
      frameId: frame.id,
      landmarks: landmarks(frame, fixture),
      approved: true,
      approvedBy: 'owner',
      checkpoints: contractDocument.review.checkpoints
    })),
    version: 1
  });
  const config = validateConfig({
    ...structuredClone(DEFAULT_CONFIG),
    canonical: { width: 32, height: 32 },
    generation: { width: 256, height: 256 },
    runtime: { width: 64, height: 64 },
    pivot: { x: 16, y: 27 }
  });
  const normalized = await normalizeContractFrames({
    contract,
    frameApproval: approval,
    outputDir: path.join(work, 'normalized')
  });
  const exported = await exportContractAnimation({
    normalized,
    contract,
    outputDir: path.join(work, 'export'),
    config,
    columns: 4,
    frameApprovalSha256: approval.sha256
  });
  const report = await validateRun({
    anchorReport: await inspectImage(anchor),
    normalized,
    exported,
    config,
  ...
  const run = {
    ...
    root: runRoot,
    sha256: sha256Value({ fixture: 1 }),
    document: { projectSha256: project.sha256, createdAt: new Date().toISOString() }
  };
  const bindings = {
  ...
  const published = await publishExportRevision({
    run,
    bindings,
    pixelExport: { root: work, artifacts: await recursiveArtifacts(work, ['normalized', 'export']) },
  ...
  const audit = await auditRun({
    ...
    expected: {
      ...
      envelope: {
        ...
        approvedBy: 'owner',
        approvedAt: run.document.createdAt
      }
    }
  });
