...
export function installMarkerAuthoring({ root, canvas, project, actionId, getFrame, getFrames, onChange }) {
  ...
  for (const [label, id, kind] of TOOL_DEFINITIONS) {
    ...
    button.addEventListener('click', () => {
      activeTool = { id, kind };
      for (const candidate of tools.querySelectorAll('[data-marker-tool]'))
  ...
  for (const socket of project.sockets.filter(({ id }) => id !== 'hand' && id !== 'effect-origin')) {
    ...
    button.addEventListener('click', () => {
      ...
      for (const candidate of tools.querySelectorAll('button'))
        candidate.setAttribute('aria-pressed', String(candidate === button));
  ...
  const timing = element('fieldset', { className: 'author-group' }, [
    element('legend', { textContent: 'Frame timing & alignment' })
  ]);
  controls.duration = element('input', { type: 'number', min: '1', max: '65535', step: '1' });
  controls.translationX = element('input', {
    type: 'number',
    min: '-16384',
    max: '16384',
    step: '1',
  ...
  controls.translationY = element('input', {
    type: 'number',
    min: '-16384',
    max: '16384',
    step: '1',
  ...
  const contactGroup = element('fieldset', { className: 'author-group' }, [
    element('legend', { textContent: 'Contact interval & travel' })
  ]);
  ...
  for (const contact of project.contacts) {
    ...
    const label =
      contact.id === 'left-foot'
        ? 'Planted left foot'
        : contact.id === 'right-foot'
          ? 'Planted right foot'
          : `Planted ${human}`;
    controls.contacts.set(contact.id, input);
    contactGroup.append(
      element('label', { className: 'check-field' }, [input, element('span', { textContent: label })])
    );
  }
  controls.travelX = element('input', {
    type: 'number',
    min: '-16384',
    max: '16384',
    step: '1',
    'aria-label': 'Ground travel X'
  });
  controls.travelY = element('input', {
    type: 'number',
    min: '-16384',
    max: '16384',
    step: '1',
    'aria-label': 'Ground travel Y'
  });
  contactGroup.append(
    labeledInput('Ground travel X', controls.travelX),
    labeledInput('Ground travel Y', controls.travelY)
  );

  const trackGroup = element('fieldset', { className: 'author-group track-group' }, [
    element('legend', { textContent: 'Tracks' })
  ]);
  ...
  for (const track of project.tracks) {
    ...
    trackGroup.append(
      element('label', { className: 'track-field' }, [
        input,
  ...
  const transformGroup = element('fieldset', { className: 'author-group repair-group' }, [
    element('legend', { textContent: 'Global repair' })
  ]);
  controls.transformOptIn = element('input', { type: 'checkbox' });
  controls.scale = element('input', {
    type: 'number',
    min: '1',
    max: '8',
    step: '1',
    value: '1',
    'aria-label': 'Global integer scale'
  });
  controls.rotation = element('select', { 'aria-label': 'Global quarter turns' });
  for (const value of [-3, -2, -1, 0, 1, 2, 3])
    controls.rotation.append(element('option', { value: String(value), textContent: String(value) }));
  controls.rotation.value = '0';
  transformGroup.append(
    element('label', { className: 'check-field' }, [
      controls.transformOptIn,
      element('span', { textContent: 'Confirm global transform repair' })
    ]),
  ...
  controls.duration.addEventListener('change', () =>
    mutate('Updated authored frame duration.', (edit) => {
      edit.durationMs = Number(controls.duration.value);
    })
  );
  controls.translationX.addEventListener('change', () =>
    mutate('Updated non-destructive X translation.', (edit) => {
      edit.translation.x = Number(controls.translationX.value);
    })
  );
  controls.translationY.addEventListener('change', () =>
    mutate('Updated non-destructive Y translation.', (edit) => {
      edit.translation.y = Number(controls.translationY.value);
    })
  );
  controls.travelX.addEventListener('change', () =>
    mutate('Updated authored ground travel.', (edit) => {
      edit.groundTravel.x = Number(controls.travelX.value);
    })
  );
  controls.travelY.addEventListener('change', () =>
    mutate('Updated authored ground travel.', (edit) => {
      edit.groundTravel.y = Number(controls.travelY.value);
    })
  );
  for (const [id, input] of controls.contacts) {
    input.addEventListener('change', () =>
      mutate('Updated planted-foot contact interval.', (edit) => {
        edit.contacts = input.checked
          ? [...new Set([...edit.contacts, id])]
          : edit.contacts.filter((contact) => contact !== id);
      })
    );
  }
  for (const [id, input] of controls.tracks) {
    input.addEventListener('change', () =>
      mutate('Updated explicit frame tracks.', (edit) => {
  ...
  controls.transformOptIn.addEventListener('change', () => {
    if (
      controls.transformOptIn.checked &&
      !window.confirm('Apply one integer scale and quarter-turn rotation to every frame in this clip?')
    ) {
      controls.transformOptIn.checked = false;
      return;
    }
    mutateAll('Updated explicit global transform repair.', (edit) => {
      edit.transform = controls.transformOptIn.checked
        ? { scale: Number(controls.scale.value), rotationQuarterTurns: Number(controls.rotation.value) }
        : null;
    });
  });
  ...
  canvas.addEventListener('click', (event) => {
    ...
    const x = Math.max(
      0,
      Math.min(project.canvas.width - 1, Math.floor(((event.clientX - rect.left) / rect.width) * project.canvas.width))
    );
    const y = Math.max(
      0,
      Math.min(
        project.canvas.height - 1,
        Math.floor(((event.clientY - rect.top) / rect.height) * project.canvas.height)
      )
    );
