...
test('run creation has one atomic winner for a selected ID', async (t) => {
  ...
  const request = {
    projectRoot,
    project,
    sourceRequest: { actionId: 'unlock', kind: 'generated-still' },
    id: 'same-run'
  };
  ...
  assert.equal(
    (await fs.readdir(path.dirname(published))).some((name) => name.startsWith('.pending-')),
    false
  );
});
...
test('run creation closes source requests and project identity', async (t) => {
  ...
  await assert.rejects(
    createRun({
      projectRoot,
      project: { ...project, sha256: '0'.repeat(64) },
      sourceRequest: { actionId: 'idle', kind: 'gif' }
    }),
...
test('CLI initializes a project and allocates an intake run', async (t) => {
  const parent = await sandbox(t);
  const projectRoot = path.join(parent, 'project');
  const initialized = await execFile(
    process.execPath,
