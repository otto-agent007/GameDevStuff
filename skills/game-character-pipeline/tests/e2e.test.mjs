...
test('Clockwork Courier completes the public workflow reproducibly', async (t) => {
  ...
  const expectedDeterministicHash =
    process.platform === 'win32' ? expected.windowsDeterministicHashesSha256 : expected.deterministicHashesSha256;
  ...
  if (process.env.ACCEPTANCE_ARTIFACT_DIR) {
    ...
    for (const relative of [
      'export/clips/idle/idle-contact-sheet.png',
      'export/clips/idle/idle.webp',
      'export/clips/walk/walk-contact-sheet.png',
      'export/clips/walk/walk.webp',
      'export/clips/unlock/unlock-contact-sheet.png',
      'export/clips/unlock/unlock.webp'
    ]) {
