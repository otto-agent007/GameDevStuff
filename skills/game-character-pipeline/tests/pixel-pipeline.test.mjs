...
function fixture(overrides = {}) {
  ...
  const selectionApproval = {
    path: '/project/run/approved/selection-approval-0001.json',
    sha256: HASH('3'),
    document: {
      runId: 'run-1',
      projectSha256: HASH('2'),
      decision: 'approved',
      selectedFrames: [{ frameId: 'walk-1', derivativeSha256: HASH('4') }]
    }
  };
  return {
    ...
    node: '/usr/bin/node',
    contract: {
      path: '/project/run/work/animation-contract-v2.json',
      sha256: HASH('5'),
      inputs: { sha256: HASH('6') },
      document: { version: 2, selectionApprovalSha256: selectionApproval.sha256 }
    },
...
test('production rejects approval or membership changes before spawning', async () => {
  const value = fixture({
    verifySelectionApproval: async () => {
      throw new Error('approval binding mismatch: selected frame membership changed');
    }
  });
