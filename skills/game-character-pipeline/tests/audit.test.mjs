...
async function runtimePng(file, interpolation = false) {
  const source = Buffer.from([10, 20, 30, 255, 50, 60, 70, 255, 80, 90, 100, 255, 120, 130, 140, 255]);
...
async function fixture({ runId = 'run-a', createdAt = '2026-07-21T12:00:00.000Z', mutation } = {}) {
  ...
  const frame = {
    id: 'walk-1',
    semantic: 'contact',
    duration: mutation === 'timing' ? 0 : 80,
    tracks: ['actor'],
    root: mutation === 'drift' ? { x: 0, y: 1 } : { x: 1, y: 1 },
    baseline: 1,
    sockets: {},
    contacts: {},
    ...
    outputs: [
      {
        ...
        attachTo: null,
        sourceSha256: HASH('8'),
        normalizedSha256: HASH('9'),
        ...(await artifact('tracks/actor/walk-1.png'))
      }
    ],
  ...
  const contract = {
    ...
    character: { id: 'hero', anchorSha256: HASH('4') },
  ...
  const engineIndex = {
    version: 2,
    animationContractSha256: HASH('b'),
    animationContract: contract,
    selectionApprovalSha256: HASH('5'),
    frameApprovalSha256: HASH('6'),
    snapReceiptSha256: HASH('7'),
    ...
    clips: [
      {
        ...
        restart: 'loop',
        frames: [frame],
        sheet: await artifact('clips/walk/walk-sheet.png'),
        contactSheet: await artifact('clips/walk/walk-contact-sheet.png'),
        metadata: await artifact('clips/walk/walk.json'),
        preview: await artifact('clips/walk/walk.webp')
      }
    ]
  };
  await fs.writeFile(path.join(pixelRoot, 'animation-contract-export.json'), JSON.stringify(engineIndex));
  const names = [
    'animation-contract-export.json',
    'tracks/actor/walk-1.png',
    'clips/walk/walk-00.png',
    'clips/walk/walk-sheet.png',
    'clips/walk/walk-contact-sheet.png',
    'clips/walk/walk.json',
    'clips/walk/walk.webp'
  ];
  const artifacts = await Promise.all(
    names.map(async (relative) => ({ path: relative, sha256: await sha256File(path.join(pixelRoot, relative)) }))
  );
  const run = {
    id: runId,
    root: runRoot,
    sha256: sha256Value({ runId }),
    document: { projectSha256: HASH('2'), createdAt }
  };
  ...
  if (mutation === 'broken-hash')
    await fs.appendFile(path.join(path.dirname(published.path), 'clips', 'walk', 'walk-00.png'), 'tamper');
  return {
    temporary,
    run,
    project,
    expected: {
      ...
      validationReport,
      envelope: { runId, createdAt, approvedBy: 'owner', approvedAt: createdAt }
    }
  };
}
...
test('two equivalent runs have identical deterministic artifact hashes', async (t) => {
  ...
  t.after(() =>
    Promise.all([leftFixture, rightFixture].map(({ temporary }) => fs.rm(temporary, { recursive: true, force: true })))
  );
