...
export function inspectAnimatedWebp(input) {
  ...
  for (const chunk of chunks(bytes, 12, bytes.length, 'WebP')) {
    if (chunk.type === 'VP8X') {
      ...
      if (!canvas || sawAnimationHeader || chunk.data.length !== 6)
        throw new Error('animated WebP ANIM chunk is invalid or out of order');
      sawAnimationHeader = true;
    } else if (chunk.type === 'ANMF') {
      if (!canvas || !sawAnimationHeader || chunk.data.length < 24)
        throw new Error('animated WebP ANMF chunk is invalid or out of order');
      const rect = {
      ...
      if (!payload.some(({ type }) => type === 'VP8 ' || type === 'VP8L'))
  ...
  if (!canvas || !animated || !sawAnimationHeader || frames.length === 0)
    throw new Error('WebP is not a complete animation');
