...
async function readCanonical(file, root, label) {
  ...
  if ((await sha256File(selected)) !== sha256Value(document))
    throw new Error(`${label} must be canonical immutable JSON`);
      ...
      for (let y = 0; y < scale; y += 1)
        for (let x = 0; x < scale; x += 1) {
          ...
          for (let channel = 0; channel < 4; channel += 1)
            if (data[offset + channel] !== data[origin + channel]) return false;
        }
    }
  return true;
}
  ...
  for (const clip of index.clips ?? []) {
    for (const frame of clip.frames ?? []) {
      for (const output of frame.outputs ?? [])
        references.push({ label: `${frame.id}/${output.trackId}`, record: output, pixels: true });
      references.push({ label: `${frame.id}/combined`, record: frame.combined, pixels: true });
    }
    for (const [name, record] of [
      ['sheet', clip.sheet],
      ['contactSheet', clip.contactSheet],
...
async function inspectEngineIndex({ index, exportRoot, enginePrefix, artifactHashes, project, manifest, failures }) {
  ...
  if (
    index.character?.id !== project.document?.id ||
    !same(index.canvas, contract.canvas) ||
    !same(index.scale, contract.scale)
  )
    failure(failures, 'METADATA_MISMATCH', { field: 'projectGeometry' });
  if (!same(index.canvas, project.document?.canvas) || !same(index.scale, project.document?.scale))
    failure(failures, 'METADATA_MISMATCH', { field: 'initializedProjectGeometry' });
  if (
    !Number.isInteger(contract.scale?.integer) ||
    contract.scale.integer < 1 ||
    contract.scale.runtime?.width !== contract.canvas?.width * contract.scale.integer ||
    contract.scale.runtime?.height !== contract.canvas?.height * contract.scale.integer
  )
    failure(failures, 'NON_INTEGER_SCALE');
  ...
  for (const [clipIndex, clip] of index.clips.entries()) {
    ...
    if (!definition || clip.id !== definition.id || clip.loopMode !== definition.loopMode) {
      failure(failures, 'METADATA_MISMATCH', { field: 'clip', clip: clip.id });
      continue;
    }
    const restart = definition.loopMode === 'loop' ? 'loop' : 'stop';
    if (clip.restart !== restart) failure(failures, 'NONCYCLIC_RESTART', { clip: clip.id });
    if (!Array.isArray(clip.frames) || clip.frames.length !== definition.frames.length) {
      failure(failures, 'FRAME_COUNT', { clip: clip.id });
      continue;
    }
    for (const [frameIndex, frame] of clip.frames.entries()) {
      ...
      if (
        ...
        failure(failures, 'TIMING_MISMATCH', { frame: frame.id });
      if (!same(frame.root, contract.canvas.pivot) || frame.baseline !== contract.canvas.baseline)
        failure(failures, 'LANDMARK_DRIFT', { frame: frame.id });
      if (!same(frame.tracks, expected.tracks) || !same(frame.groundTravel, expected.groundTravel))
        failure(failures, 'METADATA_MISMATCH', { frame: frame.id });
      if (
        (frame.groundTravel?.x !== 0 || frame.groundTravel?.y !== 0) &&
        Object.keys(frame.contacts ?? {}).length === 0
      )
        failure(failures, 'GROUND_TRAVEL_CONTACT', { frame: frame.id });
      for (const output of frame.outputs ?? []) {
        const track = tracks.get(output.trackId);
        if (
          !track ||
          output.kind !== track.kind ||
          output.attachTo !== track.attachTo ||
          (track.attachTo !== null && !expected.sockets.includes(track.attachTo))
        )
          failure(failures, 'SOCKET_ATTACHMENT', { frame: frame.id, track: output.trackId });
      }
    }
  }
  for (const { label, record, pixels } of artifactReferences(index)) {
    ...
    if (
      ...
      artifactHashes.get(publishedPath) !== record.sha256
    ) {
    ...
    if (pixels) {
      try {
        ...
        if (!(await verifyIntegerBlocks(file, contract.canvas, contract.scale.integer)))
          failure(failures, 'INTERPOLATION_DETECTED', { artifact: label });
      } catch (error) {
...
export async function auditRun({ run, project, expected }) {
  ...
  if (!run?.id || !run?.root || !project?.sha256 || !expected?.exportManifest)
    throw new Error('audit requires an immutable run, project, and expected export manifest');
  ...
  for (const artifact of expected.deterministicArtifacts ?? []) {
    try {
      ...
      if (typeof artifact.id !== 'string' || artifact.id === '' || !HASH.test(artifact.sha256))
        throw new Error('deterministic artifact identity is invalid');
  ...
  try {
    ...
    if (
      ...
      manifest.kind !== 'pixel-production-export' ||
      manifest.runId !== run.id ||
      manifest.runSha256 !== run.sha256 ||
      manifest.projectSha256 !== project.sha256
    )
      failure(failures, 'EXPORT_BINDING_MISMATCH');
    if (!Array.isArray(manifest.artifacts) || manifest.artifacts.length === 0)
      failure(failures, 'ARTIFACT_SET_MISMATCH');
    ...
    for (const artifact of manifest.artifacts ?? []) {
      try {
        ...
        if (
          artifact.path === 'animation-contract-export.json' ||
          artifact.path.endsWith('/animation-contract-export.json')
        ) {
          engineIndex = JSON.parse(await fs.readFile(file, 'utf8'));
          enginePath = artifact.path;
        } else if (artifact.path === 'validation-report.json')
          publishedValidation = JSON.parse(await fs.readFile(file, 'utf8'));
    ...
    if (!engineIndex) failure(failures, 'METADATA_MISMATCH', { field: 'animation-contract-export.json' });
      ...
      deterministicHashes['export/animation-contract-export.semantic.json'] = sha256Value(
        deterministicValue(engineIndex)
      );
      await inspectEngineIndex({
        index: engineIndex,
        exportRoot,
        enginePrefix: path.posix.dirname(enginePath),
        artifactHashes,
        project,
        manifest,
        failures
  ...
  if (
    ...
    failure(failures, 'MISSING_VALIDATION_REPORT');
  else {
    ...
    for (const item of validation.failures)
      failure(failures, item.code ?? 'OBJECTIVE_VALIDATION_FAILURE', { source: 'pixel-validation' });
    for (const warning of validation.warnings) reviews.push(structuredClone(warning));
    if (validation.passed !== true && validation.failures.length === 0)
      failure(failures, 'OBJECTIVE_VALIDATION_FAILURE');
  }
  return {
    passed: failures.length === 0,
    deterministicHashes: Object.fromEntries(
      Object.entries(deterministicHashes).sort(([left], [right]) => left.localeCompare(right))
    ),
    evidence,
    failures,
    reviews,
...
export function compareRuns(left, right) {
  ...
  const changedDeterministicArtifacts = [...new Set([...Object.keys(leftHashes), ...Object.keys(rightHashes)])]
    .sort()
    .flatMap((artifact) => {
  ...
  const envelopeDifferences = [...new Set([...Object.keys(leftEnvelope), ...Object.keys(rightEnvelope)])]
    .sort()
    .flatMap((field) =>
      same(leftEnvelope[field], rightEnvelope[field])
        ? []
        : [{ field, left: leftEnvelope[field] ?? null, right: rightEnvelope[field] ?? null }]
    );
  return {
    passed: left?.passed === true && right?.passed === true && changedDeterministicArtifacts.length === 0,
    changedDeterministicArtifacts,
    envelopeDifferences
  };
}
...
function revisionNumber(value) {
  if (!Number.isInteger(value) || value < 1 || value > 9999)
    throw new Error('audit export revision must be an integer from 1 to 9999');
  return value;
}
...
export async function recordProductionValidation({ run, exportRevision, exportManifestSha256, validationReport }) {
  ...
  if (
    !run?.id ||
    !run?.root ||
    !HASH.test(exportManifestSha256 ?? '') ||
    !validationReport ||
    typeof validationReport !== 'object'
  )
    throw new Error('production validation record requires a run, export manifest, and report');
  const document = {
    schemaVersion: 1,
...
export async function loadAuditExpected({ run, revision }) {
  ...
  const exportManifest = path.join(
    run.root,
    'exports',
    `revision-${String(selected).padStart(4, '0')}`,
    'manifest.json'
  );
  ...
  const validationFile = path.join(
    run.root,
    'reports',
    `production-validation-${String(selected).padStart(4, '0')}.json`
  );
  const validation = await readCanonical(validationFile, run.root, 'production validation report');
  exactObject(
    validation.document,
    ['schemaVersion', 'kind', 'runId', 'exportRevision', 'exportManifestSha256', 'report'],
  ...
  const approvalFiles = (await fs.readdir(path.join(run.root, 'approved'))).filter((name) =>
    /^selection-approval-\d{4}\.json$/.test(name)
  );
  ...
  for (const frame of source.document.frames ?? [])
    deterministicArtifacts.push({ id: `decoded/${frame.id}`, path: frame.path, sha256: frame.sha256 });
  for (const derivative of approval.derivatives ?? [])
    deterministicArtifacts.push({ id: `edit/${derivative.frameId}`, path: derivative.path, sha256: derivative.sha256 });
  for (const [id, record] of [
    ['edit/contact-sheet', approval.contactSheet],
    ['edit/rendered-manifest', approval.renderedManifest],
    ['edit/rendered-review', approval.renderedReview]
  ]) {
  ...
  return {
    ...
    envelope: {
      runId: run.id,
      createdAt: run.document.createdAt,
