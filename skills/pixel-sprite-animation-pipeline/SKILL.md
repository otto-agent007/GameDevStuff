...
Preserve approved inputs and write every attempt to a new versioned run. Node.js 22.12.0 or newer is required. Use `node scripts/cli.mjs --help` for command syntax; run `npm ci` at the repository root in a source checkout, or `npm install --omit=dev` in this directory when using an installed or copied bundle and dependencies are absent.
