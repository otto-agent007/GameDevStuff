    ...
    clips: [
      {
        id: 'idle',
        loopMode: 'loop',
        loopTransition: { fromFrameId: 'idle-2', toFrameId: 'idle-1', reviewCheckpoint: 'motion' },
        frames: [
          {
            id: 'idle-1',
            pose: 'rest',
...
async function makeExtendedLimbFrames() {
  ...
  for (const [index, file] of frames.entries()) {
    ...
    await sharp(data, { raw: { width: 64, height: 64, channels: 4 } })
      .png()
      .toFile(file);
  }
  return {
    frames,
    landmarks: frames.map((_, index) => ({
      frameId: `idle-${index + 1}`,
      source: { x: 30, y: 50 },
      target: { x: 64, y: 112 }
    })),
    torso
...
async function v2Fixture() {
  ...
  for (const [frameIndex, definition] of definitions.entries()) {
    ...
    approvalFrames.push({
      ...
      landmarks: {
        ...
        contacts: [{ id: 'left-foot', x: rootX, y: 11 }],
  ...
  const frameApproval = {
    path: path.join(runDir, 'frame-approval-01.json'),
    sha256: HASH('e'),
    document: {
      payload: {
        version: 2,
...
test('v2 normalization keeps scale fixed and maps sockets exactly', async () => {
  ...
  assert.equal(
    result.frames.every((frame) => frame.scale === 2),
    true
  );
  assert.equal(
    result.frames.every((frame) => frame.sockets.hand.x === 12),
    true
  );
  assert.equal(
    result.frames.every((frame) => frame.root.x === 8 && frame.root.y === 14),
    true
  );
  assert.deepEqual(
    result.frames.map((frame) => Object.keys(frame.tracks)),
    [
      ['actor', 'satchel'],
      ['actor', 'satchel']
    ]
  ...
  const source = path.join(
    path.dirname(changed.frameApproval.path),
    changed.frameApproval.document.payload.frames[0].outputs[0].path
  );
...
test('authored roots stay fixed when pose bounds change', async () => {
  ...
  const result = await normalizeFrames({
    inputs: frames,
    landmarks,
    outputDir: path.join(path.dirname(frames[0]), 'out'),
    config: DEFAULT_CONFIG,
    scaleFactor: 1
  });

  assert.deepEqual(
    result.measurements.map((item) => item.frameId),
    ['idle-1', 'idle-2']
  );
  assert.deepEqual(
    result.measurements.map((item) => item.sourceLandmark),
    [
      { x: 30, y: 50 },
      { x: 30, y: 50 }
    ]
...
test('contract landmarks require exact ordered coverage and contained integer coordinates', async () => {
  ...
  for (const item of cases) {
    await assert.rejects(
      normalizeFrames({ inputs: frames, landmarks: item.value, outputDir, config: DEFAULT_CONFIG }),
      item.error
    );
...
test('approved landmark overflow rejects the whole batch before output', async () => {
  ...
  await assert.rejects(
    normalizeFrames({ inputs: frames, landmarks, outputDir, config: DEFAULT_CONFIG, scaleFactor: 10 }),
    /exceeds canonical cell at approved landmark/i
  );
  assert.deepEqual(await pngNames(outputDir), []);
});
...
test('normalization snapshots approved landmarks before asynchronous image reads', async () => {
  const { frames, landmarks } = await makeExtendedLimbFrames();
  const pending = normalizeFrames({
    inputs: frames,
    landmarks,
    outputDir: path.join(path.dirname(frames[0]), 'snapshot'),
    config: DEFAULT_CONFIG
  });
...
test('normalization snapshots input ordering and relevant config before asynchronous reads', async () => {
  ...
  const pending = normalizeFrames({
    ...
    landmarks,
    animationContract: contract,
    outputDir: path.join(path.dirname(frames[0]), 'contract-snapshot'),
    config
  });
  frames.reverse();
  config.pivot.x = 2;
  ...
  const result = await pending;
  assert.deepEqual(
    result.measurements.map((item) => item.input),
    expectedInputs
  );
  assert.deepEqual(
    result.measurements.map((item) => item.canonicalLandmark),
...
test('normalization preserves one scale and plants every frame on the shared baseline', async () => {
  ...
  assert.deepEqual(
    result.measurements.map((item) => item.bottom),
    [111, 111]
  );
  assert.deepEqual(
    result.measurements.map((item) => item.scaleFactor),
    [1, 1]
  );
  assert.deepEqual(
    result.measurements.map(({ width, height }) => ({ width, height })),
    [
      { width: 15, height: 30 },
      { width: 28, height: 18 }
    ]
...
test('component recovery uses four-neighbor connectivity and largest mode masks secondary foreground', async () => {
  ...
  assert.deepEqual(
    diagonal.map((component) => component.length),
    [1, 1]
  );
  ...
  await sharp(data, { raw: { width: 7, height: 7, channels: 4 } })
    .png()
    .toFile(input);
...
test('opaque dominant-border chroma is removed and output outside the subject is transparent', async () => {
  ...
  await sharp(data, { raw: { width: 9, height: 7, channels: 4 } })
    .png()
    .toFile(input);
...
test('configured background color takes precedence over border detection and honors tolerance', async () => {
  ...
  await sharp(data, { raw: { width: 5, height: 5, channels: 4 } })
    .png()
    .toFile(input);
...
test('default all policy retains detached foreground and uses union bounds', async () => {
  ...
  await sharp(data, { raw: { width: 12, height: 5, channels: 4 } })
    .png()
    .toFile(input);
...
test('detached foreground union drives overflow while largest remains opt-in', async () => {
  ...
  await sharp(data, { raw: { width: 10, height: 3, channels: 4 } })
    .png()
    .toFile(input);
  const config = { ...DEFAULT_CONFIG, canonical: { width: 6, height: 6 }, pivot: { x: 3, y: 5 } };

  await assert.rejects(normalizeFrames({ inputs: [input], outputDir: allOutput, config }), /exceeds canonical cell/);
  assert.deepEqual(await pngNames(allOutput), []);

  const largest = await normalizeFrames({
    inputs: [input],
    outputDir: path.join(dir, 'largest'),
    config,
    retentionPolicy: 'largest'
  });
  assert.deepEqual(
...
test('reject-multiple errors before writing any frame in the batch', async () => {
  ...
  await sharp(data, { raw: { width: 8, height: 4, channels: 4 } })
    .png()
    .toFile(detached);

  await assert.rejects(
    normalizeFrames({
      inputs: [valid, detached],
      outputDir,
      config: DEFAULT_CONFIG,
      retentionPolicy: 'reject-multiple'
    }),
...
test('one integer scale is applied unchanged to every pose on transparent canonical canvases', async () => {
  ...
  assert.deepEqual(
    result.measurements.map(({ width, height, scaleFactor, bottom }) => ({
      width,
      height,
      scaleFactor,
      bottom
