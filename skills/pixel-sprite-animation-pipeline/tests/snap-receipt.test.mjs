...
async function fixture() {
  ...
  await Promise.all([
    ...
    fs.writeFile(output, 'snapped'),
    fs.writeFile(handoff, '{"version":1}\n')
  ]);
  const run = { id: 'run-1', outputDir, manifestSha256: 'a'.repeat(64) };
  const contract = { sha256: 'b'.repeat(64) };
  const identity = {
    origin: 'managed-cache',
    physicalPath: '/verified/snapper',
    sha256: 'c'.repeat(64),
    size: 1,
    version: '1.2.3',
    helpSha256: 'd'.repeat(64),
    fixtureRgbaSha256: 'e'.repeat(64),
...
test('receipt output records are contained while external inputs remain hash-bound', async () => {
  ...
  await assert.rejects(
    verifySnapReceipt({ projectDir: value.projectDir, file, expectedRun: value.run, expectedContract: value.contract }),
    /output|contained|escape/i
  );
});
...
test('receipt schemas reject signed extra and malformed identity fields', async () => {
  ...
  await assert.rejects(
    verifySnapReceipt({ projectDir: value.projectDir, file, expectedRun: value.run, expectedContract: value.contract }),
    /schema|identity|binary/i
  );

  const manual = await writeManualHandoffReceipt({
    projectDir: value.projectDir,
    run: value.run,
    handoff: value.handoff,
    inputs: [value.input],
  ...
  await writeSignedState({
    projectDir: value.projectDir,
    file: manualFile,
    domain: 'pixel-sprite-manual-handoff-receipt/v1',
    payload: manualMalformed
  });
  await assert.rejects(
    verifySnapReceipt({ projectDir: value.projectDir, file: manualFile, expectedRun: value.run }),
    /schema|date/i
  );
});
...
test('receipt pins use the manifest tag grammar and bind the abbreviated commit', async () => {
  ...
  await writeSnapReceipt({
    ...
    args: ['16'],
    identity: { ...build.identity, pinnedReleaseTag: 'pixel-snapper-v1.2.3+build.7-commit.abcdef0', upstreamCommit }
  });
  ...
  for (const [tag, commit] of [
    ...
    await assert.rejects(
      writeSnapReceipt({
        ...
        outputs: [value.output],
        args: ['16'],
        identity: { ...value.identity, pinnedReleaseTag: tag, upstreamCommit: commit }
      }),
      /pin|identity/i
    );
  }
  ...
  const unpinnedReceipt = await writeSnapReceipt({
    projectDir: unpinned.projectDir,
    run: unpinned.run,
    contract: unpinned.contract,
    inputs: [unpinned.input],
    outputs: [unpinned.output],
    args: ['16'],
    identity: unpinned.identity
  });
  assert.deepEqual(
    [unpinnedReceipt.document.payload.binary.pinnedReleaseTag, unpinnedReceipt.document.payload.binary.upstreamCommit],
    [null, null]
  );
});
