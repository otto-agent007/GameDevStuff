...
test('createRun binds a redacted inspection snapshot to matching source bytes and dimensions', async () => {
  ...
  await sharp({ create: { width: 3, height: 4, channels: 4, background: { r: 0, g: 255, b: 0, alpha: 1 } } })
    .png()
    .toFile(input);
  ...
  await assert.rejects(
    createRun({
      ...
      config: stableConfig(),
      inputs: [input],
      inspectionSnapshot: { ...inspection, width: 99 },
      idFactory: () => 'bad-inspection'
    }),
  ...
  await assert.rejects(
    createRun({
      ...
      config: stableConfig(),
      inputs: [input],
      inspectionSnapshot: { ...inspection, sha256: '0'.repeat(64) },
      idFactory: () => 'bad-hash'
    }),
...
async function passingReport(run, runDir, additions = {}) {
  ...
  const receipt = await writeSnapReceipt({
    ...
    inputs: [path.join(runDir, source.path)],
    outputs: [path.join(runDir, output.path)],
    args: ['16'],
    identity: {
      ...
      size: 1,
      version: 'test',
      helpSha256: 'c'.repeat(64),
      fixtureRgbaSha256: 'd'.repeat(64),
...
async function correctionEvidence(run, { failureCode = 'BACKGROUND_REMAINS', correction = 'rekey' } = {}) {
  ...
  await sharp({ create: { width: 2, height: 2, channels: 4, background: { r: 0, g: 255, b: 0, alpha: 1 } } })
    .png()
    .toFile(beforeFile);
  await sharp({ create: { width: 2, height: 2, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } } })
    .png()
    .toFile(afterFile);
    ...
    actions: [
      {
        ...
        evidenceVerification: {
          ...
          afterValidation: {
            ...
            failures: [],
            artifacts: [after],
            measurements: { background: { opaqueBorderPixels: 0, configuredColorPixels: 0 } }
          }
        }
      }
    ]
...
async function canvasCorrectionEvidence(
  run,
  { stage, beforeSize = [2, 2], afterSize = [4, 4], actionExpected = afterSize } = {}
) {
  ...
  await fs.mkdir(path.dirname(afterFile), { recursive: true });
  await sharp({
    create: { width: beforeSize[0], height: beforeSize[1], channels: 4, background: { r: 1, g: 2, b: 3, alpha: 1 } }
  })
    .png()
    .toFile(beforeFile);
  await sharp({
    create: { width: afterSize[0], height: afterSize[1], channels: 4, background: { r: 1, g: 2, b: 3, alpha: 1 } }
  })
    .png()
    .toFile(afterFile);
...
test('createRun publishes an immutable manifest with portable hashed inputs', async () => {
  ...
  assert.deepEqual(manifest.inputs, [
    { id: 'art/pilot.png', sha256: hash('pilot'), provenance: { source: 'pixel-snapper' } }
  ]);
  assert.equal(JSON.stringify(manifest).includes(projectDir), false);
  await assert.rejects(
    createRun({ projectDir, config: stableConfig(), inputs: [], idFactory: () => 'run-fixed' }),
...
test('concurrent run creation publishes exactly one complete manifest', async () => {
  ...
  const attempts = await Promise.allSettled(
    [1, 2].map(() => createRun({ projectDir, config: stableConfig(), inputs: [], idFactory: () => 'same-run' }))
  );
  assert.equal(attempts.filter((item) => item.status === 'fulfilled').length, 1);
  assert.equal(attempts.filter((item) => item.status === 'rejected').length, 1);
  const manifest = JSON.parse(
    await fs.readFile(path.join(projectDir, '.pixel-sprite-pipeline', 'runs', 'same-run', 'manifest.json'), 'utf8')
  );
  assert.equal(manifest.runId, 'same-run');
  assert.deepEqual(
    (await fs.readdir(path.join(projectDir, '.pixel-sprite-pipeline', 'runs'))).filter((name) => name.includes('.tmp')),
    []
  );
});
...
test('run IDs reject Windows device stems, extensions, trailing punctuation, and controls on every platform', async () => {
  for (const id of [
    ...
    const projectDir = await project();
    await assert.rejects(
      createRun({ projectDir, config: stableConfig(), inputs: [], idFactory: () => id }),
      /Windows-safe/
    );
  }
});
...
test('createRun rejects non-JSON-safe state, secrets, and external inputs without a portable id', async () => {
  ...
  await assert.rejects(
    createRun({
      projectDir,
      config: stableConfig(),
      inputs: [{ path: external, id: 'external/private.png', provenance: { apiToken: 'nope' } }]
    }),
    /secret/
  );
  await assert.rejects(
    createRun({
      projectDir,
      config: { ...stableConfig(), correction: { generativeAttempts: 2, skillProposalEvidence: 0 } },
      inputs: []
    }),
    /skillProposalEvidence/
  );
  await assert.rejects(
    createRun({ projectDir, config: { ...stableConfig(), surprise: true }, inputs: [] }),
    /unknown config key/
  );
  await assert.rejects(
    createRun({
      projectDir,
      config: { ...stableConfig(), background: { ...stableConfig().background, surprise: true } },
      inputs: []
    }),
    /unknown config key/
  );
  await assert.rejects(
    createRun({
      projectDir,
      config: { ...stableConfig(), background: { mode: 'configured', color: null, tolerance: 0 } },
      inputs: []
    }),
    /configured background.*RGBA/
  );
  await assert.rejects(
    createRun({
      projectDir,
      config: {
        ...stableConfig(),
        background: { mode: 'border', color: { r: 0, g: 255, b: 0, a: 255 }, tolerance: 0 }
      },
      inputs: []
    }),
  ...
  await assert.rejects(
    createRun({
      projectDir,
      config: stableConfig(),
      inputs: [{ path: external, id: 'external/private.png', provenance: { sourcePath: external } }]
    }),
...
test('input normalization never follows file or parent-directory symlinks', async (t) => {
  ...
  const regular = await createRun({
    projectDir,
    config: stableConfig(),
    inputs: [local],
    idFactory: () => 'regular-input'
  });
  ...
  await assert.rejects(
    createRun({
      projectDir,
      config: stableConfig(),
      inputs: [{ path: fileLink, id: 'external/file.png' }],
      idFactory: () => 'file-link'
    }),
    /symlink/
  );
  await assert.rejects(
    createRun({
      projectDir,
      config: stableConfig(),
      inputs: [{ path: path.join(directoryLink, 'external.png'), id: 'external/parent.png' }],
      idFactory: () => 'parent-link'
    }),
    /symlink/
  );
  const explicit = await createRun({
    projectDir,
    config: stableConfig(),
    inputs: [{ path: external, id: 'external/direct.png' }],
    idFactory: () => 'external-direct'
  });
...
test('recordRunResult is immutable, tied to its manifest, and supports explicit versions', async () => {
  ...
  await assert.rejects(
    recordRunResult({ projectDir, runId: run.runId, report: { ...report, runId: 'wrong' }, version: 3 }),
    /run ID/
  );
  await assert.rejects(
    recordRunResult({
      projectDir,
      runId: run.runId,
      report: { ...report, manifestSha256: '0'.repeat(64) },
      version: 3
    }),
    /manifest hash/
  );
  await assert.rejects(
    recordRunResult({
      projectDir,
      runId: run.runId,
      report: { ...report, notes: path.join(projectDir, 'private.txt') },
      version: 3
    }),
    /absolute private paths/
  );
...
test('profile promotion requires artifact-backed passing evidence and preserves an existing profile on failure', async () => {
  ...
  const badReport = {
    runId: run.runId,
    manifestSha256: run.manifestSha256,
    validation: { passed: true, artifacts: [] }
  };
...
test('profile promotion requires a verified signed snap receipt provenance binding', async () => {
  ...
  await recordRunResult({
    projectDir,
    runId: run.runId,
    report: { ...report, toolProvenanceVerified: false, snapReceipt: null }
  });
...
test('profile promotion rejects symlinked and hard-linked evidence artifacts', async (t) => {
  ...
  const symlinkReport = {
    runId: run.runId,
    manifestSha256: run.manifestSha256,
    validation: { passed: true, artifacts: [{ path: 'linked.txt', sha256: await fileHash(outside) }] }
  };
  ...
  const hardReport = {
    runId: run.runId,
    manifestSha256: run.manifestSha256,
    validation: { passed: true, artifacts: [{ path: 'hard.txt', sha256: await fileHash(outside) }] }
  };
  await recordRunResult({ projectDir, runId: run.runId, report: hardReport, version: 2 });
  await assert.rejects(
    promoteVerifiedProfile({ projectDir, runId: run.runId, reportVersion: 2 }),
    /hard-linked|artifact-backed/
  );
});
...
test('recorded lessons require correction-manifest evidence, not caller booleans', async () => {
  ...
  await assert.rejects(
    recordRunResult({
      ...
      report: {
        ...
        lessons: [
          {
            failureCode: 'BACKGROUND_REMAINS',
            correction: 'rekey',
            proposedRule: 'Prefer chroma cleanup',
            approved: true
          }
        ]
      }
  ...
  const rows = (await fs.readFile(path.join(projectDir, '.pixel-sprite-pipeline', 'lessons.jsonl'), 'utf8'))
    .trim()
    .split('\n')
    .map(JSON.parse);
...
test('lesson recording rejects correction evidence whose backing artifact was changed', async () => {
  ...
  await assert.rejects(
    recordRunResult({
      projectDir,
      runId: run.runId,
      report: await passingReport(run, run.runDir, {
        lessons: [
          {
            failureCode: 'BACKGROUND_REMAINS',
            correction: 'rekey',
...
test('lesson recording validates every artifact in the before and after evidence sets', async (t) => {
  ...
  document.actions[0].evidenceVerification.afterValidation.artifacts.push({
    path: 'correction-01/extra.png',
    sha256: await fileHash(outside)
  });
  await fs.writeFile(file, `${JSON.stringify(document)}\n`);
...
test('lesson authenticity rejects wrong run binding and non-objective arbitrary artifacts', async () => {
  ...
  action.evidenceVerification.beforeValidation.artifacts = [
    { path: 'input/before.png', sha256: action.evidenceVerification.beforeSha256 }
  ];
  action.evidenceVerification.afterValidation.artifacts = [
    { path: 'correction-01/after.png', sha256: action.evidenceVerification.afterSha256 }
  ];
  await fs.writeFile(file, `${JSON.stringify(document)}\n`);
        ...
        lessons: [
          {
            ...
            correction: 'rekey',
            proposedRule: 'Prefer chroma cleanup',
            correctionManifest: arbitrary,
            actionIndex: 0
          }
        ]
      })
    }),
    /objective correction evidence|unsupported image format/
  );
});
...
test('canvas lessons derive expected dimensions from the bound run stage, never action claims', async () => {
  ...
  const forgedManifest = await canvasCorrectionEvidence(forgedRun, {
    stage: 'canonical',
    afterSize: [3, 3],
    actionExpected: [3, 3]
  });
  await assert.rejects(
    recordRunResult({
      projectDir,
      runId: forgedRun.runId,
      report: await passingReport(forgedRun, forgedRun.runDir, {
        lessons: [
          {
            failureCode: 'CANVAS_SIZE',
            correction: 'repad',
            proposedRule: 'Repad canonical cells',
            correctionManifest: forgedManifest,
            actionIndex: 0
          }
        ]
      })
  ...
  await assert.rejects(
    recordRunResult({
      ...
      report: await passingReport(ambiguousRun, ambiguousRun.runDir, {
        lessons: [
          {
            failureCode: 'CANVAS_SIZE',
            correction: 'repad',
            proposedRule: 'Repad canonical cells',
            correctionManifest: ambiguousManifest,
            actionIndex: 0
          }
        ]
      })
    }),
    /valid target stage|objective correction evidence/
  );

  const validRun = await createRun({ projectDir, config, inputs: [], idFactory: () => 'canvas-valid' });
  const validManifest = await canvasCorrectionEvidence(validRun, {
    stage: 'runtime',
    beforeSize: [2, 2],
    afterSize: [12, 12],
    actionExpected: [99, 99]
  });
  const recorded = await recordRunResult({
    projectDir,
    runId: validRun.runId,
    report: await passingReport(validRun, validRun.runDir, {
      lessons: [
        {
          failureCode: 'CANVAS_SIZE',
          correction: 'repad',
          proposedRule: 'Repad runtime cells',
          correctionManifest: validManifest,
          actionIndex: 0
        }
      ]
    })
...
test('proposal counts independently verified runs once and never applies a rule', async () => {
  ...
  const proposal = await proposeSkillRule({
    projectDir,
    lesson: { failureCode: 'BACKGROUND_REMAINS', correction: 'rekey', proposedRule: 'Prefer chroma cleanup' },
    config: stableConfig()
  });
  assert.equal(proposal.eligible, true);
  assert.equal(proposal.evidenceCount, 3);
  assert.deepEqual(
    proposal.evidence.map((item) => item.runId),
    ['run-1', 'run-2', 'run-3']
  );
...
test('proposal discloses threshold overrides and insufficient evidence', async () => {
  ...
  const regular = await proposeSkillRule({
    projectDir,
    lesson: { failureCode: 'BACKGROUND_REMAINS', correction: 'rekey', proposedRule: 'Prefer chroma cleanup' },
    config: stableConfig()
  });
  ...
  const overridden = await proposeSkillRule({
    projectDir,
    lesson: { failureCode: 'BACKGROUND_REMAINS', correction: 'rekey', proposedRule: 'Prefer chroma cleanup' },
    config: stableConfig(),
    thresholdOverride: 1
...
test('an identical report can be retried to recover missing lesson-index publication', async () => {
  ...
  await assert.rejects(
    recordRunResult({ projectDir, runId: run.runId, report: { ...report, note: 'conflict' } }),
...
test('malformed lesson JSONL is surfaced instead of silently skipped', async () => {
  ...
  await assert.rejects(
    proposeSkillRule({
      projectDir,
      lesson: { failureCode: 'X', correction: 'Y', proposedRule: 'Z' },
      config: stableConfig()
    }),
    /malformed lessons.jsonl row 1/
  );
});
