...
test('prepare pads without changing foreground pixels and exports exact sizes', async () => {
  ...
  for (let y = 3; y <= 11; y += 1) {
    for (let x = 5; x <= 7; x += 1) {
      ...
      for (let scaleY = 0; scaleY < 8; scaleY += 1)
        for (let scaleX = 0; scaleX < 8; scaleX += 1) {
...
test('prepare keys every matching background pixel including enclosed spaces', async () => {
  ...
  for (let y = 0; y < 5; y += 1)
    for (let x = 0; x < 5; x += 1) {
      ...
      assert.equal(
        pixelAt(transparent, left + x, top + y)[3],
        sourcePixel[0] === 0 && sourcePixel[1] === 255 && sourcePixel[2] === 0 ? 0 : 255
      );
    }
});
...
test('prepare forces detected or configured chroma backgrounds opaque', async () => {
  ...
  const configured = await prepareAnchor({
    ...
    config: {
      ...DEFAULT_CONFIG,
      background: { mode: 'configured', color: { r: 90, g: 80, b: 70, a: 0 }, tolerance: 0 }
    }
  });
