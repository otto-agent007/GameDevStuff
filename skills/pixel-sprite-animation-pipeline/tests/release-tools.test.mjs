...
function publicMetadata() {
  return {
    ...
    assets: PUBLIC_ASSET_NAMES.map((name) => ({ name, sha256: H('a'), size: 1 }))
  };
}
...
function record(target, rustTarget, archiveFormat) {
  ...
  return {
    ...
    rustTarget,
    archive: {
      name: `pixel-snapper-${target}.${archiveFormat === 'zip' ? 'zip' : 'tar.gz'}`,
      format: archiveFormat,
      sha256: hash(`archive-${target}`),
      size: 100 + target.length
    },
    ...
    build: {
      workflowCommit: WORKFLOW,
      rustVersion: '1.88.0',
      cargoVersion: 'cargo 1.88.0',
      cargoLockSha256: H('a'),
      cargoSbomVersion: '0.10.0',
      cargoAboutVersion: '0.8.4',
      binaryVersion: 'spritefusion-pixel-snapper 1.0.0',
      helpSha256: H('3')
    },
    fixture: {
      inputRgbaSha256: FIXTURE_HASH,
      rgbaSha256: FIXTURE_HASH,
      width: 3,
      height: 3,
      paletteSha256: PALETTE_HASH
    },
    files: {
...
test('release assembly rejects a missing native target or mismatched fixture pixels', async () => {
  await assert.rejects(
    assembleRelease({ inputs: fiveRecords().slice(0, 4), releaseTag: RELEASE_TAG }),
    /missing release target: linux-arm64/
  );
  const changed = fiveRecords();
  changed[4].fixture.rgbaSha256 = H('9');
  await assert.rejects(
    assembleRelease({ inputs: changed, releaseTag: RELEASE_TAG }),
    /fixture (?:RGBA hash|approved identity) mismatch: linux-arm64/
  );
});
...
test('release assembly emits a closed manifest with full commits and exact portable targets', async () => {
  ...
  assert.deepEqual(
    Object.keys(result.manifest.assets),
    TARGETS.map(([target]) => target)
  );
  assert.equal(result.manifest.release.tag, RELEASE_TAG);
  await assert.rejects(
    assembleRelease({
      inputs: [...fiveRecords(), { ...fiveRecords()[0], target: 'freebsd-x64' }],
      releaseTag: RELEASE_TAG
    }),
    /unexpected release target/
  );
  const unknown = fiveRecords();
  unknown[0].surprise = true;
  await assert.rejects(assembleRelease({ inputs: unknown, releaseTag: RELEASE_TAG }), /closed schema/);
  const collision = fiveRecords();
  collision[1].archive.name = collision[0].archive.name.toUpperCase();
  await assert.rejects(
    assembleRelease({ inputs: collision, releaseTag: RELEASE_TAG }),
    /archive identity mismatch|portable asset name collision/
  );
});
...
test('assembly binds identical upstream, lock, workflow, toolchain and fixture identities', async () => {
  for (const [field, value, pattern] of [
    ...
    values[2][parent][child] = value;
    await assert.rejects(assembleRelease({ inputs: values, releaseTag: RELEASE_TAG }), pattern);
  }
  const badTag = fiveRecords();
  badTag[0].upstream.commit = 'abc';
  await assert.rejects(assembleRelease({ inputs: badTag, releaseTag: RELEASE_TAG }), /full 40-character/);
  await assert.rejects(
    assembleRelease({ inputs: fiveRecords(), releaseTag: 'pixel-snapper-v1.0.0-commit.fffffff' }),
    /release tag commit suffix/
  );
});
...
test('packager rejects unsafe inputs and writes an atomic five-file archive artifact', async () => {
  ...
  for (const [key, name, body] of [
    ...
    files[key] = path.join(dir, name);
    await fs.writeFile(files[key], body, { mode: key === 'binaryFile' ? 0o755 : 0o644 });
  }
  ...
  const result = await packagePixelSnapper({
    ...
    fixture: {
      inputRgbaSha256: FIXTURE_HASH,
      rgbaSha256: FIXTURE_HASH,
      expectedRgbaSha256: FIXTURE_HASH,
      width: 3,
      height: 3,
      paletteSha256: PALETTE_HASH
    }
  ...
  assert.deepEqual(
    ...
    [result.record.archive.name, 'target-release-record.json', ...result.archiveEntries].sort()
  );
  ...
  await assert.rejects(
    packagePixelSnapper({
      ...
      ...files,
      licenseFile: linked,
      upstream: result.record.upstream,
      build: result.record.build,
      fixture: { ...result.record.fixture, expectedRgbaSha256: result.record.fixture.rgbaSha256 }
    }),
...
test(
  'native fixture probe uses the approved source-compatible 3x3 pixels',
...
`,
    ...
    const result = await probeNativePixelSnapper({
      binaryFile,
      upstreamVersion: '1.0.0',
      expectedInputRgbaSha256: FIXTURE_HASH,
      expectedRgbaSha256: FIXTURE_HASH
    });
...
test('release verifier rejects traversal, symlink ambiguity and changed public bytes', async () => {
  await assert.rejects(
    verifyRelease({
      metadata: { schemaVersion: 1, releaseTag: RELEASE_TAG, assets: [{ name: '../escape', sha256: H('a'), size: 1 }] },
  ...
  await assert.rejects(
    verifyRelease({
      metadata,
      fetchImpl: async (url) => new Response(url.pathname.endsWith('/checksums.json') ? JSON.stringify(metadata) : 'x')
    }),
    /public release asset hash mismatch/
  );
});
...
test('release verifier cancels a chunked body immediately when its cumulative bytes exceed the hard limit', async () => {
  ...
  await assert.rejects(
    verifyRelease({ metadata: publicMetadata(), fetchImpl: async () => response }),
    /public release asset exceeded size limit: checksums\.json/
  );
            ...
            headers: {
              get: (name) =>
                name.toLowerCase() === 'location'
                  ? 'https://release-assets.githubusercontent.com/github-production-release-asset/1/checksums.json'
                  : null
            }
          };
...
test('release verifier rejects malformed or unsupported Content-Encoding before reading', async () => {
  for (const [contentEncoding, pattern] of [
    ['gzip, br', /invalid Content-Encoding/],
    ['compress', /unsupported Content-Encoding/]
  ]) {
    const state = {};
...
test('cargo-sbom normalization removes volatile time and UUID while retaining locked content', () => {
  ...
  assert.equal(
    ...
    `https://github.com/Hugo-Dz/spritefusion-pixel-snapper/sbom/${UPSTREAM}/${H('a')}`
  );
...
test('five packaged native artifacts assemble atomically and verify as exact public bytes', async () => {
  ...
  for (const [key, name, body] of [
    ...
    shared[key] = path.join(dir, name);
    await fs.writeFile(shared[key], body);
  }
  const packaged = [];
  for (const [target, rustTarget] of TARGETS) {
    ...
    const expectedBinaryName =
      target === 'windows-x64' ? 'spritefusion-pixel-snapper.exe' : 'spritefusion-pixel-snapper';
    ...
    await fs.mkdir(path.dirname(canonicalBinary));
    await fs.rename(namedBinary, canonicalBinary);
    packaged.push(
      await packagePixelSnapper({
        target,
        rustTarget,
        outputDir: artifact,
        binaryFile: canonicalBinary,
        ...shared,
        ...
        fixture: {
          ...record(target, rustTarget, target === 'windows-x64' ? 'zip' : 'tar.gz').fixture,
          expectedRgbaSha256: FIXTURE_HASH
        }
      })
    );
  ...
  assert.deepEqual(
    (await fs.readdir(outputDir)).sort(),
    [
      ...packaged.map((value) => value.record.archive.name),
      'LICENSE-Pixel-Snapper',
      'THIRD-PARTY-NOTICES',
      'pixel-snapper.spdx.json',
      'build-metadata.json',
  ...
  const verified = await verifyRelease({
    ...
    fetchImpl: async (url) =>
      new Response(await fs.readFile(path.join(outputDir, decodeURIComponent(path.posix.basename(url.pathname)))))
  });
  ...
  for (const name of await fs.readdir(outputDir))
    archiveForgery.set(name, await fs.readFile(path.join(outputDir, name)));
  ...
  await fs.appendFile(
    path.join(inputDir, 'pixel-snapper-build-linux-arm64', packaged.at(-1).record.archive.name),
    'tampered'
  );
  await assert.rejects(
    assembleRelease({ inputDir, outputDir: path.join(dir, 'bad-release'), releaseTag: RELEASE_TAG }),
...
test('workflow policy is pinned, least-privileged, native, locked and publish never executes binaries', async () => {
  ...
  assert.deepEqual(
    ...
    TARGETS.map(([key, target], index) => ({
      key,
      os: ['windows-2025', 'macos-15-intel', 'macos-15', 'ubuntu-24.04', 'ubuntu-24.04-arm'][index],
      target
    }))
  );
  ...
  for (const required of [
    ...
    'cargo-about@0.8.4',
    'rev-parse',
    '08c1323a65243400a4a6ce7ac0051ad116e39869f3276630c2a16a02cc2e05b4',
    'Image too small (minimum 3x3)',
    'immutable_releases_confirmed',
    '--json isImmutable',
    'gh release create',
    'verify-release.mjs',
    'normalize-compliance.mjs'
  ])
  ...
  assert.doesNotMatch(
    ...
    /cargo sbom|cargo about generate/
  );
  ...
  const license = await fs.readFile(
    path.join(ROOT, 'skills/pixel-sprite-animation-pipeline/references/pixel-snapper-upstream.LICENSE')
  );
...
test('release workflow verifies committed license bytes independent of checkout line endings', async () => {
  ...
  assert.match(
    source,
    new RegExp(`git -C release-tools rev-parse "HEAD:${referencePath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"`)
  );
});
...
test('release workflow hashes committed Cargo.lock bytes independent of checkout line endings', async () => {
  const workflowFile = path.join(ROOT, '.github', 'workflows', 'pixel-snapper-release.yml');
  const source = await fs.readFile(workflowFile, 'utf8');
  const committedLockRead =
    /execFileSync\("git", \["-C", "upstream", "show", `\$\{commit\}:Cargo\.lock`\], \{ encoding: null, shell: false \}\)/g;
...
test('packaged tool manifest pins the verified immutable Pixel Snapper release', async () => {
  const manifestFile = path.join(
    ROOT,
    'skills/pixel-sprite-animation-pipeline/references/pixel-snapper-tool-manifest.json'
  );
