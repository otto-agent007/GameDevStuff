...
test('real correction revalidates, records authenticated learning, and proposes only at the independent-run threshold', async () => {
  ...
  for (let number = 1; number <= 3; number += 1) {
    ...
    const good = await normalizeFrames({
      inputs: [source],
      outputDir: path.join(runDir, 'good-normalized'),
      config: DEFAULT_CONFIG
    });
    const exported = await exportAnimation({
      frames: good.frames,
      outputDir: path.join(runDir, 'good-runtime'),
      config: DEFAULT_CONFIG,
      columns: 1,
      durations: [100],
      name: 'animation'
    ...
    const result = await repairValidationRun({
      request,
      run: { runDir, corrections: [], inputs: [source, badFrame], generativeAttempts: {} },
      config: DEFAULT_CONFIG,
      expected: { metadata: expectedMetadata },
      delivery: { name: 'animation', columns: 1, durations: [100], frameSize: DEFAULT_CONFIG.runtime }
    });
    ...
    const artifacts = await Promise.all(
      delivered.map(async (file) => ({
        path: path.relative(runDir, file).replaceAll('\\', '/'),
        sha256: await sha256(file)
      }))
    );
    const correctionManifest = {
      path: path.relative(runDir, result.correction.manifest).replaceAll('\\', '/'),
    ...
    await recordRunResult({
      ...
      report: {
        ...
        lessons: [{ ...identity, correctionManifest, actionIndex }]
      }
    });
...
test('canvas correction persists while identity drift remains review-only with no delivery report', async () => {
  ...
  const good = await normalizeFrames({
    inputs: [source],
    outputDir: path.join(runDir, 'good'),
    config: DEFAULT_CONFIG
  });
  const exported = await exportAnimation({
    frames: good.frames,
  ...
  const bad = path.join(runDir, 'bad.png');
  await sharp(good.frames[0]).extract({ left: 0, top: 0, width: 127, height: 128 }).png().toFile(bad);
  const manifestBytes = `${JSON.stringify({ version: 1, runId: 'mixed-run', config: DEFAULT_CONFIG, inputs: [] }, null, 2)}\n`;
  await fs.writeFile(path.join(runDir, 'manifest.json'), manifestBytes);
  const expectedMetadata = JSON.parse(await fs.readFile(exported.metadata, 'utf8'));
  const request = {
    anchorReport,
    normalized: { ...good, frames: [bad] },
    exported,
    semanticEvidence: [{ code: 'IDENTITY_DRIFT', frame: 0, failed: true, evidence: { requires: 'artist-review' } }]
  };
  const result = await repairValidationRun({
    request,
    run: { runDir, corrections: [], inputs: [source, bad], generativeAttempts: {} },
    config: DEFAULT_CONFIG,
    expected: { metadata: expectedMetadata },
    delivery: { name: 'animation', columns: 1, durations: [100], frameSize: DEFAULT_CONFIG.runtime }
  });
  ...
  assert.deepEqual(
    result.afterValidation.failures.map(({ code }) => code),
    ['IDENTITY_DRIFT']
  );
...
test('simultaneous image and canonical/runtime metadata pivot drift repairs every owning artifact', async () => {
  ...
  const runDir = path.join(project, '.pixel-sprite-pipeline', 'runs', 'pivot-run');
  await fs.mkdir(runDir, { recursive: true, mode: 0o700 });
  const source = path.join(runDir, 'source.png');
  await makeAnchor(source);
  const anchorReport = await inspectImage(source);
  ...
  await sharp(good.frames[0])
    .extract({ left: 0, top: 0, width: 128, height: 127 })
    .extend({ top: 1, background: { r: 0, g: 0, b: 0, alpha: 0 } })
    .png()
    .toFile(badFrame);
  const damagedMetadata = structuredClone(expectedMetadata);
  damagedMetadata.canonicalPivot = { x: 60, y: 110 };
  damagedMetadata.pivot = { x: 120, y: 220 };
  await fs.writeFile(exported.metadata, `${JSON.stringify(damagedMetadata)}\n`);
  const manifestBytes = `${JSON.stringify({ version: 1, runId: 'pivot-run', config: DEFAULT_CONFIG, inputs: [] }, null, 2)}\n`;
  await fs.writeFile(path.join(runDir, 'manifest.json'), manifestBytes);
  const request = {
    anchorReport,
    normalized: { ...good, frames: [badFrame], canonicalPivot: { x: 60, y: 110 } },
    exported,
    semanticEvidence: []
  };
  const result = await repairValidationRun({
    request,
    run: { runDir, corrections: [], inputs: [source, badFrame, exported.metadata], generativeAttempts: {} },
    config: DEFAULT_CONFIG,
    expected: { metadata: expectedMetadata },
    delivery: { name: 'animation', columns: 1, durations: [100], frameSize: DEFAULT_CONFIG.runtime }
  });
  ...
  const outputs = pivotActions
    .filter(({ status }) => status === 'applied')
    .map(({ result: actionResult }) => actionResult.output);
