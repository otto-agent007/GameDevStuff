...
async function fixture() {
  ...
  await Promise.all([
    fs.writeFile(input, 'source'),
    fs.writeFile(outputOne, 'snapped-1'),
    fs.writeFile(outputTwo, 'snapped-2')
  ]);
  ...
  const frames = snapReceipt.document.payload.outputs.map((output, index) => ({
    id: contract.document.clips[0].frames[index].id,
    path: output.path,
...
async function v2Fixture() {
  ...
  const contract = await loadAnimationContract(contractFile);
  const input = path.join(projectDir, 'source.png');
  const outputFiles = ['actor', 'satchel', 'unlock-spark'].map((trackId) =>
    path.join(runDir, `unlock-release--${trackId}.png`)
  );
  await Promise.all([
  ...
  const snapReceipt = await writeSnapReceipt({
    projectDir,
    run: { id: 'run-v2', outputDir: runDir, manifestSha256: HASH('e') },
    contract,
    inputs: [input],
    outputs: outputFiles,
    args: ['16'],
  ...
  const approvals = [
    {
      ...
      landmarks: {
        ...
        sockets: [
          { id: 'hand', x: 58, y: 50 },
          { id: 'effect-origin', x: 64, y: 44 }
        ],
        contacts: [{ id: 'left-foot', x: 43, y: 83 }],
        groundTravel: { x: 0, y: 0 }
      },
      approved: true,
      approvedBy: 'owner',
...
test('frame approval is created only after snap and covers every ordered output hash', async () => {
  ...
  await assert.rejects(
    writeFrameApproval({ ...value, approvals: value.approvals.slice(1), version: 2 }),
...
test('frame approval rejects a symlinked signed snap receipt', async (t) => {
  ...
  try {
    await fs.symlink(path.basename(value.snapReceipt.path), alias);
  } catch (error) {
    if (error.code === 'EPERM') {
...
test('approval verification rejects tampered, extra, missing, duplicate, and mismatched ordered frames', async () => {
  ...
  for (const mutate of [
    ...
    (payload) => {
      payload.frames[1].id = payload.frames[0].id;
    },
    (payload) => {
    ...
    await assert.rejects(
      verifyFrameApproval({
        ...
        snapReceipt: second.snapReceipt,
        version: 1
      }),
      /schema|frame|order|coverage|approval/i
    );
  }
});
...
test('numbered approval revisions are immutable and selected explicitly', async () => {
  ...
  const second = await writeFrameApproval({
    ...value,
    version: 2,
    approvals: value.approvals.map((item) => ({ ...item, landmark: { ...item.landmark, x: item.landmark.x + 1 } }))
  });
  ...
  await assert.rejects(
    verifyFrameApproval({
      projectDir: relocated.projectDir,
      file: renamed,
      contract: relocated.contract,
      snapReceipt: relocated.snapReceipt,
      version: 1
    }),
    /numbered|versioned/i
  );
});
...
test('frame approvals reject a signed escaped output chain but retain manual approval provenance', async () => {
  ...
  await writeSignedState({
    projectDir: escaped.projectDir,
    file: receiptFile,
    domain: 'pixel-sprite-snap-receipt/v1',
    payload
  });
  const escapedFrames = structuredClone(escaped.frames);
  escapedFrames[0].path = '../escaped.png';
  await assert.rejects(
    writeFrameApproval({ ...escaped, snapReceipt: { path: receiptFile }, frames: escapedFrames, version: 1 }),
  ...
  const receipt = await writeManualHandoffReceipt({
    projectDir: manual.projectDir,
    run: { id: 'manual-1', outputDir: manual.runDir, manifestSha256: HASH('d') },
    handoff,
    inputs: [path.join(manual.projectDir, 'source.png')],
    outputs: [path.join(manual.runDir, 'idle-01.png'), path.join(manual.runDir, 'idle-02.png')]
  });
