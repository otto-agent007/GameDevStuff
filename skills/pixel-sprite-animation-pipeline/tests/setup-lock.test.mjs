...
async function project() {
  return fs.mkdtemp(path.join(os.tmpdir(), 'pixel-snapper-lock-'));
}
function lockRoot(projectDir) {
  return path.join(projectDir, '.pixel-sprite-pipeline', 'tools', '.locks', TAG);
}
...
async function seedTicket(projectDir, owner, name = `ticket-${owner.nonce}`) {
  ...
  const sequence = {
    ...
    sequence: 1,
    nonce: owner.nonce,
    pendingBasename: `.pending-sequence-${owner.nonce}-${publicationToken}`,
    publicationToken
  };
  await fs
    .writeFile(path.join(lockRoot(projectDir), 'sequence-0000000000000001'), `${JSON.stringify(sequence)}\n`, {
      flag: 'wx'
    })
    .catch((error) => {
...
test('concurrent setup operations are serialized by immutable per-owner tickets', async () => {
  ...
  assert.deepEqual(
    (await fs.readdir(lockRoot(projectDir))).filter((name) => name.startsWith('ticket-')),
    []
  );
});
...
test('unknown owner state fails closed and exposes stable lock-contention classification', async () => {
  ...
  assert.equal(
    JSON.parse(await fs.readFile(path.join(lockRoot(projectDir), `ticket-${owner.nonce}`, 'owner.json'))).nonce,
    owner.nonce
  );
});
...
test('release touches only the caller ticket whose directory identity and owner still match', async () => {
  ...
  await withSetupLock({
    ...
    operation: async () => {
      ...
      await fs.writeFile(
        path.join(ticket, 'owner.json'),
        `${JSON.stringify({ ...owner, nonce: '22222222-2222-4222-8222-222222222222' })}\n`
      );
    }
  });
  assert.equal(
    JSON.parse(await fs.readFile(path.join(ticket, 'owner.json'))).nonce,
...
test('operation failure releases its ticket and traversal-like tags are rejected', async () => {
  ...
  await assert.rejects(
    withSetupLock({ projectDir, releaseTag: '../escape', operation: async () => {} }),
    /invalid Pixel Snapper release tag/
  );
});
...
test('permission bits are enforced only when POSIX uid capability exists', async () => {
  ...
  await assert.rejects(
    withSetupLock({ projectDir, releaseTag: TAG, operation: async () => {}, getUid: () => process.getuid() }),
    /unsafe Pixel Snapper setup directory/
  );
  assert.equal(
    await withSetupLock({ projectDir, releaseTag: TAG, operation: async () => 'windows-capability', getUid: null }),
    'windows-capability'
  );
});
...
test('a delayed publisher receives a later sequence and cannot overlap an already-active owner', async () => {
  ...
  const seenPublished = new Promise((resolve) => {
    published = resolve;
  });
  let active = 0;
  let maximum = 0;
  ...
  const delayed = withSetupLock({
    projectDir,
    releaseTag: TAG,
    now: () => 100,
    hooks: {
      afterTicketPublished: async () => {
        published();
        await pause;
      }
    },
...
test('a crash after sequence hardlink publication recovers the owned pending link', async () => {
  ...
  assert.equal(
    await withSetupLock({
      ...
      releaseTag: TAG,
      now: () => 700_000,
      processProbe: (pid) => (pid === owner.pid ? 'dead' : 'alive'),
      operation: async () => 'recovered'
    }),
    'recovered'
  );
...
test('a prefix-matching alternate cannot substitute for the exact pending sequence basename', async () => {
  ...
  await fs.link(
    sequence,
    path.join(lockRoot(projectDir), `.pending-sequence-${owner.nonce}-bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb`)
  );
  let tick = 1;
  await assert.rejects(
    withSetupLock({
...
test('a contender never unlinks a healthy publisher pending sequence', async () => {
  ...
  const publisher = withSetupLock({
    projectDir,
    releaseTag: TAG,
    hooks: {
      afterSequenceLinked: async () => {
        linked();
        await pause;
      }
    },
