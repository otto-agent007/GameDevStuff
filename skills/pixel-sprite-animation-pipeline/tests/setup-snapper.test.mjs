...
test('idempotent reuse revalidates the receipt, executable hash, and deterministic fixture', async (t) => {
  ...
  assert.deepEqual(
    ...
    [fixture.manifest.assets[fixture.target].executable, ...SUPPORT_FILES.map(([name]) => name)]
  );
  assert.ok(
    receipt.installedFiles.every((item) => Number.isSafeInteger(item.size) && /^[a-f0-9]{64}$/.test(item.sha256))
  );
});
...
test('setup quarantines a changed cached executable until force restore', async (t) => {
  ...
  await assert.rejects(
    setupPixelSnapper(fixture.options),
    (error) => error.code === 'PIXEL_SNAPPER_INSTALLATION_TAMPERED' && /hash mismatch/.test(error.cause?.message ?? '')
  );
  await assert.rejects(fs.lstat(fixture.finalDir), { code: 'ENOENT' });
  assert.ok(
    (await fs.readdir(path.dirname(fixture.finalDir))).some((name) => name.startsWith(`.${fixture.target}.tampered-`))
  );
...
test('a changed installation receipt is quarantined and cannot authorize reuse', async (t) => {
  ...
  await assert.rejects(
    setupPixelSnapper(fixture.options),
    (error) =>
      error.code === 'PIXEL_SNAPPER_INSTALLATION_TAMPERED' &&
      /installation receipt mismatch/.test(error.cause?.message ?? '')
  );
  await assert.rejects(fs.lstat(fixture.finalDir), { code: 'ENOENT' });
});
...
test('failed setup removes only its owned interrupted stage and never activates partial content', async (t) => {
  ...
  await assert.rejects(
    setupPixelSnapper(failed),
    (error) => error.code === 'PIXEL_SNAPPER_NETWORK_ERROR' && /network interrupted/.test(error.cause?.message ?? '')
  );
  await assert.rejects(fs.lstat(fixture.finalDir), { code: 'ENOENT' });
  const tools = path.join(fixture.projectDir, '.pixel-sprite-pipeline', 'tools');
  const staging = path.join(tools, '.staging');
  assert.deepEqual(
    await fs.readdir(staging).catch((error) => (error.code === 'ENOENT' ? [] : Promise.reject(error))),
    []
  );
...
test('force replacement stages and verifies the new installation before activation', async (t) => {
  ...
  const fixture = await setupFixture();
  const first = await setupPixelSnapper(fixture.options);
  const failed = setupPixelSnapper({
    ...fixture.options,
    force: true,
    fetchImpl: async () => {
  ...
  await assert.rejects(
    failed,
    (error) => error.code === 'PIXEL_SNAPPER_NETWORK_ERROR' && /replacement failed/.test(error.cause?.message ?? '')
  );
...
test('idempotent reuse removes an owned install stage left by an interrupted process', async (t) => {
  ...
  const stage = path.join(
    ...
    'tools',
    '.staging',
    `.install-${fixture.tag}-${fixture.target}-${nonce}`
  );
  await fs.mkdir(stage, { recursive: true, mode: 0o700 });
  const stageInfo = await fs.lstat(stage);
  await fs.writeFile(
    path.join(stage, '.pixel-snapper-install-stage.json'),
    `${JSON.stringify({
      schemaVersion: 1,
...
test('reuse rejects unexpected installed files instead of trusting an incomplete inventory', async (t) => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error.code === 'PIXEL_SNAPPER_INSTALLATION_TAMPERED' &&
      /installed file inventory mismatch/.test(error.cause?.message ?? '')
  );
  await assert.rejects(fs.lstat(fixture.finalDir), { code: 'ENOENT' });
});
...
test('reuse rejects a hard-linked managed executable', async (t) => {
  ...
  await assert.rejects(
    setupPixelSnapper(fixture.options),
    (error) =>
      error.code === 'PIXEL_SNAPPER_INSTALLATION_TAMPERED' && /must have one link/.test(error.cause?.message ?? '')
  );
  await assert.rejects(fs.lstat(fixture.finalDir), { code: 'ENOENT' });
});
...
test('interrupted-stage cleanup preserves a path replacement whose persisted identity does not match', async (t) => {
  ...
  const marker = {
    ...
    target: fixture.target,
    nonce,
    dev: originalInfo.dev,
    ino: originalInfo.ino
  };
...
test('activation failure restores only a reverified identity-bound backup', async (t) => {
  ...
  await assert.rejects(
    ...
    (error) =>
      error.code === 'PIXEL_SNAPPER_ACTIVATION_FAILED' && /injected activation failure/.test(error.cause?.message ?? '')
  );
...
async function seedActivationBackup(fixture, { tamper = false } = {}) {
  ...
  await fs.writeFile(
    `${backup}.marker.json`,
    `${JSON.stringify({ schemaVersion: 1, kind: 'pixel-snapper-install-move', reason: 'activation', nonce, dev: info.dev, ino: info.ino })}\n`
  );
  return backup;
}
...
test('setup exposes stable classifications for platform, download, archive, extraction, executable, and probe failures', async (t) => {
  ...
  await assert.rejects(
    setupPixelSnapper({ ...unsupported.options, platform: { platform: 'freebsd', arch: 'x64' } }),
    (error) => error.code === 'PIXEL_SNAPPER_UNSUPPORTED_PLATFORM'
  );
  ...
  await assert.rejects(
    ...
    (error) => error.code === 'PIXEL_SNAPPER_NETWORK_ERROR'
  );
  ...
  await assert.rejects(
    setupPixelSnapper({
      ...missing.options,
      fetchImpl: async () => ({ status: 404, ok: false, headers: { get: () => null }, body: null })
    }),
    (error) => error.code === 'PIXEL_SNAPPER_RELEASE_NOT_FOUND'
  );
  ...
  await assert.rejects(
    setupPixelSnapper({
      ...integrity.options,
      fetchImpl: async () => response(Buffer.alloc(integrity.archive.length))
    }),
    (error) => error.code === 'PIXEL_SNAPPER_ARCHIVE_INTEGRITY'
  );
  ...
  await assert.rejects(
    setupPixelSnapper({
      ...extraction.options,
      faults: {
        beforeExtraction: async () => {
          throw new Error('injected extraction');
        }
      }
    }),
    (error) => error.code === 'PIXEL_SNAPPER_EXTRACTION_FAILED'
  );
  ...
  await assert.rejects(
    setupPixelSnapper(mismatch.options),
    (error) => error.code === 'PIXEL_SNAPPER_EXECUTABLE_MISMATCH'
  );

  const probe = await setupFixture();
  const brokenProbe = Buffer.from('#!/usr/bin/env node\nprocess.exit(9)\n');
  await probe.setArchive(await tarGz(probe.manifest.assets[probe.target].executable, brokenProbe), {
    executableSize: brokenProbe.length,
    executableSha256: hash(brokenProbe)
  });
...
test('a crash after stage marker creation leaves only unpublished non-blocking debris', async (t) => {
  ...
  await assert.rejects(
    setupPixelSnapper({
      ...fixture.options,
      faults: {
        afterStageMarker: async () => {
          throw new Error('crash after stage marker');
        }
      }
    }),
    /crash after stage marker/
  );
  assert.equal((await setupPixelSnapper(fixture.options)).status, 'installed');
});
...
test('an orphan prepublished backup marker is harmless and the active installation remains canonical', async (t) => {
  ...
  await assert.rejects(
    setupPixelSnapper({
      ...
      force: true,
      faults: {
        afterMoveMarker: async () => {
          throw new Error('crash before backup rename');
        }
      }
    }),
    /crash before backup rename/
  );
  assert.equal((await setupPixelSnapper(fixture.options)).identity.sha256, installed.identity.sha256);
});
...
test('final verification failure keeps the verified rollback backup until restoration completes', async (t) => {
  ...
  await assert.rejects(
    ...
    (error) => error.code === 'PIXEL_SNAPPER_ACTIVATION_FAILED'
  );
  assert.equal((await setupPixelSnapper(fixture.options)).identity.sha256, installed.identity.sha256);
});
...
test('failed verification after recovery rename quarantines the canonical directory before returning', async (t) => {
  ...
  await assert.rejects(
    setupPixelSnapper({
      ...fixture.options,
      faults: {
        afterRecoveryRename: async ({ finalDir }) => {
          await fs.appendFile(
            path.join(finalDir, fixture.manifest.assets[fixture.target].executable),
            'tamper after recovery rename'
          );
        }
      }
    }),
...
test('failed recovery move writes an identity-bound invalidation marker into canonical state', async (t) => {
  ...
  await assert.rejects(
    setupPixelSnapper({
      ...
      faults: {
        afterRecoveryRename: async ({ finalDir }) =>
          fs.appendFile(path.join(finalDir, fixture.manifest.assets[fixture.target].executable), 'tamper'),
        beforeDeactivationMove: async () => {
...
for (const crashPoint of ['afterCleanupDirectoryMove', 'afterCleanupMarkerMove']) {
  test(`guarded backup cleanup recovers interruption ${crashPoint}`, async (t) => {
    ...
    assert.equal((await setupPixelSnapper(fixture.options)).status, 'already-installed');
    assert.deepEqual(
      (await fs.readdir(path.dirname(fixture.finalDir))).filter((name) => name.includes('.cleanup-')),
      []
    );
  });
}
