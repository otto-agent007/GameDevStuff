    ...
    clips: [
      {
        id: 'run',
        loopMode: 'loop',
        loopTransition: { fromFrameId: 'run-02', toFrameId: 'run-00', reviewCheckpoint: 'motion' },
        frames: [
...
async function v2ContractFixture() {
  ...
  for (const [frameIndex, definition] of document.clips[0].frames.entries()) {
    ...
    for (const [trackIndex, trackId] of definition.tracks.entries()) {
      ...
      tracks[trackId] = {
        ...
        sourceSha256: HASH(String(trackIndex + 1)),
        path: file,
        normalizedSha256
      };
    ...
    frames.push({
      ...
      root: { x: 8, y: 14 },
      baseline: 13,
      sockets: { hand: { x: 12, y: 8 } },
      contacts: { 'left-foot': { x: 8 + frameIndex, y: 13 } },
  ...
  const normalized = {
    ...
    frameApprovalSha256: HASH('e'),
    snapReceiptSha256: HASH('f'),
    frames
  };
  const config = {
    ...DEFAULT_CONFIG,
  ...
  return {
    ...
    normalized,
    contract,
    outputDir: path.join(dir, 'contract-v2-out'),
    config,
    columns: 2,
    frameApprovalSha256: HASH('e')
  };
...
test('v2 contract export emits track frames and engine-neutral provenance', async () => {
  ...
  assert.deepEqual(
    index.clips[0].frames.map(({ id, semantic, duration, tracks }) => ({ id, semantic, duration, tracks })),
    [
  ...
  assert.ok(
    index.clips[0].frames.every((frame) => frame.outputs.every((output) => /^[a-f0-9]{64}$/.test(output.sha256)))
  );
...
test('contract export preserves exact clip order, frame IDs, nonuniform durations, loop modes, and bindings', async () => {
  ...
  assert.deepEqual(
    index.clips.map(({ id }) => id),
    ['run', 'salute']
  );
  assert.deepEqual(
    index.clips[0].frames.map(({ id, duration }) => ({ id, duration })),
    [
      { id: 'run-00', duration: 80 },
      { id: 'run-01', duration: 90 },
      { id: 'run-02', duration: 110 }
    ]
  );
  assert.ok(
    index.clips
      .flatMap((clip) => [clip.sheet, clip.metadata, clip.preview, ...clip.frames.map((frame) => frame.file)])
      .every((file) => !path.isAbsolute(file) && !file.includes('\\') && !file.startsWith('../'))
  );
...
test('contract export rejects missing, extra, reordered, and unsafe frames before atomic publication', async (t) => {
  for (const [label, mutate, pattern] of [
    [
      'missing',
      (fixture) => {
        fixture.normalized.frames.pop();
        fixture.normalized.measurements.pop();
      },
      /exact ordered normalized frame coverage/i
    ],
    [
      'extra',
      (fixture) => {
        fixture.normalized.frames.push(fixture.normalized.frames[0]);
        fixture.normalized.measurements.push({ ...fixture.normalized.measurements[0], frameId: 'extra' });
      },
      /exact ordered normalized frame coverage/i
    ],
    [
      'reordered',
      (fixture) => {
        fixture.normalized.measurements.reverse();
      },
      /frame order/i
    ],
    [
      'missing landmark',
      (fixture) => {
        delete fixture.normalized.measurements[0].sourceLandmark;
      },
      /landmark measurements/i
    ],
    [
      'unsafe clip',
      (fixture) => {
        fixture.contract.document.clips[0].id = '../escape';
        fixture.contract.sha256 = stableHash(fixture.contract.document);
      },
      /safe.*clip/i
    ],
    [
      ...
      (fixture) => {
        fixture.contract.document.clips[0].id = 'idle.';
        fixture.contract.sha256 = stableHash(fixture.contract.document);
      },
      /portable.*clip|safe.*clip/i
    ],
    [
      'reserved clip',
      (fixture) => {
        fixture.contract.document.clips[0].id = 'CON';
        fixture.contract.sha256 = stableHash(fixture.contract.document);
      },
      /portable.*clip|safe.*clip/i
    ],
    [
      ...
      (fixture) => {
        fixture.contract.document.clips[0].id = 'Idle';
        fixture.contract.document.clips[1].id = 'idle';
        fixture.contract.sha256 = stableHash(fixture.contract.document);
      },
      /portable.*unique|collision/i
    ]
  ])
    await t.test(label, async () => {
      ...
      assert.deepEqual(
        (await fs.readdir(fixture.dir)).filter((entry) => entry.includes('.sprite-contract-stage-')),
        []
      );
    });
});
...
test('exports nearest-neighbor runtime frames, a transparent partial sheet row, metadata, and a lossless loop', async () => {
  ...
  assert.deepEqual(
    result.runtimeFrames.map((file) => path.basename(file)),
    ['test-run-00.png', 'test-run-01.png', 'test-run-02.png']
  );
  ...
  assert.deepEqual(metadata.config, {
    ...
    integration: { forbiddenIntegrationPaths: [], projectId: null },
...
test('validates every argument before creating the output directory', async (t) => {
  ...
  const cases = [
    ...
    [
      'duration below reliable WebP limit',
      { durations: [10] },
      /one integer duration per frame in the range 11\.\.65535/
    ],
    ...
    [
      'integer horizontal scale',
      { config: { ...DEFAULT_CONFIG, runtime: { width: 255, height: 256 } } },
      /runtime width must be an integer multiple/
    ],
    [
      'integer vertical scale',
      { config: { ...DEFAULT_CONFIG, runtime: { width: 256, height: 255 } } },
      /runtime height must be an integer multiple/
    ],
    [
      'uniform scale',
      { config: { ...DEFAULT_CONFIG, runtime: { width: 256, height: 384 } } },
      /runtime scale must be identical on both axes/
    ]
  ];
...
test('preserves legacy flat-export filename stems while contract clip IDs use stricter portability rules', async () => {
  ...
  const result = await exportAnimation({
    frames: [frame],
    outputDir: path.join(dir, 'legacy'),
    config: DEFAULT_CONFIG,
    columns: 1,
    durations: [80],
    name: 'legacy-'
...
test('rendering failure leaves no output directory or staging debris', async () => {
  ...
  await assert.rejects(fs.access(outputDir), { code: 'ENOENT' });
  assert.deepEqual(
    (await fs.readdir(dir)).filter((entry) => entry.includes('.sprite-export-stage-')),
    []
  );
});
