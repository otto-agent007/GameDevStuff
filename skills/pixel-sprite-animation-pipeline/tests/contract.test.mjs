...
import {
  createCorrectionContract,
  loadCorrectionContext,
  sealCorrectionContract,
  stableHash
...
async function fixture() {
  ...
  const normalized = await normalizeFrames({
    inputs: [source],
    outputDir: path.join(runDir, 'normalized'),
    config: DEFAULT_CONFIG
  });
  const exported = await exportAnimation({
    frames: normalized.frames,
    outputDir: path.join(runDir, 'runtime'),
    config: DEFAULT_CONFIG,
    columns: 1,
    durations: [100],
    name: 'animation'
  ...
  const provenance = {
    animationContractSha256: 'a'.repeat(64),
    snapReceiptSha256: 'b'.repeat(64),
    frameApprovalSha256: 'c'.repeat(64),
    toolProvenanceVerified: false
  };
  const contract = await createCorrectionContract({
    runDir,
    runId: 'run-1',
    config: DEFAULT_CONFIG,
    anchorReport: await inspectImage(source),
    normalized,
    exported,
    provenance
  });
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify({ runId: 'run-1', manifestSha256, correctionContract: { path: 'correction-contract-v1.json', sha256: contract.sha256 }, ...provenance }, null, 2)}\n`
  );
  const receipts = await Promise.all(
    Array.from({ length: 8 }, () => sealCorrectionContract({ projectDir: project, runDir, runId: 'run-1', contract }))
  );
...
test('correction contracts authenticate the complete animation approval chain before repair', async () => {
  ...
  const loaded = await loadCorrectionContext({
    projectDir: value.project,
    runId: 'run-1',
    contractSha256: value.contract.sha256,
    receiptSha256: value.receipt.sha256,
    receiptSignature: value.receipt.signature
  });
  ...
  await assert.rejects(
    ...
    /receipt|approval|provenance|report/i
  );
});
...
test('immutable correction contract authenticates ancestors and permits only the declared corrupted target', async () => {
  ...
  await sharp(value.normalized.frames[0])
    .extract({ left: 0, top: 0, width: 127, height: 128 })
    .png()
    .toFile(`${value.normalized.frames[0]}.changed`);
  ...
  const targeted = await loadCorrectionContext({
    projectDir: value.project,
    runId: 'run-1',
    contractSha256: value.contract.sha256,
    receiptSha256: value.receipt.sha256,
  ...
  await assert.rejects(
    loadCorrectionContext({
      ...
      receiptSignature: value.receipt.signature,
      declaredFailure: { code: 'CANVAS_SIZE', stage: 'canonical', frame: 0 },
      targetOverride: substitute
    }),
    /unknown correction request field|caller may not choose/
  );
});
...
test('metadata expectations remain immutable when failed metadata fields are tampered', async () => {
  ...
  const loaded = await loadCorrectionContext({
    ...
    receiptSha256: value.receipt.sha256,
    receiptSignature: value.receipt.signature,
    declaredFailure: { code: 'METADATA_MISMATCH', stage: 'metadata' }
  });
...
test('replacement contract is rejected even when caller supplies its new hash', async () => {
  ...
  await assert.rejects(
    loadCorrectionContext({
      projectDir: value.project,
      runId: 'run-1',
      contractSha256: await sha256(value.contract.path),
      receiptSha256: value.receipt.sha256,
      receiptSignature: value.receipt.signature,
      declaredFailure: { code: 'CANVAS_SIZE', stage: 'canonical', frame: 0 }
    }),
...
test('signed receipt fails closed when missing stale or jointly replaced with report and contract', async () => {
  ...
  await assert.rejects(
    loadCorrectionContext({
      ...
      receiptSignature: document.signature
    }),
    /receipt signature|contract hash/
  );
});
...
test(
  'signed correction loading rejects group or world-accessible key directories',
...
test(
  'signed correction loading rejects a group or world-writable project state directory',
  { skip: process.platform === 'win32' },
  async () => {
    const value = await fixture();
...
test(
  'signed correction loading rejects signing keys and key directories owned by another uid',
  {
    skip:
  ...
  async (context) => {
    ...
    try {
      await fs.chown(key, 1, process.getegid());
    } catch (error) {
    ...
    await assert.rejects(
      loadCorrectionContext({
        projectDir: keyValue.project,
        runId: 'run-1',
        contractSha256: keyValue.contract.sha256,
        receiptSha256: keyValue.receipt.sha256,
        receiptSignature: keyValue.receipt.signature
      }),
    ...
    await assert.rejects(
      loadCorrectionContext({
        projectDir: directoryValue.project,
        runId: 'run-1',
        contractSha256: directoryValue.contract.sha256,
        receiptSha256: directoryValue.receipt.sha256,
        receiptSignature: directoryValue.receipt.signature
      }),
...
test('secure existing state directory key directory and signing key remain usable', async () => {
  ...
  if (process.platform !== 'win32') {
    ...
    if (typeof process.geteuid === 'function')
      assert.deepEqual(
        [stateStat.uid, keysStat.uid, keyStat.uid],
        [process.geteuid(), process.geteuid(), process.geteuid()]
      );
  }
  const loaded = await loadCorrectionContext({
    projectDir: value.project,
...
test(
  'an existing owner-only key directory remains usable without republishing the key',
  { skip: process.platform === 'win32' },
  async () => {
    const value = await fixture();
    const keys = path.join(value.project, '.pixel-sprite-pipeline', 'keys');
    await fs.chmod(keys, 0o500);
    const resealed = await sealCorrectionContract({
      projectDir: value.project,
      runDir: value.runDir,
      runId: 'run-1',
...
test(
  'signed correction loading requires an existing key to remain exactly 0600',
  { skip: process.platform === 'win32' },
  async () => {
    const value = await fixture();
    const key = path.join(value.project, '.pixel-sprite-pipeline', 'keys', 'correction-signing-v1.key');
...
test('contract containment compares canonical physical paths while recording slash-portable paths', async (context) => {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify({ version: 1, runId: 'alias-run', config: DEFAULT_CONFIG, inputs: [] })}\n`
  );
  const alias = path.join(project, 'physical-run-alias');
  try {
    await fs.symlink(runDir, alias, 'dir');
  } catch (error) {
    if (['EPERM', 'ENOSYS'].includes(error.code)) {
      context.skip('directory aliases unavailable');
      return;
    }
    throw error;
  }
...
test('contract containment rejects an external original anchor instead of confusing it with the staged approved anchor', async () => {
  ...
  await fs.writeFile(
    path.join(runDir, 'manifest.json'),
    `${JSON.stringify({ version: 1, runId: 'external-run', config: DEFAULT_CONFIG, inputs: [] })}\n`
  );
  await assert.rejects(
    createCorrectionContract({
      runDir,
      runId: 'external-run',
      config: DEFAULT_CONFIG,
      anchorReport: await inspectImage(external),
      normalized,
      exported
    }),
...
test('Windows canonical containment is case-insensitive, slash-portable, and drive-safe', async () => {
  ...
  assert.equal(
    module.portableContainedPath(
      'C:\\Users\\RUNNER~1\\project\\run',
      'c:\\users\\runner~1\\project\\run\\source\\anchor.png',
      path.win32
    ),
    'source/anchor.png'
  );
  assert.throws(
    () => module.portableContainedPath('C:\\project\\run', 'D:\\project\\run\\anchor.png', path.win32),
    /artifact escaped the run/
  );
  assert.throws(
    () => module.portableContainedPath('C:\\project\\run', 'C:\\project\\run-sibling\\anchor.png', path.win32),
    /artifact escaped the run/
  );
});
