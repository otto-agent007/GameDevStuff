...
function zipFixture(entries, options = {}) {
  return Buffer.from(
    zipSync(Object.fromEntries(entries.map(({ name, data = '' }) => [name, Buffer.from(data)])), options)
  );
}
...
test('ZIP preflight and extraction produce only the complete expected regular-file set', async () => {
  const bytes = zipFixture([
    { name: 'pixel-snapper.exe', data: 'binary' },
    { name: 'LICENSE.txt', data: 'license' }
  ]);
  const inspection = inspectArchive({
    bytes,
    format: 'zip',
    expectedFiles: ['pixel-snapper.exe', 'LICENSE.txt'],
    limits: LIMITS
  });
  ...
  assert.deepEqual(
    result.files.map((file) => path.relative(outputDir, file)),
    ['pixel-snapper.exe', 'LICENSE.txt']
  );
...
test('tar.gz preflight preserves safe nested files and archive executable metadata', async () => {
  ...
  const inspection = inspectArchive({
    bytes,
    format: 'tar.gz',
    expectedFiles: ['bin/pixel-snapper', 'share/data.bin'],
    limits: LIMITS
  });
  const { outputDir } = await emptyDestination();
...
test('archive preflight writes nothing for a case-fold or Unicode-normalization collision', async () => {
  for (const entries of [
    ...
    [
      { name: 'caf\u00e9.txt', data: 'a' },
      { name: 'cafe\u0301.txt', data: 'b' }
    ]
  ]) {
    const { parent } = await emptyDestination();
    const bytes = zipFixture(entries);
    assert.throws(
      () => inspectArchive({ bytes, format: 'zip', expectedFiles: [entries[0].name], limits: LIMITS }),
      /case-fold collision/
    );
...
test('archive rejects ZIP and TAR links and all non-regular entries', async () => {
  ...
  assert.throws(
    () => inspectArchive({ bytes: symlinkZip, format: 'zip', expectedFiles: ['tool'], limits: LIMITS }),
    /unsafe archive entry/
  );
  ...
  for (const type of ['symlink', 'link', 'directory', 'fifo', 'character-device', 'block-device']) {
    ...
    assert.throws(
      () => inspectArchive({ bytes, format: 'tar.gz', expectedFiles: ['tool'], limits: LIMITS }),
      /unsafe archive entry/,
...
test('archive enforces entry, compressed, total, per-file, and compression-ratio limits', async () => {
  ...
  assert.throws(
    () => inspectArchive({ bytes: two, format: 'zip', expectedFiles: ['a', 'b'], limits: { ...LIMITS, entries: 1 } }),
    /entry limit/
  );
  assert.throws(
    () =>
      inspectArchive({
        ...
        format: 'zip',
        expectedFiles: ['a', 'b'],
        limits: { ...LIMITS, compressed: two.length - 1 }
      }),
    /compressed size limit/
  );
  assert.throws(
    () => inspectArchive({ bytes: two, format: 'zip', expectedFiles: ['a', 'b'], limits: { ...LIMITS, total: 1 } }),
    /total size limit/
  );
  ...
  assert.throws(
    () =>
      inspectArchive({ bytes: large, format: 'zip', expectedFiles: ['tool'], limits: { ...LIMITS, perFile: 4095 } }),
    /per-file size limit/
  );
  assert.throws(
    () => inspectArchive({ bytes: large, format: 'zip', expectedFiles: ['tool'], limits: { ...LIMITS, ratio: 2 } }),
  ...
  assert.throws(
    () =>
      inspectArchive({ bytes: tarBomb, format: 'tar.gz', expectedFiles: ['tool'], limits: { ...LIMITS, ratio: 2 } }),
    /compression ratio limit/
  );
});
...
test('ZIP rejects DEFLATE and stored streams that expand beyond forged declared sizes', () => {
  ...
  for (const bytes of [
    ...
    assert.throws(
      () => inspectArchive({ bytes, format: 'zip', expectedFiles: ['tool.exe'], limits: LIMITS }),
      /declared size|invalid ZIP archive/
    );
  }
});
...
test('archive callers cannot weaken the built-in safety ceilings', () => {
  ...
  for (const limits of [
    ...
    assert.throws(
      () => inspectArchive({ bytes, format: 'zip', expectedFiles: ['tool.exe'], limits }),
      /invalid archive limits/
    );
  }
});
...
test('archive rejects missing, unexpected, duplicate, corrupt, and unsupported inputs', async () => {
  const bytes = zipFixture([{ name: 'tool.exe', data: 'binary' }]);
  assert.throws(
    () => inspectArchive({ bytes, format: 'zip', expectedFiles: ['tool.exe', 'LICENSE'], limits: LIMITS }),
    /expected file set/
  );
  assert.throws(() => inspectArchive({ bytes, format: 'zip', expectedFiles: [], limits: LIMITS }), /expected file set/);
  assert.throws(
    () => inspectArchive({ bytes, format: 'rar', expectedFiles: ['tool.exe'], limits: LIMITS }),
    /unsupported archive format/
  );
  assert.throws(
    () =>
      inspectArchive({ bytes: Buffer.from('not a zip'), format: 'zip', expectedFiles: ['tool.exe'], limits: LIMITS }),
    /invalid ZIP archive/
  );
  ...
  corrupted[0] ^= 0xff;
  assert.throws(
    () => inspectArchive({ bytes: corrupted, format: 'zip', expectedFiles: ['tool.exe'], limits: LIMITS }),
    /invalid ZIP archive/
  );
});
...
test('failed archive preflight never creates an output directory and inspections cannot be forged', async () => {
  ...
  await assert.rejects(
    extractInspectedArchive({ inspection: Object.freeze({}), outputDir }),
    /invalid archive inspection/
  );
  await assert.rejects(fs.access(outputDir));
});
...
test('extraction rejects symlinked ancestors and insecure final parents', async (context) => {
  ...
  await assert.rejects(
    extractInspectedArchive({ inspection, outputDir: path.join(linked, 'published') }),
    /unsafe archive output parent/
  );
  ...
  if (mode !== 0o777) context.skip(`filesystem did not retain insecure mode: ${mode.toString(8)}`);
    ...
    await assert.rejects(
      extractInspectedArchive({ inspection, outputDir: path.join(secure, 'published') }),
      /unsafe archive output parent/
    );
});
