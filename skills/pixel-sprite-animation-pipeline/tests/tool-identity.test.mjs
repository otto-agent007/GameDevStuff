...
async function managedBinaryFixture() {
  ...
  const executable = path.join(
    ...
    'tools',
    'pixel-snapper',
    'pixel-snapper-v1.2.3-commit.0123456',
    'linux-x64',
    'snapper'
  );
...
test(
  'external binary records no pinned identity unless its hash matches',
...
test(
  'PATH lookup is deterministic and records its origin',
  { skip: process.platform === 'win32' && 'POSIX executable fixture' },
  async () => {
...
test('explicit missing binary failures propagate instead of becoming handoffs', async () => {
  ...
  await assert.rejects(
    resolvePixelSnapper({
      ...
      configProvenance: { snapperExecutable: 'default' },
      manifest,
      env: { PIXEL_SNAPPER_BIN: path.join(projectDir, 'missing') },
      pathValue: '',
      platform: { platform: 'linux', arch: 'x64' }
    }),
    { code: 'ENOENT' }
  ...
  await assert.rejects(
    resolvePixelSnapper({
      ...
      configProvenance: { snapperExecutable: 'profile' },
      manifest,
      env: {},
      pathValue: '',
      platform: { platform: 'linux', arch: 'x64' }
...
test('managed cache rejects a symlinked executable even when it points to matching bytes', async (t) => {
  ...
  try {
    await fs.symlink(outside, fixture.executable);
  } catch (error) {
    if (error.code === 'EPERM') {
...
test('Windows PATH candidate lookup adds exe only when PATHEXT permits it', () => {
  ...
  assert.deepEqual(
    pixelSnapperPathCandidates('spritefusion-pixel-snapper', binDir, 'win32', { PATHEXT: '.COM;.EXE' }),
    [path.join(binDir, 'spritefusion-pixel-snapper'), path.join(binDir, 'spritefusion-pixel-snapper.exe')]
  );
...
test(
  'a hash pinned for another target does not receive current-platform attribution',
  { skip: process.platform === 'win32' && 'POSIX executable fixture' },
  async () => {
    ...
    manifest.assets['linux-x64'] = {
      ...manifest.assets['linux-x64'],
      executableSha256: 'd111111111111111111111111111111111111111111111111111111111111111'
    };
