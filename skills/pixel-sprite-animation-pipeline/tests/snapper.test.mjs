...
test('missing Pixel Snapper produces a resumable manifest', async () => {
  ...
  const manifest = JSON.parse(
    await fs.readFile(new URL('./fixtures/tool-manifest.fixture.json', import.meta.url), 'utf8')
  );
  assert.equal(
    (await detectPixelSnapper(config, { manifest, projectDir: outputDir, env: {}, pathValue: '' })).available,
    false
  );
...
test('handoff command uses the resolver environment rather than ambient process state', async () => {
  ...
  try {
    const result = await writeSnapperHandoff({
      inputs: ['frame-00.png'],
      outputDir,
      config: { snapper: { executable: 'configured', args: [] } },
      env: { PIXEL_SNAPPER_BIN: 'resolved-pixel-snapper' }
    });
    const handoff = JSON.parse(await fs.readFile(result.handoffPath, 'utf8'));
...
test(
  'guided contract palette is passed to Pixel Snapper and bound into its receipt',
  ...
  async () => {
    ...
    await runPixelSnapper({
      inputs: [input],
      outputDir,
      config: { snapper: { args: ['16'] } },
      identity,
      paletteHex: ['06fd08', '040614', 'f1eff0'],
...
test(
  'standalone snap receipts allow an explicit null manifest binding',
  { skip: process.platform === 'win32' && 'POSIX executable fixture' },
  async () => {
    ...
    await verifySnapReceipt({
      projectDir,
      file: path.join(outputDir, 'snap-receipt.json'),
      expectedRun: run,
...
test(
  'v2 production binds the contracted pixel size and canonicalizes transparent Snapper padding before signing',
  { skip: process.platform === 'win32' && 'POSIX executable fixture' },
  async () => {
    ...
    const executable = path.join(projectDir, 'snapper.mjs');
    await fs.mkdir(path.join(projectDir, '.pixel-sprite-pipeline'), { mode: 0o700 });
    await sharp({ create: { width: 10, height: 11, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } } })
      .png()
      .toFile(input);
    await fs.writeFile(
      executable,
...
test(
  'v2 production preserves an aligned source with the full 16-color Snapper palette',
  { skip: process.platform === 'win32' && 'POSIX executable fixture' },
  async () => {
    ...
    for (let y = 0; y < 4; y += 1)
      for (let x = 0; x < 4; x += 1) scaled.set(logical[Math.floor(y / 2) * 2 + Math.floor(x / 2)], (y * 4 + x) * 4);
    await sharp(scaled, { raw: { width: 4, height: 4, channels: 4 } })
      .png()
      .toFile(input);
    await fs.writeFile(
      executable,
...
test(
  'v2 production preserves canonical palette input without resnapping or scaling it',
  { skip: process.platform === 'win32' && 'POSIX executable fixture' },
  async () => {
    ...
    await sharp(Buffer.from(inputPixels.flat()), { raw: { width: 2, height: 2, channels: 4 } })
      .png()
      .toFile(input);
    await fs.writeFile(
      executable,
...
test(
  'v2 production remaps canonical limited-palette variants without changing occupied pixels',
  { skip: process.platform === 'win32' && 'POSIX executable fixture' },
  async () => {
    ...
    const paletteSha256 = crypto.createHash('sha256').update(JSON.stringify(paletteRgba)).digest('hex');
    await sharp(Buffer.from([transparent, variant, light, transparent].flat()), {
...
test(
  'verified snap receipt is published atomically and blocks changed retry identity before execution',
  { skip: process.platform === 'win32' && 'POSIX executable fixture' },
  async () => {
    ...
    const contract = { sha256: 'e'.repeat(64) };
    const first = await runPixelSnapper({
      inputs: [input],
      outputDir,
      config: { snapper: { args: [] } },
      identity,
...
test(
  'durable guided receipt is reused before spawning and rejects drift',
  { skip: process.platform === 'win32' && 'POSIX executable fixture' },
  async () => {
    ...
    await runPixelSnapper({
      inputs: [input],
      outputDir: durableDir,
      config: { snapper: { args: [] } },
      identity,
      receipt: { projectDir, run, contract }
    });
    const reused = await runPixelSnapper({
      inputs: [input],
      outputDir: stageDir,
      config: { snapper: { args: [] } },
      identity,
      receipt: {
        projectDir,
        run: { ...run, outputDir: stageDir },
        contract,
        durableReceiptFile: path.join(durableDir, 'snap-receipt.json')
      }
    });
