...
async function makePose(
  file,
  { width = 16, height = 16, color = [20, 30, 60, 255], background = [0, 0, 0, 0], xOffset = 0 } = {}
) {
  const data = Buffer.alloc(width * height * 4);
  for (let offset = 0; offset < data.length; offset += 4) data.set(background, offset);
  for (let y = 4; y <= 11; y += 1)
    for (let x = 6 + xOffset; x <= 9 + xOffset; x += 1) data.set(color, (y * width + x) * 4);
...
async function makePassingRun() {
  ...
  const exported = await exportAnimation({
    ...
    outputDir: path.join(dir, 'export'),
    config: DEFAULT_CONFIG,
    columns: 1,
    durations: [80],
    name: 'walk'
...
async function makeApprovedRun(contractDocument = approvedContractDocument()) {
  ...
  const snapReceipt = await writeSnapReceipt({
    projectDir,
    run: { id: 'run-1', outputDir: runDir, manifestSha256: HASH('d') },
    ...
    identity: {
      ...
      version: '1.2.3',
      helpSha256: HASH('f'),
      fixtureRgbaSha256: HASH('0'),
      pinnedReleaseTag: null,
      upstreamCommit: null
    }
  });
  const frames = snapReceipt.document.payload.outputs.map((output, index) => ({
    id: animationContract.document.clips[0].frames[index].id,
    path: output.path,
    sha256: output.sha256
  }));
  const approvals = frames.map((frame) => ({
    frameId: frame.id,
    landmark: { x: 8, y: 12 },
    approved: true,
    approvedBy: 'artist@example.test',
    checkpoints: ['identity', 'motion']
  }));
  ...
  const frameApproval = {
    projectDir,
    file: approval.path,
    snapReceipt: { path: snapReceipt.path, sha256: snapReceipt.sha256 },
    version: 1
  };
  return {
    projectDir,
...
async function writeAnimatedPreview(runtimeFrames, output, durations) {
  const pages = await Promise.all(
    runtimeFrames.map((file) => sharp(file).ensureAlpha().raw().toBuffer({ resolveWithObject: true }))
  );
...
async function replaceOpaqueColor(file, color) {
  ...
  for (let offset = 0; offset < image.data.length; offset += 4)
    if (image.data[offset + 3] !== 0) image.data.set(color, offset);
    ...
    clips: [
      {
        ...
        frames: [
          ...
          },
          {
            ...
            tracks: ['actor', 'satchel'],
            sockets: ['hand'],
            contacts: travelWithoutContact ? [] : ['left-foot'],
            groundTravel: { x: travelWithoutContact ? 2 : 0, y: 0 }
          }
        ]
      }
    ],
...
async function makeV2ApprovedContractExportRun(options = {}) {
  ...
  for (const [frameIndex, definition] of document.clips[0].frames.entries()) {
    for (const [trackIndex, trackId] of definition.tracks.entries()) {
      ...
      await makePose(file, {
        color: trackIndex === 0 && frameIndex === 0 ? [20, 30, 60, 255] : [220, 60, 40, 255],
        xOffset: frameIndex + trackIndex
      });
  ...
  const snapReceipt = await writeSnapReceipt({
    projectDir,
    run: { id: 'run-v2', outputDir: runDir, manifestSha256: HASH('4') },
    contract: animationContract,
    inputs: [originalInput],
    outputs: outputFiles,
    args: ['16'],
  ...
  for (const [frameIndex, definition] of document.clips[0].frames.entries()) {
    ...
    approvals.push({
      ...
      approved: true,
      approvedBy: 'owner',
      checkpoints: ['identity', 'motion', 'landmarks']
    });
  }
  const approval = await writeFrameApproval({
    projectDir,
  ...
  const normalized = await normalizeApi.normalizeContractFrames({
    contract: animationContract,
    frameApproval: approval,
    outputDir: path.join(projectDir, 'normalized-v2')
  });
  const config = {
    ...DEFAULT_CONFIG,
    canonical: { width: 16, height: 16 },
    runtime: { width: 32, height: 32 },
    pivot: { x: 8, y: 14 }
  };
  const exported = await exportContractAnimation({
    normalized,
    contract: animationContract,
    outputDir: path.join(projectDir, 'contract-export-v2'),
    config,
    columns: 2,
...
test('v2 validation proves attachments and rejects foot travel outside contacts or noncyclic restart', async () => {
  ...
  assert.ok(
    detachedReport.failures.some(({ code }) => code === 'SOCKET_ATTACHMENT'),
    JSON.stringify(detachedReport.failures)
  );

  const travel = await makeV2ApprovedContractExportRun({ travelWithoutContact: true });
  const travelReport = await validateRun(travel);
  assert.ok(
    travelReport.failures.some(({ code }) => code === 'GROUND_TRAVEL_CONTACT'),
    JSON.stringify(travelReport.failures)
  );
  ...
  assert.ok(
    onceReport.failures.some(({ code }) => code === 'NONCYCLIC_RESTART'),
    JSON.stringify(onceReport.failures)
  );
});
...
async function shiftForegroundRight(file) {
  ...
  for (let y = 0; y < image.info.height; y += 1)
    for (let x = 0; x < image.info.width - 1; x += 1) {
      image.data.copy(
        shifted,
        (y * image.info.width + x + 1) * 4,
        (y * image.info.width + x) * 4,
...
async function makeSmallExport(frameCount = 2) {
  ...
  const config = {
    ...DEFAULT_CONFIG,
    canonical: { width: 16, height: 16 },
    generation: { width: 16, height: 16 },
    runtime: { width: 16, height: 16 },
    pivot: { x: 8, y: 14 }
  };
...
function validationTarget(failure) {
  return Object.fromEntries(
    ['code', 'frame', 'stage', 'target'].filter((key) => failure[key] !== undefined).map((key) => [key, failure[key]])
  );
}
...
async function revalidationFor(failure, before, after, { removed = true } = {}) {
  return {
    ...
    beforeValidation: {
      passed: false,
      failures: [validationTarget(failure)],
      artifacts: [{ path: before, sha256: await sha256(before) }]
    },
    afterValidation: {
      passed: removed,
      failures: removed ? [] : [validationTarget(failure)],
      artifacts: [{ path: after, sha256: await sha256(after) }]
    }
  };
}
...
test('integer scaling requires equal positive whole-number factors on both axes', () => {
  assert.deepEqual(
    validateIntegerScale({ source: { width: 128, height: 128 }, output: { width: 256, height: 256 } }),
    []
  );
  assert.deepEqual(
    validateIntegerScale({ source: { width: 128, height: 128 }, output: { width: 256, height: 384 } }).map(
      ({ code }) => code
    ),
    ['NON_INTEGER_SCALE']
  );
  assert.deepEqual(
    validateIntegerScale({ source: { width: 0, height: 128 }, output: { width: 256, height: 256 } }).map(
      ({ code }) => code
    ),
    ['NON_INTEGER_SCALE']
  );
...
test('classifies the complete correction taxonomy and gates unknown failures', () => {
  ...
  assert.deepEqual(
    classified.map(({ correction }) => correction),
    [
      'repad',
      'nearest-rescale',
      'nearest-rescale',
...
test('pivot and baseline correction routing follows the owning artifact stage', () => {
  ...
  assert.deepEqual(
    classified.map(({ correction }) => correction),
    ['realign', 'realign', 'reexport-metadata', 'reexport-metadata', 'reexport-preview', 'reexport-sheet']
  );
});
...
test('validates every contract clip against signed approval, contract index, landmarks, sheets, and previews', async () => {
  ...
  assert.deepEqual(report.failures, []);
  assert.deepEqual(
    report.measurements.contractClips.map(({ id, frames, durations, loopMode }) => ({
      id,
      frames,
      durations,
      loopMode
...
test('contract validation objectively rejects forged bindings, order, timing, landmarks, and artifact sets', async (t) => {
  for (const [label, mutate, expectedCode] of [
    [
      'contract hash',
      (index) => {
        index.animationContractSha256 = HASH('9');
      },
      'METADATA_MISMATCH'
    ],
    [
      'approval hash',
      (index) => {
        index.frameApprovalSha256 = HASH('8');
      },
      'SOURCE_HASH_MISMATCH'
    ],
    [
      'palette',
      (index) => {
        index.palette.snapperPaletteHex = ['ffffff'];
      },
      'PALETTE_DRIFT'
    ],
    [
    ...
    [
      'duration',
      (index) => {
        index.clips[0].frames[0].duration = 111;
      },
      'TIMING_MISMATCH'
    ],
    [
    ...
    [
      'landmark',
      (index) => {
        index.measurements[0].canonicalLandmark.x += 1;
      },
      'LANDMARK_DRIFT'
    ],
    [
      'extra clip',
      (index) => {
        index.clips.push(structuredClone(index.clips[0]));
        index.clips[1].id = 'extra';
      },
      'FRAME_COUNT'
    ]
  ])
    await t.test(label, async () => {
      ...
      assert.ok(
        report.failures.some(({ code }) => code === expectedCode),
...
test('contract validation binds actual clip JSON and WebP timing directly to the contract', async () => {
  ...
  metadata.frames.forEach((frame, index) => {
    frame.duration = metadata.durations[index];
  });
  ...
  assert.equal(report.passed, false);
  assert.ok(
    report.failures.some(({ code, field }) => code === 'TIMING_MISMATCH' && /metadata|preview/i.test(field ?? '')),
...
test('contract validation grounds anchor and frozen palette in captured artifact bytes', async () => {
  ...
  await sharp(anchorSwap.anchorReport.path)
    .withMetadata({ comments: [{ keyword: 'Comment', text: 'different bytes' }] })
    .png()
    .toFile(replacementAnchor);
  ...
  assert.ok(
    anchorReport.failures.some(({ code, field }) => code === 'SOURCE_HASH_MISMATCH' && field === 'contractAnchor')
  );

  const recolored = await makeApprovedContractExportRun();
  const clip = recolored.exported.clips.idle;
  const color = [70, 80, 90, 255];
  for (const file of [...recolored.normalized.frames, ...clip.runtimeFrames, clip.sheet])
    await replaceOpaqueColor(file, color);
  await writeAnimatedPreview(clip.runtimeFrames, `${clip.preview}.replacement`, clip.durations);
  await fs.rename(`${clip.preview}.replacement`, clip.preview);
  const metadata = JSON.parse(await fs.readFile(clip.metadata, 'utf8'));
  metadata.sources = await Promise.all(
    recolored.normalized.frames.map(async (file, index) => ({ ...metadata.sources[index], sha256: await sha256(file) }))
  );
  metadata.palette.colors = [
    {
      rgba: [0, 0, 0, 0],
      count: 128 * 128 * recolored.normalized.frames.length - 32 * recolored.normalized.frames.length
    },
  ...
  for (let frameIndex = 0; frameIndex < index.measurements.length; frameIndex += 1)
    index.measurements[frameIndex].normalizedSha256 = await sha256(recolored.normalized.frames[frameIndex]);
...
test('contract validation uses one captured snapshot even when artifact paths are replaced after capture', async () => {
  ...
  const report = await validateRun(
    { ...fixture, config: DEFAULT_CONFIG },
    {
      afterContractCapture: async () => {
        hookCalled = true;
        for (const file of [
          fixture.normalized.frames[0],
          fixture.exported.clips.idle.runtimeFrames[0],
          fixture.exported.clips.idle.sheet,
          fixture.exported.clips.idle.preview,
          fixture.exported.clips.idle.metadata,
          fixture.exported.metadata
        ])
          await fs.writeFile(file, 'replacement after capture');
      }
    }
  );
...
test('contract validation rejects a compatible normalization source outside the signed approval chain', async () => {
  ...
  const landmarks = frames.map((frame) => ({
    frameId: frame.id,
    source: { x: 8, y: 12 },
    target: frame.landmarkSemantic.target
  }));
  fixture.normalized = await normalizeFrames({
    inputs: [unsigned, second],
    landmarks,
    animationContract: fixture.animationContract,
    outputDir: path.join(fixture.projectDir, 'unsigned-normalized'),
    config: DEFAULT_CONFIG
  });
  ...
  fixture.exported = await exportContractAnimation({
    normalized: fixture.normalized,
    contract: fixture.animationContract,
    outputDir: path.join(fixture.projectDir, 'unsigned-contract-export'),
    config: DEFAULT_CONFIG,
    columns: 2,
    frameApprovalSha256: fixture.approval.sha256
  });
  ...
  assert.equal(report.passed, false);
  assert.ok(
    report.failures.some(({ code, field }) => code === 'SOURCE_HASH_MISMATCH' && field === 'approvedSource'),
    JSON.stringify(report.failures)
  );
});
...
test('contract validation derives canonical artifact paths instead of trusting a consistently renamed index', async () => {
  ...
  for (let frameIndex = 0; frameIndex < runtimeNames.length; frameIndex += 1)
    await fs.rename(
      fixture.exported.clips.idle.runtimeFrames[frameIndex],
      path.join(clipDir, runtimeNames[frameIndex])
    );
  const metadata = JSON.parse(await fs.readFile(fixture.exported.clips.idle.metadata, 'utf8'));
  metadata.frames.forEach((frame, frameIndex) => {
    frame.file = runtimeNames[frameIndex];
  });
  ...
  const index = JSON.parse(await fs.readFile(fixture.exported.metadata, 'utf8'));
  index.clips[0].frames.forEach((frame, frameIndex) => {
    frame.file = `idle/${runtimeNames[frameIndex]}`;
  });
  ...
  clip.frames.forEach((frame, frameIndex) => {
    frame.file = clip.runtimeFrames[frameIndex];
  });
  ...
  assert.ok(
    report.failures.some(
      ({ code, field }) =>
        (code === 'METADATA_MISMATCH' && field === 'canonicalArtifacts') ||
        (code === 'FRAME_COUNT' && field === 'artifactSet')
    ),
    JSON.stringify(report.failures)
  );
});
...
test('contract validation treats missing or caller-injected clip artifacts as objective failures', async () => {
  ...
  assert.ok(
    missingCallerReport.failures.some(({ code, field }) => code === 'METADATA_MISMATCH' && field === 'callerArtifacts')
  );
...
test('derives anchor palette and normalized scale from actual artifacts instead of caller claims', async () => {
  ...
  const report = await validateRun({ ...fixture, config: DEFAULT_CONFIG });
  assert.equal(
    report.failures.some(({ code }) => code === 'PALETTE_DRIFT'),
    false
  );
  assert.equal(
    report.failures.some(({ code }) => code === 'GLOBAL_SCALE_DRIFT'),
    false
  );
...
test('derives scale with largest, minimum-size, and reject-multiple normalization policies', async () => {
  ...
  for (const [name, foreground] of [
    ...
    const config = {
      ...DEFAULT_CONFIG,
      background: { mode: 'color', color: { r: 0, g: 255, b: 0 }, tolerance: 0 },
      foreground
    };
    const normalized = await normalizeFrames({
      inputs: [anchor],
      outputDir: path.join(dir, `normalized-${name}`),
      config
    });
    const exported = await exportAnimation({
      frames: normalized.frames,
      outputDir: path.join(dir, `export-${name}`),
      config,
      columns: 1,
      durations: [80],
      name
    ...
    assert.equal(
      report.failures.some(({ code }) => code === 'GLOBAL_SCALE_DRIFT'),
      false,
      `${name}: ${JSON.stringify(report.failures)}`
    );
    assert.deepEqual(report.measurements.normalizedScales, [1]);
  }
});
...
test('detects forged canonical pivot, config snapshot, and preview pixels', async () => {
  ...
  await sharp(wrong, { raw: { width: 256, height: 256, channels: 4 } })
    .webp({ lossless: true })
    .toFile(`${fixture.exported.preview}.bad`);
...
test('detects tampered pixels, hashes, timing, canvas, background, clipping, and provenance', async () => {
  ...
  for (let y = 0; y < image.info.height; y += 1)
    for (let x = 0; x < 4; x += 1) image.data.set([0, 255, 0, 255], (y * image.info.width + x) * 4);
  ...
  for (const code of [
    ...
    assert.ok(codes.has(code), `missing ${code}: ${JSON.stringify(report.failures)}`);
  assert.equal(report.passed, false);
});
...
test('semantic failures require explicit evidence and are otherwise human review warnings', async () => {
  ...
  assert.equal(
    withoutEvidence.failures.some(({ code }) => code === 'IDENTITY_DRIFT'),
    false
  );
  const withEvidence = await validateRun({
    ...fixture,
    config: DEFAULT_CONFIG,
    semanticEvidence: [{ code: 'IDENTITY_DRIFT', frame: 0, failed: true, evidence: { reviewer: 'artist' } }]
  });
...
test('uncontracted recorded landmark claims cannot become authenticated drift evidence', async () => {
  ...
  const report = await validateRun({ ...fixture, config: DEFAULT_CONFIG });
  assert.equal(
    report.failures.some(({ code }) => code === 'LANDMARK_DRIFT'),
    false
  );
});
...
test('loop root transition review is suppressed only by an internally verified frame approval', async () => {
  ...
  await assert.rejects(
    validateRun({ ...fixture, config: DEFAULT_CONFIG, frameApproval: undefined }),
    /signed frame approval is required/i
  );

  const forgedEvidence = {
    code: 'LOOP_ROOT_TRANSITION',
    clipId: 'idle',
    transition: fixture.animationContract.document.clips[0].loopTransition,
    approved: true,
    evidence: { approvedBy: 'artist@example.test', checkpoint: 'motion' }
  };
  await assert.rejects(
    validateRun({ ...fixture, config: DEFAULT_CONFIG, frameApproval: undefined, semanticEvidence: [forgedEvidence] }),
    /signed frame approval is required/i
  );

  const approved = await validateRun({ ...fixture, config: DEFAULT_CONFIG });
  assert.equal(
    approved.warnings.some(({ check }) => check === 'LOOP_ROOT_TRANSITION'),
    false
  );
});
...
test('validation snapshots the selected approval and contract before authenticated file reads', async () => {
  ...
  assert.equal(
    report.warnings.some(({ check }) => check === 'LOOP_ROOT_TRANSITION'),
    false
  );
});
...
test('frame approval selection fails closed for tampering, forgery, unauthorized approvers, wrong contracts, and wrong versions', async () => {
  ...
  await assert.rejects(
    validateRun({
      ...wrongVersion,
      config: DEFAULT_CONFIG,
      frameApproval: { ...wrongVersion.frameApproval, version: 2 }
    }),
    /version/i
  );
  ...
  await fs.writeFile(
    forgedFile,
    JSON.stringify({ version: 1, payload: forged.approval.document.payload, signature: HASH('f') })
  );
  await assert.rejects(
    validateRun({
      ...forged,
      config: DEFAULT_CONFIG,
      frameApproval: { ...forged.frameApproval, file: forgedFile, version: 2 }
    }),
    /signature|approval/i
  );
  ...
  unauthorizedPayload.frames.forEach((frame) => {
    frame.approvedBy = 'intruder@example.test';
  });
  const unauthorizedFile = path.join(unauthorized.runDir, 'frame-approval-02.json');
  await writeSignedState({
    projectDir: unauthorized.projectDir,
    file: unauthorizedFile,
  ...
  await assert.rejects(
    validateRun({
      ...unauthorized,
      config: DEFAULT_CONFIG,
      frameApproval: { ...unauthorized.frameApproval, file: unauthorizedFile, version: 2 }
    }),
    /authorized/i
  );
  ...
  await assert.rejects(
    validateRun({ ...wrongContract, config: DEFAULT_CONFIG, animationContract: otherContract }),
    /contract.*binding|contract/i
  );
  ...
  await assert.rejects(
    validateRun({ ...replacedSource, config: DEFAULT_CONFIG }),
    /output hash mismatch|source hash mismatch/i
  );
});
...
test('contract validation requires every ordered landmark measurement to match signed roots', async () => {
  ...
  const missingReport = await validateRun({ ...missing, config: DEFAULT_CONFIG });
  assert.ok(
    missingReport.failures.some(({ code, stage }) => code === 'METADATA_MISMATCH' && stage === 'normalization-landmark')
  );
  ...
  assert.ok(
    invalidCoordinatesReport.failures.some(
      ({ code, stage, field }) =>
        code === 'METADATA_MISMATCH' && stage === 'normalization-landmark' && field === 'ordered-frame'
    )
  );
  ...
  assert.ok(
    forgedReport.failures.filter(
      ({ code, stage }) => code === 'METADATA_MISMATCH' && stage === 'normalization-landmark'
    ).length >= 2
  );
  ...
  assert.ok(
    escapedReport.failures.some(
      ({ code, stage, field }) =>
        code === 'METADATA_MISMATCH' && stage === 'normalization-landmark' && field === 'approved-source'
    )
  );

  const outside = await makeApprovedRun();
  const frames = outside.snapReceipt.document.payload.outputs.map((output, index) => ({
    id: outside.animationContract.document.clips[0].frames[index].id,
    path: output.path,
    sha256: output.sha256
  }));
  const approvals = frames.map((frame) => ({
  ...
  const outsideApproval = await writeFrameApproval({
    projectDir: outside.projectDir,
    runDir: outside.runDir,
    contract: outside.animationContract,
    snapReceipt: outside.snapReceipt,
    frames,
    approvals,
    version: 2
  });
  const outsideReport = await validateRun({
    ...outside,
    config: DEFAULT_CONFIG,
    frameApproval: { ...outside.frameApproval, file: outsideApproval.path, version: 2 }
  });
  assert.ok(
    outsideReport.failures.some(
      ({ code, stage, field }) =>
        code === 'METADATA_MISMATCH' && stage === 'normalization-landmark' && field === 'sourceLandmarkContainment'
    )
  );
});
...
test('once and hold-last clips cannot validate forged root metadata without a signed approval', async () => {
  for (const loopMode of ['once', 'hold-last']) {
    ...
    fixture.normalized.measurements[0].canonicalLandmark = { x: 64, y: 112 };
    fixture.normalized.measurements[0].landmarkDrift = { x: 0, y: 0 };
    fixture.exported = await exportAnimation({
      frames: fixture.normalized.frames,
      outputDir: path.join(fixture.projectDir, `forged-${loopMode}`),
      config: DEFAULT_CONFIG,
      columns: 2,
      durations: [100, 120],
      name: `idle-${loopMode}`
...
test('validates sheet geometry and recorded palette against actual exported sources', async () => {
  ...
  await fs.writeFile(fixture.exported.metadata, JSON.stringify(metadata));
  await sharp(fixture.exported.sheet)
    .extract({ left: 0, top: 0, width: 255, height: 256 })
    .png()
    .toFile(`${fixture.exported.sheet}.bad`);
...
test('correction preflight is atomic when an operation is missing or run state is invalid', async () => {
  ...
  await assert.rejects(
    applyDeterministicCorrections({
      failures: [
        { code: 'CANVAS_SIZE', correction: 'repad' },
        { code: 'PIVOT_DRIFT', correction: 'realign' }
      ],
      run,
      config: DEFAULT_CONFIG,
      operations: { repad: async () => ({ validationPassed: true }) }
    }),
    /missing deterministic correction operation: realign/
  );
  assert.deepEqual(await fs.readdir(runDir), []);
  await assert.rejects(
    applyDeterministicCorrections({
      failures: [],
      run: { runDir, corrections: null },
      config: DEFAULT_CONFIG,
      operations: {}
    }),
    /run corrections must be an array/
  );
  assert.deepEqual(await fs.readdir(runDir), []);
});
...
test('deduplicates equivalent deterministic work while retaining traceability', async () => {
  ...
  let calls = 0;
  const failures = [
    { code: 'CANVAS_SIZE', target: 'anchor', expected: [128, 128], actual: [16, 16], correction: 'repad', before },
    { code: 'FRAME_BLEED', target: 'anchor', correction: 'repad', before: path.basename(before) }
  ];
  const result = await applyDeterministicCorrections({
    ...
    run: {
      ...
      inputs: [before],
      generativeAttempts: {},
      expected: { sheet: { runtimeFrames: [trustedFrame], columns: 1, frameSize: { width: 128, height: 128 } } }
    },
    config: DEFAULT_CONFIG,
    operations: {
      repad: async ({ failures: shared, outputDir }) => {
        calls += 1;
        const output = path.join(outputDir, 'fixed.png');
        const sidecar = path.join(outputDir, 'measurements.json');
        await makePose(output, { width: 128, height: 128 });
        await fs.writeFile(sidecar, '{}');
        return {
          output,
          details: { artifact: output, nested: { sidecar } },
          validationPassed: true,
          improved: true,
          revalidations: await Promise.all(shared.map((failure) => revalidationFor(failure, before, output)))
        };
      }
    }
  });
  ...
  assert.deepEqual(correctionManifest.run, {
    id: 'bound-run',
    manifestSha256: crypto.createHash('sha256').update(runManifest).digest('hex')
  });
});
...
test('shared execution leaves a deduplicated failure unapproved when its revalidation still fails', async () => {
  ...
  const failures = [
    { code: 'CANVAS_SIZE', target: 'anchor', expected: [128, 128], actual: [16, 16], correction: 'repad', before },
    { code: 'FRAME_BLEED', target: 'anchor', correction: 'repad', before }
  ];
  const result = await applyDeterministicCorrections({
    failures,
    ...
    operations: {
      repad: async ({ failures: shared, outputDir }) => {
        ...
        return {
          ...
          improved: true,
          revalidations: [
            await revalidationFor(shared[0], before, output),
            await revalidationFor(shared[1], before, output, { removed: false })
          ]
        };
      }
    }
...
test('does not approve a removed target when the after validation report still fails', async () => {
  ...
  const result = await applyDeterministicCorrections({
    ...
    config: DEFAULT_CONFIG,
    operations: {
      repad: async ({ outputDir }) => {
        const output = path.join(outputDir, 'fixed.png');
        await makePose(output, { width: 128, height: 128 });
        const revalidation = await revalidationFor(failure, before, output);
        revalidation.afterValidation.passed = false;
        revalidation.afterValidation.failures = [
          { code: 'METADATA_MISMATCH', correction: 'reexport-metadata', frame: 0 }
        ];
        return { output, validationPassed: true, improved: true, revalidations: [revalidation] };
      }
    }
  });
  ...
  const result = await applyDeterministicCorrections({
    ...
    operations: {
      repad: async ({ outputDir }) => {
        const output = path.join(outputDir, 'fixed.png');
        await makePose(output, { width: 128, height: 128 });
        const revalidation = await revalidationFor(objective, before, output);
        revalidation.afterValidation.passed = false;
        revalidation.afterValidation.failures = [artistic];
        return { output, validationPassed: true, improved: true, revalidations: [revalidation] };
      }
    }
  });
...
test('does not approve byte-identical pivot output despite a claimed passing report', async () => {
  ...
  const result = await applyDeterministicCorrections({
    ...
    config: DEFAULT_CONFIG,
    operations: {
      realign: async ({ outputDir }) => {
        const output = path.join(outputDir, 'same.png');
        await fs.copyFile(before, output);
        const revalidation = await revalidationFor(failure, before, output);
        revalidation.afterValidation.measurements = { pivot: { x: 64, y: 112 } };
        return { output, validationPassed: true, improved: true, revalidations: [revalidation] };
      }
    }
  });
...
test('automatic correction classes require their failure-specific after measurements', async () => {
  ...
  for (const item of cases) {
    ...
    const result = await applyDeterministicCorrections({
      ...
      config: DEFAULT_CONFIG,
      operations: {
        [item.correction]: async ({ outputDir }) => {
          const output = path.join(outputDir, 'changed.png');
          await makePose(output, { color: [40, 50, 70, 255] });
          return {
            output,
            validationPassed: true,
            improved: true,
            revalidations: [await revalidationFor(failure, before, output)]
          };
        }
      }
    });
...
test('rejects malformed metadata and unrelated preview or sheet artifacts', async () => {
  ...
  const cases = [
    {
      ...
      correction: 'reexport-metadata',
      extension: 'json',
      contents: 'not json',
      expectedArtifact: {
        metadata: {
          frameSize: { width: 16, height: 16 },
          canonicalPivot: { x: 8, y: 14 },
          pivot: { x: 8, y: 14 },
          durations: [80, 120],
          sources: [],
          frames: [],
  ...
  for (const item of cases) {
    ...
    const result = await applyDeterministicCorrections({
      failures: [failure],
      run: { runDir, corrections: [], inputs: [before], generativeAttempts: {}, expected: item.expectedArtifact },
      config: DEFAULT_CONFIG,
      operations: {
        [item.correction]: async ({ outputDir }) => {
          const output = path.join(outputDir, `candidate.${item.extension}`);
          if (item.contents) await fs.writeFile(output, item.contents);
          else if (item.extension === 'webp') await sharp(runtimeFrames[0]).webp({ lossless: true }).toFile(output);
          else await makePose(output, { color: [80, 90, 100, 255] });
          return {
            output,
            validationPassed: true,
            improved: true,
...
test('approves metadata, preview, and sheet corrections only when artifacts match trusted expectations', async () => {
  ...
  const exported = await exportAnimation({
    frames: sourceFrames,
    outputDir: path.join(fixtureDir, 'export'),
    config: smallConfig,
    columns: 2,
    durations: [80, 120],
    name: 'trusted'
  ...
  const cases = [
    ...
    },
    {
      code: 'PREVIEW_MISMATCH',
      correction: 'reexport-preview',
      source: exported.preview,
      extension: 'webp',
      expected: { preview: { runtimeFrames: exported.runtimeFrames, durations: [80, 120] } }
    },
    {
      code: 'FRAME_BLEED',
      correction: 'repad',
      source: exported.sheet,
      extension: 'png',
      expected: { sheet: { runtimeFrames: exported.runtimeFrames, columns: 2, frameSize: { width: 16, height: 16 } } }
    }
  ];
  for (const item of cases) {
    const runDir = await temporaryDirectory('sprite-correct-');
    ...
    const result = await applyDeterministicCorrections({
      failures: [failure],
      run: { runDir, corrections: [], inputs: [before], generativeAttempts: {}, expected: item.expected },
      config: smallConfig,
      operations: {
        [item.correction]: async ({ outputDir }) => {
          const output = path.join(outputDir, `fixed.${item.extension}`);
          await fs.copyFile(item.source, output);
          return {
            output,
            validationPassed: true,
            improved: true,
...
test('metadata correction requires the complete trusted document including future delivery fields', async () => {
  ...
  const result = await applyDeterministicCorrections({
    ...
    run: {
      ...
      inputs: [before],
      generativeAttempts: {},
      expected: { metadata: fixture.metadata }
    },
    config: fixture.config,
    operations: {
      'reexport-metadata': async ({ outputDir }) => {
        const output = path.join(outputDir, 'candidate.json');
        await fs.writeFile(output, JSON.stringify(candidate));
        return {
          output,
          validationPassed: true,
          improved: true,
    ...
    const result = await applyDeterministicCorrections({
      ...
      run: {
        ...
        expected: {
          ...
          preview: { runtimeFrames: fixture.exported.runtimeFrames, durations: fixture.durations }
        }
      },
      config: fixture.config,
      operations: {
...
test('single-frame preview verifies pixels while timing comes from trusted metadata', async () => {
  ...
  for (const [metadataDuration, wrongPixels, approved] of [
    [80, false, true],
    [99, false, false],
    [80, true, false]
  ]) {
    ...
    const result = await applyDeterministicCorrections({
      ...
      run: {
        ...
        generativeAttempts: {},
        expected: {
          metadata: { durations: [metadataDuration] },
          preview: { runtimeFrames: fixture.exported.runtimeFrames, durations: fixture.durations }
        }
      },
      config: fixture.config,
      operations: {
        'reexport-preview': async ({ outputDir }) => {
          const output = path.join(outputDir, 'fixed.webp');
          if (wrongPixels)
            await sharp(fixture.frames[0]).negate({ alpha: false }).webp({ lossless: true }).toFile(output);
          else await fs.copyFile(fixture.exported.preview, output);
          return {
            output,
            validationPassed: true,
            improved: true,
    ...
    assert.equal(
      result.actions[0].approved,
      approved,
      `trusted duration ${metadataDuration}, wrongPixels ${wrongPixels}`
    );
  }
});
...
test('rejects non-image correction output even when supplied reports claim success', async () => {
  ...
  await assert.rejects(
    applyDeterministicCorrections({
      ...
      config: DEFAULT_CONFIG,
      operations: {
        repad: async ({ failure, outputDir }) => {
          const output = path.join(outputDir, 'banana.png');
          await fs.writeFile(output, 'banana');
          return {
            output,
            validationPassed: true,
            improved: true,
            revalidations: [
              {
                ...
                afterValidation: { failures: [], artifacts: [{ path: output, sha256: await sha256(output) }] }
              }
            ]
          };
        }
    ...
    applyDeterministicCorrections({
      ...
      operations: {
        repad: async ({ outputDir }) => {
          ...
          return {
            output,
            details: { missing: path.join(outputDir, 'missing.json') },
            validationPassed: true,
            improved: true,
            revalidations: [await revalidationFor(failure, before, output)]
          };
...
test('does not approve true validation without explicit improvement and tied evidence', async () => {
  ...
  const result = await applyDeterministicCorrections({
    ...
    operations: {
      repad: async ({ outputDir }) => {
        const output = path.join(outputDir, 'after.png');
        await makePose(output);
        return { output, validationPassed: true };
      }
    }
  });
  assert.equal(result.actions[0].status, 'unapproved');
    ...
    operations: {
      repad: async ({ outputDir }) => {
        ...
        return {
          ...
          improved: true,
          evidence: {
            failureCode: 'PIVOT_DRIFT',
            target: 'anchor',
            before: { reference: before, measurement: { width: 131 } },
            after: { reference: before, measurement: { width: 128 } }
          }
        };
      }
    }
    ...
    operations: {
      repad: async ({ outputDir }) => {
        ...
        return {
          ...
          improved: true,
          evidence: {
            failureCode: 'CANVAS_SIZE',
            target: 'anchor',
            before: { reference: before, measurement: { width: 131 } },
            after: { reference: output, measurement: { width: 131 } }
          }
        };
      }
    }
...
test('non-improving corrections remain unapproved and never replace an artifact', async () => {
  ...
  const result = await applyDeterministicCorrections({
    failures: [{ code: 'CANVAS_SIZE', frame: 0, correction: 'repad', before: input }],
    run: { runDir, corrections: [], inputs: [input], generativeAttempts: {} },
    config: DEFAULT_CONFIG,
    operations: {
      repad: async ({ outputDir }) => {
        const output = path.join(outputDir, 'candidate.png');
        await makePose(output);
        return { output, validationPassed: false, improved: false, after: { width: 131 } };
      }
    }
  });
  assert.equal(result.actions[0].status, 'unapproved');
...
test('rejects escaped correction outputs and removes the staged batch', async () => {
  ...
  await assert.rejects(
    applyDeterministicCorrections({
      ...
      config: DEFAULT_CONFIG,
      operations: { repad: async () => ({ output: path.join(runDir, 'escaped.png'), validationPassed: true }) }
    }),
    /outside correction directory/
  );
  assert.deepEqual(
    (await fs.readdir(runDir)).filter((name) => name.startsWith('correction-') || name.startsWith('.correction-')),
    []
  );
});
...
test('rejects a symlinked output that resolves outside the correction directory', async (t) => {
  ...
  await assert.rejects(
    applyDeterministicCorrections({
      ...
      operations: {
        repad: async ({ outputDir }) => {
          const output = path.join(outputDir, 'linked.png');
          try {
            await fs.symlink(outside, output);
          } catch (error) {
            if (error.code === 'EPERM') t.skip('symlinks unavailable');
            throw error;
          }
          return { output, validationPassed: true };
        }
  ...
  await assert.rejects(
    applyDeterministicCorrections({
      ...
      run: {
        runDir,
        corrections: [],
        manifest: { inputs: { sources: [{ file: 'input.png' }] }, unrelated: { cache: 'not-an-input.png' } },
        generativeAttempts: {}
      },
      config: DEFAULT_CONFIG,
      operations: {
        repad: async ({ outputDir }) => {
          const output = path.join(outputDir, 'alias.png');
          await fs.link(input, output);
          return { output, validationPassed: true, improved: true };
        }
      }
    }),
...
test('generative retry accounting is capped per affected frame and never calls operations', async () => {
  ...
  const result = await applyDeterministicCorrections({
    failures: [
      { code: 'IDENTITY_DRIFT', frame: 2, correction: 'stop-for-regeneration' },
      { code: 'CLIPPED_FOREGROUND', frame: 4, correction: 'stop-for-regeneration' },
      { code: 'LOOP_SEAM', correction: 'timing-or-transition-review' },
      { code: 'NEW_FAILURE', correction: 'stop-for-review' }
    ],
    run: { runDir, corrections: [], generativeAttempts: { 2: 1, 4: 2 } },
    config: DEFAULT_CONFIG,
    operations: {}
  });
  assert.deepEqual(result.generativeAttemptsRemaining, { 2: 1, 4: 0 });
  assert.deepEqual(
    result.actions.map(({ requires }) => requires),
    ['generative-retry', 'generative-retry-exhausted', 'user-review', 'user-review']
  );
});
...
test('correction cleanup requests documented Windows lock retries and propagates persistent failure', async () => {
  ...
  let observed;
  await module.removeCorrectionTree('staging', async (directory, options) => {
    observed = { directory, options };
  });
  ...
  await assert.rejects(
    module.removeCorrectionTree('staging', async () => {
      throw locked;
    }),
    (error) => error === locked
  );
