...
test('inspection reports mixed blocks, alpha, clipping, and smoothing suspicion without certainty', async () => {
  ...
  for (let y = 0; y < 4; y += 1)
    for (let x = 0; x < 3; x += 1) data.set([20 + x, 30, 60, x === 1 ? 128 : 255], (y * 8 + x) * 4);
  await sharp(data, { raw: { width: 8, height: 8, channels: 4 } })
    .png()
    .toFile(file);
...
test('inspection conservatively flags two statistically repeated horizontal block modes', async () => {
  ...
  for (let y = 1; y < 11; y += 1) {
    for (let x = 1; x < 17; x += 4)
      for (let dx = 0; dx < 2; dx += 1) data.set([20, 30, 60, 255], (y * width + x + dx) * 4);
    for (let x = 21; x < 39; x += 6)
      for (let dx = 0; dx < 3; dx += 1) data.set([20, 30, 60, 255], (y * width + x + dx) * 4);
  }
  ...
  assert.ok(
    report.pixelGrid.evidence.repeatedIncompatibleModes.some(
      ({ axis, modes }) =>
        axis === 'horizontal' && modes.some(({ length }) => length === 2) && modes.some(({ length }) => length === 3)
    )
  );
