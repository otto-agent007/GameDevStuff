...
import {
  ...
  isPathContained,
  sameCanonicalPath
} from './lib/path-security.mjs';

const EXIT = Object.freeze({ success: 0, error: 1, handoff: 2, objectiveFailure: 3, review: 4 });
const REVIEW_CORRECTIONS = new Set([
  'palette-remap-review',
  'stop-for-regeneration',
  'stop-for-review',
  'timing-or-transition-review'
]);
...
function tolerance(value) {
  ...
  if (!Number.isInteger(parsed) || parsed < 0 || parsed > 255)
    throw new InvalidArgumentError('tolerance must be an integer from 0 to 255');
  return parsed;
}
...
async function ensureReceiptState(projectDir) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink())
    throw new Error('snap receipt state directory must be a real directory');
  ...
  if (result.passed) {
    if (
      result.warnings?.some(
        (warning) => warning.code === 'HUMAN_REVIEW_REQUIRED' || warning.requiresUserReview === true
      )
    )
      process.exitCode = EXIT.review;
    return;
  }
  const objective = result.failures.some(
    (failure) => failure.requiresUserReview !== true && !REVIEW_CORRECTIONS.has(failure.correction)
  );
...
function validateRequest(document) {
  ...
  for (const key of Object.keys(document))
    if (!allowed.has(key)) throw new Error(`unknown validation request field: ${key}`);
  ...
  for (const key of ['anchorReport', 'normalized', 'exported']) {
    if (!document[key] || typeof document[key] !== 'object' || Array.isArray(document[key]))
      throw new Error(`validation request ${key} must be an object`);
  }
  if (document.semanticEvidence !== undefined && !Array.isArray(document.semanticEvidence))
    throw new Error('validation request semanticEvidence must be an array');
  return document;
}

function validateRepairRequest(document) {
  if (!document || typeof document !== 'object' || Array.isArray(document))
    throw new Error('correction request must be an object');
  ...
  for (const key of Object.keys(document))
    if (!allowed.has(key)) throw new Error(`unknown correction request field: ${key}`);
  if (document.version !== 1) throw new Error('correction request version must be 1');
  safeRunId(document.runId);
  if (!/^[a-f0-9]{64}$/.test(document.contractSha256 ?? ''))
    throw new Error('correction request contractSha256 is required');
  if (!/^[a-f0-9]{64}$/.test(document.receiptSha256 ?? '') || !/^[a-f0-9]{64}$/.test(document.receiptSignature ?? ''))
    throw new Error('correction request signed receipt is required');
  if (
    !document.declaredFailure ||
    typeof document.declaredFailure !== 'object' ||
    Array.isArray(document.declaredFailure) ||
    typeof document.declaredFailure.code !== 'string'
  )
    throw new Error('correction request declaredFailure is required');
  for (const key of Object.keys(document.declaredFailure))
    if (!['code', 'stage', 'frame', 'target'].includes(key)) throw new Error(`unknown declared failure field: ${key}`);
  if (document.reportVersion !== undefined && (!Number.isInteger(document.reportVersion) || document.reportVersion < 2))
    throw new Error('correction reportVersion must be an integer of at least 2');
  return document;
}

function validateApprovalRequest(document) {
  if (!document || typeof document !== 'object' || Array.isArray(document))
    throw new Error('frame approval request must be an object');
  const allowed = new Set(['version', 'frames', 'approvals']);
  for (const key of Object.keys(document))
    if (!allowed.has(key)) throw new Error(`unknown frame approval request field: ${key}`);
  if (document.version !== 1) throw new Error('frame approval request version must be 1');
  if (!Array.isArray(document.frames) || !Array.isArray(document.approvals))
    throw new Error('frame approval request requires explicit frames and approvals');
  return document;
}
...
function sameFailure(left, right) {
  return (
    ...
    ['stage', 'frame', 'target'].every((key) => right[key] === undefined || left[key] === right[key])
  );
}

async function portablePath(runDir, file) {
  try {
    return await canonicalRelativePath(runDir, file);
  } catch {
    throw new Error('run artifact escaped the versioned run directory');
  }
}
...
function portableValue(value, runDir) {
  ...
  if (value && typeof value === 'object')
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, portableValue(item, runDir)]));
  return value;
}
...
async function writeJsonIdempotent(file, value, label) {
  ...
  try {
    ...
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink > 1)
      throw new Error(`${label} existing state is not a regular authenticated file`);
    if ((await fs.readFile(file, 'utf8')) !== contents)
      throw new Error(`${label} existing state does not match the authenticated retry`);
    return;
  } catch (error) {
    if (error.code !== 'ENOENT') throw error;
  }
...
function parseTransitionToken(value) {
  ...
  const keys = Object.keys(parsed ?? {})
    ...
    .join(',');
  if (
    keys !== 'handoffSha256,mac,runId,state,version' ||
    parsed.version !== 1 ||
    !HANDOFF_FILES[parsed.state] ||
    !/^[a-f0-9]{64}$/.test(parsed.handoffSha256 ?? '') ||
...
async function safeRunArtifact(runDir, relative, expectedHash, label) {
  ...
  if (normalized !== relative.replaceAll('\\', '/') || normalized === '..' || normalized.startsWith('../'))
    throw new Error(`${label} escaped the run directory`);
  ...
  if (expectedHash && actualHash !== expectedHash)
    throw new Error(`${label} artifact hash does not match canonical handoff state`);
...
function nextInvocation(projectDir, runId, flag) {
  return {
    ...
    argv: [
      ...
      '<TOKEN>',
      flag,
      flag === '--frame' ? '<GENERATED_FRAME>' : '<SNAPPED_FRAME>',
      '--project-dir',
      projectDir
    ]
  };
...
function manifestAnimationContract(manifest) {
  ...
  if (
    ...
    Object.keys(selected).sort().join(',') !== 'document,sha256'
  )
    throw new Error('run manifest animation contract binding is invalid');
  validateAnimationContract(selected.document);
  if (stableHash(selected.document) !== selected.sha256)
    throw new Error('run manifest animation contract hash is invalid');
  return selected;
}
...
async function stageBatch({
  ...
  targetDir,
  label,
  outputNames = inputs.map((_, index) => `frame-${String(index).padStart(2, '0')}.png`)
}) {
  if (
    outputNames.length !== inputs.length ||
    new Set(outputNames).size !== outputNames.length ||
    outputNames.some((name) => path.basename(name) !== name || name === '.' || name === '..')
  ) {
    throw new Error(`${label} output contract is invalid`);
  }
  const inspected = [];
  for (let index = 0; index < inputs.length; index += 1)
    inspected.push(await inspectExplicitInput(inputs[index], `${label} ${index}`));
  try {
    const targetStat = await fs.lstat(targetDir);
    if (!targetStat.isDirectory() || targetStat.isSymbolicLink())
      throw new Error(`${label} existing destination is not an authenticated batch directory`);
    const actualNames = (await fs.readdir(targetDir)).sort();
    const expectedNames = [...outputNames].sort();
    if (JSON.stringify(actualNames) !== JSON.stringify(expectedNames))
      throw new Error(`${label} existing destination has an unexpected filename set`);
    ...
    for (let index = 0; index < outputNames.length; index += 1) {
      ...
      if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink > 1)
        throw new Error(`${label} existing file is not a regular authenticated batch member`);
      const physical = await fs.realpath(output);
      const containment = path.relative(root, physical);
      if (containment === '..' || containment.startsWith(`..${path.sep}`) || path.isAbsolute(containment))
        throw new Error(`${label} existing file escaped the authenticated batch`);
      if ((await sha256(output)) !== inspected[index].sha256)
        throw new Error(`${label} existing file hash does not match the supplied authenticated batch`);
      outputs.push(output);
    }
    return outputs;
  } catch (error) {
  ...
  try {
    ...
    for (let index = 0; index < inspected.length; index += 1) {
      ...
      if ((await sha256(output)) !== inspected[index].sha256)
        throw new Error(`${label} ${index} changed during staging`);
      outputs.push(output);
    }
...
async function flatDirectoryManifest(directory, label) {
  ...
  async function visit(current, prefix = '') {
    for (const name of (await fs.readdir(current)).sort()) {
      ...
      const containment = path.relative(root, physical);
      if (containment === '..' || containment.startsWith(`..${path.sep}`) || path.isAbsolute(containment))
        throw new Error(`${label} artifact escaped its directory`);
...
async function productionInputs({ contractFile, contract, projectDir }) {
  ...
  const keys = Object.keys(document ?? {})
    ...
    .join(',');
  if (
    keys !== 'anchor,frames,selectionApprovalSha256,version' ||
    document.version !== 1 ||
    document.selectionApprovalSha256 !== contract.document.selectionApprovalSha256 ||
    !Array.isArray(document.frames)
  )
    throw new Error('production input manifest approval binding mismatch');
  if (
    !document.anchor ||
    Object.keys(document.anchor).sort().join(',') !== 'path,sha256' ||
    document.anchor.sha256 !== contract.document.character.anchorSha256
  )
    throw new Error('production input manifest character anchor binding mismatch');
  const anchor = await safeRunArtifact(projectDir, document.anchor.path, document.anchor.sha256, 'production anchor');
  const definitions = contract.document.clips.flatMap((clip) =>
    clip.frames.flatMap((frame) => frame.tracks.map((trackId) => ({ frameId: frame.id, trackId })))
  );
  if (document.frames.length !== definitions.length)
    throw new Error('production input manifest membership does not match the v2 contract');
  const inputs = [];
  for (const [index, record] of document.frames.entries()) {
    if (
      !record ||
      Object.keys(record).sort().join(',') !== 'frameId,path,sha256,trackId' ||
      record.frameId !== definitions[index].frameId ||
      record.trackId !== definitions[index].trackId ||
      !/^[a-f0-9]{64}$/.test(record.sha256 ?? '')
    )
      throw new Error('production input manifest ordered membership is invalid');
...
function approvalVersion(file) {
  ...
  if (!match || Number(match[1]) < 1)
    throw new Error('selected frame approval must use its immutable numbered filename');
...
async function productionArtifacts(root, areas) {
  ...
  for (const area of areas) {
    for (const { name, sha256: artifactSha256 } of await flatDirectoryManifest(
      path.join(root, area),
      `pixel production ${area}`
    )) {
...
async function publishDirectory(stage, target, label) {
  ...
  const [staged, existing] = await Promise.all([
    flatDirectoryManifest(stage, `${label} staging`),
    flatDirectoryManifest(target, `${label} existing`)
  ]);
  if (JSON.stringify(staged) !== JSON.stringify(existing))
    throw new Error(`${label} existing artifacts do not match the authenticated retry`);
...
function rebasePaths(value, from, to) {
  if (typeof value === 'string' && path.isAbsolute(value)) {
    ...
    return relative !== '..' && !relative.startsWith(`..${path.sep}`) && !path.isAbsolute(relative)
      ? path.join(to, relative)
      : value;
  }
  if (Array.isArray(value)) return value.map((item) => rebasePaths(item, from, to));
  if (value && typeof value === 'object')
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, rebasePaths(item, from, to)]));
  return value;
}
...
async function recordRunResultIdempotent({ context, report }) {
  ...
  try {
    ...
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink > 1)
      throw new Error('existing run report is not a regular authenticated file');
    if ((await fs.readFile(reportPath, 'utf8')) !== contents)
      throw new Error('existing run report does not match the authenticated retry');
...
async function initialRun(options) {
  ...
  const animationContract = options.contract
    ? await loadAnimationContract(path.resolve(options.contract), { projectId: config.integration.projectId })
    : null;
  if (animationContract && animationContract.document.anchor.sha256 !== sourceInspection.sha256)
    throw new Error('animation contract anchor hash does not match the approved guided-run input');
  ...
  const handoff = {
    ...
    references: {
      anchor: {
        path: await portablePath(run.runDir, prepared.generationPlate),
        sha256: prepared.hashes.generationPlate
      },
      matrix: {
        path: await portablePath(run.runDir, prepared.pixelMatrix),
        sha256: prepared.hashes.pixelMatrix,
        blockSize: generationScale,
        usage: 'constraint-only-not-composited'
      },
    ...
    requiredOutputs: {
      ...
      count: animationContract
        ? animationContract.document.clips.reduce((count, clip) => count + clip.frames.length, 0)
        : null,
      ...(animationContract
        ? {
            frameIds: animationContract.document.clips.flatMap((clip) =>
              clip.frames.map((frameDefinition) => frameDefinition.id)
            )
          }
        : {}),
      format: 'PNG',
      mode: 'one-frame-at-a-time',
      references: animationContract ? ['anchor', 'matrix', 'animationContract'] : ['anchor', 'matrix']
    },
...
function validateHandoff(document) {
  if (
    !document ||
    document.version !== 1 ||
    !['image-generation', 'pixel-snapper', 'frame-approval'].includes(document.kind)
  )
    throw new Error('unsupported run handoff schema');
  const approval = document.kind === 'frame-approval';
  if (document.schema !== (approval ? APPROVAL_HANDOFF_SCHEMA : HANDOFF_SCHEMA))
    throw new Error('unsupported run handoff schema');
  for (const key of ['runId', 'manifestSha256', 'configSha256', 'state'])
    if (typeof document[key] !== 'string' || document[key] === '') throw new Error(`run handoff ${key} is required`);
  if (
    !HANDOFF_FILES[document.state] ||
    !Array.isArray(document.sourceFrames) ||
    !document.next ||
    !/^[a-f0-9]{64}$/.test(document.transition?.secretSha256 ?? '')
  )
    throw new Error('run handoff state is incomplete');
  const expectedState = {
    'image-generation': 'awaiting-generated-frames',
    'pixel-snapper': 'awaiting-snapped-frames',
    'frame-approval': 'awaiting-frame-approval'
  }[document.kind];
  if (document.state !== expectedState) throw new Error('run handoff kind/state transition is invalid');
  if (approval) {
    if (
      !/^[a-f0-9]{64}$/.test(document.animationContractSha256 ?? '') ||
      !/^[a-f0-9]{64}$/.test(document.snapReceiptSha256 ?? '') ||
      !document.snapReceipt ||
      document.snapReceipt.sha256 !== document.snapReceiptSha256 ||
      !Array.isArray(document.frames) ||
      document.frames.length === 0 ||
      typeof document.toolProvenanceVerified !== 'boolean'
    )
      throw new Error('frame approval handoff binding is incomplete');
    if (
      !document.next.approveFrames ||
      !Array.isArray(document.next.approveFrames.argv) ||
      !document.next.resume ||
      !Array.isArray(document.next.resume.argv)
    )
      throw new Error('frame approval handoff invocations are incomplete');
    const ids = new Set();
    for (const [index, frameRecord] of document.frames.entries()) {
      if (
        frameRecord.index !== index ||
        typeof frameRecord.id !== 'string' ||
        frameRecord.id === '' ||
        ids.has(frameRecord.id) ||
        typeof frameRecord.path !== 'string' ||
        !/^[a-f0-9]{64}$/.test(frameRecord.sha256 ?? '') ||
        !frameRecord.landmarkSemantic
      )
        throw new Error('frame approval handoff ordered frame binding is invalid');
      ids.add(frameRecord.id);
    }
    return document;
  }
  if (!document.requiredOutputs || !Array.isArray(document.next.argv))
    throw new Error('run handoff state is incomplete');
  if (document.kind === 'pixel-snapper') {
    if (
      !Array.isArray(document.requiredOutputs) ||
      document.sourceFrames.length === 0 ||
      document.requiredOutputs.length !== document.sourceFrames.length
    )
      throw new Error('Pixel Snapper handoff frame count is invalid');
    ...
    for (let index = 0; index < document.sourceFrames.length; index += 1) {
      ...
      if (
        source.index !== index ||
        output.index !== index ||
        output.sourceSha256 !== source.sha256 ||
        !Number.isInteger(source.width) ||
        source.width < 1 ||
        !Number.isInteger(source.height) ||
        source.height < 1
      )
        throw new Error('Pixel Snapper handoff frame order or provenance is invalid');
      if (
        typeof output.path !== 'string' ||
        path.isAbsolute(output.path) ||
        path.win32.isAbsolute(output.path) ||
        output.path.includes('..') ||
        outputPaths.has(output.path)
      )
        throw new Error('Pixel Snapper handoff required output path is invalid');
...
async function loadResume(options) {
  ...
  for (const directory of [projectDir, path.join(projectDir, '.pixel-sprite-pipeline'), stateRoot, logicalRunDir]) {
    ...
    if (!runStat.isDirectory() || runStat.isSymbolicLink())
      throw new Error('run directory must be a real non-symlink directory');
  }
  ...
  const handoffArtifact = await safeRunArtifact(
    runDir,
    HANDOFF_FILES[token.state],
    token.handoffSha256,
    'canonical handoff'
  );
  const handoff = validateHandoff(JSON.parse(await fs.readFile(handoffArtifact.path, 'utf8')));
  if (handoff.runId !== requestedRunId || handoff.state !== token.state)
    throw new Error('resume token does not authenticate canonical handoff state');
  const secretArtifact = await safeRunArtifact(
    runDir,
    '.transition-secret',
    handoff.transition?.secretSha256,
    'transition secret'
  );
  const transitionSecret = await fs.readFile(secretArtifact.path);
  const expectedMac = transitionMac(transitionSecret, token);
  if (!secureHexMatches(token.mac, expectedMac))
    throw new Error('resume token does not authenticate canonical handoff state');
  const receipt = path.join(runDir, `transition-${handoff.kind}.json`);
  try {
    await fs.lstat(receipt);
    throw new Error('handoff state transition was already consumed');
  } catch (error) {
    if (error.code !== 'ENOENT') throw error;
  }
  ...
  if (manifest.runId !== requestedRunId || stableHash(manifest.config) !== handoff.configSha256)
    throw new Error('canonical handoff does not match the immutable run manifest');
  const animationContract = manifestAnimationContract(manifest);
  if (
    (handoff.animationContractSha256 !== undefined || handoff.kind === 'frame-approval') &&
    handoff.animationContractSha256 !== animationContract?.sha256
  )
    throw new Error('canonical handoff does not match the immutable animation contract');
  ...
  if (handoff.anchor?.path !== anchorInput.id || handoff.anchor?.sha256 !== anchorInput.sha256)
    throw new Error('canonical handoff anchor does not match manifest provenance');
  for (const [name, reference] of Object.entries(handoff.references ?? {}))
    await safeRunArtifact(runDir, reference.path, reference.sha256, `${name} reference`);
  for (const source of handoff.sourceFrames)
    await safeRunArtifact(runDir, source.path, source.sha256, `source frame ${source.index}`);
  if (handoff.kind === 'frame-approval') {
    const snapReceiptArtifact = await safeRunArtifact(
      runDir,
      handoff.snapReceipt.path,
      undefined,
      'selected snap receipt'
    );
    for (const frameRecord of handoff.frames) {
      if (
        path.isAbsolute(frameRecord.path) ||
        path.win32.isAbsolute(frameRecord.path) ||
        frameRecord.path.includes('\\') ||
        frameRecord.path === '..' ||
        frameRecord.path.startsWith('../') ||
        path.posix.normalize(frameRecord.path) !== frameRecord.path
      )
        throw new Error('frame approval handoff output path is invalid');
      const physical = path.join(path.dirname(snapReceiptArtifact.path), ...frameRecord.path.split('/'));
      await safeRunArtifact(
        runDir,
        await portablePath(runDir, physical),
        frameRecord.sha256,
        `frame approval output ${frameRecord.index}`
      );
    }
  }
  return {
    ...
    handoffSha256: handoffArtifact.sha256,
    handoff,
    manifest,
...
async function writeSnapperResume(context, generated, snapResult) {
  ...
  const sourceFrames = await Promise.all(
    generated.map(async (file, index) => {
      ...
      return {
        index,
        path: await portablePath(context.runDir, file),
  ...
  const requiredOutputs = generated.map((file, index) => ({
    index,
    path: `snapped/frame-${String(index).padStart(2, '0')}-snapped.png`,
    sourceSha256: sourceFrames[index].sha256
  }));
  const handoff = {
    version: 1,
    schema: HANDOFF_SCHEMA,
  ...
  await writeJsonIdempotent(
    path.join(context.runDir, `transition-${context.handoff.kind}.json`),
    { version: 1, from: context.handoffSha256, to: await sha256(handoffPath) },
    'generation transition receipt'
  );
...
async function writeApprovalResume(context, snapProvenance) {
  ...
  const selectedReceipt = await verifySnapReceipt({
    ...
    file: receiptPath,
    expectedRun: {
      id: context.handoff.runId,
      outputDir: context.runDir,
      manifestSha256: context.handoff.manifestSha256
    },
    expectedContract: context.animationContract
  });
  if (selectedReceipt.sha256 !== snapProvenance.receipt.sha256)
    throw new Error('frame approval handoff snap receipt hash mismatch');
  ...
  if (outputs.length !== definitions.length)
    throw new Error('signed snap receipt does not cover every contracted frame');
  ...
  const handoff = {
    ...
    frames,
    next: approvalInvocations(
      context.projectDir,
      context.handoff.runId,
      path.join(context.runDir, ...contractPath.split('/')),
      receiptPath
    )
  };
  ...
  await writeJsonIdempotent(
    path.join(context.runDir, `transition-${context.handoff.kind}.json`),
    { version: 1, from: context.handoffSha256, to: await sha256(handoffPath) },
    'snap-to-approval transition receipt'
  );
...
async function acquireApprovalTransition(context, selection) {
  ...
  const claim = {
    ...
    frameApprovalSha256: selection.sha256,
    approvalVersion: selection.version
  };
  ...
  try {
    ...
    if (
      existing.handoffSha256 === claim.handoffSha256 &&
      existing.frameApprovalSha256 === claim.frameApprovalSha256 &&
      existing.approvalVersion === claim.approvalVersion
    )
      throw new Error('approval transition is already in progress');
  ...
  async function release() {
    try {
      ...
      if (
        ...
        stat.isSymbolicLink() ||
        stat.nlink > 1 ||
        (await fs.readFile(claimPath, 'utf8')) !== contents
      )
        return;
  ...
  return {
    ...
    async complete(reportPath) {
      await writeJsonNew(finalPath, {
        version: 1,
        from: context.handoffSha256,
        frameApprovalSha256: selection.sha256,
        approvalVersion: selection.version,
        report: await portablePath(context.runDir, reportPath)
      });
      await release();
    }
  };
...
async function finishRun(context, snappedFrames, snapProvenance = { toolProvenanceVerified: false, receipt: null }) {
  ...
  if (contracted && !snapProvenance.approval)
    throw new Error('a signed frame approval is required before normalization');
  const approvalPayload = snapProvenance.approval?.document?.payload;
  const landmarks = contracted
    ? approvalPayload.frames.map((approved, index) => ({
        frameId: approved.id,
        source: { ...approved.landmark },
        target: {
          ...context.animationContract.document.clips.flatMap((clip) => clip.frames)[index].landmarkSemantic.target
        }
      }))
    : undefined;
  ...
  let normalized;
  try {
    const staged = await normalizeFrames({
      inputs: snappedFrames,
      outputDir: normalizedStage,
      config: context.config,
      scaleFactor: 1,
      ...(contracted ? { landmarks, animationContract: context.animationContract } : {})
    });
  ...
  const provenance = contracted
    ? {
        ...
        frameApproval: {
          path: snapProvenance.approval.path,
          sha256: snapProvenance.approval.sha256,
          version: snapProvenance.approval.version
        }
      }
    : undefined;
  const correctionContract = await createCorrectionContract({
    ...
    ...(contracted
      ? {
          ...
          frameApproval: {
            ...
            snapReceipt: {
              path: path.join(context.runDir, ...snapProvenance.receipt.path.split('/')),
              sha256: snapProvenance.receipt.sha256
            },
            version: snapProvenance.approval.version
          }
        }
  ...
  if (
    !contracted &&
    !validation.passed &&
    validation.failures.some((failure) => !REVIEW_CORRECTIONS.has(failure.correction))
  ) {
  ...
  const artifactFiles = contracted
    ? [
        exported.metadata,
        ...Object.values(exported.clips).flatMap((clip) => [
          ...clip.runtimeFrames,
          clip.sheet,
          clip.metadata,
          clip.preview
  ...
  const artifacts = await Promise.all(
    artifactFiles.map(async (file) => ({ path: await portablePath(context.runDir, file), sha256: await sha256(file) }))
  );
  const correctionManifest = automaticCorrection?.correction?.manifest
    ? {
        path: await portablePath(context.runDir, automaticCorrection.correction.manifest),
        sha256: await sha256(automaticCorrection.correction.manifest)
      }
    : null;
  const lessons = correctionManifest
    ? automaticCorrection.correction.actions.flatMap((action, actionIndex) =>
        action.approved &&
        LEARNABLE_FAILURES.has(action.code) &&
        action.evidenceVerification?.afterValidation?.passed === true
          ? [
              {
    ...
    : [];
  const humanReviewOutstanding = validation.warnings.some(
    (warning) => warning.code === 'HUMAN_REVIEW_REQUIRED' || warning.requiresUserReview === true
  );
  const report = {
    version: 1,
    ...
    correctionContract: {
      path: await portablePath(context.runDir, correctionContract.path),
      sha256: correctionContract.sha256
    },
    ...(provenance ?? {}),
    toolProvenanceVerified: snapProvenance.toolProvenanceVerified === true,
    snapReceipt: snapProvenance.receipt
      ? { path: snapProvenance.receipt.path, sha256: snapProvenance.receipt.sha256 }
      : null,
    ...(contracted
      ? {
          frameApproval: {
    ...
    ...(contracted
      ? {
          projectAcceptance: {
            ...
            requiresUserApproval: true,
            reason:
              snapProvenance.toolProvenanceVerified === true
                ? 'Requires private-project visual acceptance.'
                : 'Manual handoff provenance is permanently ineligible for private-project release acceptance.'
          }
        }
      : {})
  };
  const recorded = await recordRunResultIdempotent({ context, report });
  const correctionReceipt = await sealCorrectionContract({
    projectDir: context.projectDir,
    runDir: context.runDir,
    runId: context.handoff.runId,
    contract: correctionContract
  });
  if (validation.passed && context.handoff.kind !== 'frame-approval')
    await writeJsonIdempotent(
      path.join(context.runDir, `transition-${context.handoff.kind}.json`),
      { version: 1, from: context.handoffSha256, report: await portablePath(context.runDir, recorded.reportPath) },
      'completion transition receipt'
    );
  const result = {
    state: 'complete',
    runId: context.handoff.runId,
    ...
    exported,
    validation,
    correction: automaticCorrection?.correction ?? null,
    correctionReceipt: {
      path: await portablePath(context.runDir, correctionReceipt.path),
      sha256: correctionReceipt.sha256,
      signature: correctionReceipt.signature
    },
    report: { ...report, ...recorded },
    recorded,
    profilePromotion: report.profilePromotion
  };
...
async function resumeRun(options, { manifestPath = packagedToolManifest() } = {}) {
  ...
  if (context.handoff.kind !== 'frame-approval' && (options.frameApproval || options.approvalVersion))
    throw new Error('frame approval selection is valid only for an authenticated awaiting-frame-approval state');
  if (context.handoff.kind === 'frame-approval') {
    if (!options.frameApproval || !options.approvalVersion)
      throw new Error('an explicitly selected numbered signed frame approval is required before normalization');
    const approvalPath = path.resolve(options.frameApproval);
    const approvalArtifact = await safeRunArtifact(
      context.runDir,
      await portablePath(context.runDir, approvalPath),
      undefined,
      'selected frame approval'
    );
    const receiptArtifact = await safeRunArtifact(
      context.runDir,
      context.handoff.snapReceipt.path,
      undefined,
      'selected snap receipt'
    );
    ...
    if (
      verified.document.payload.animationContractSha256 !== context.handoff.animationContractSha256 ||
      verified.document.payload.snapReceiptSha256 !== context.handoff.snapReceiptSha256 ||
      approvedFrames.length !== context.handoff.frames.length
    )
      throw new Error('signed frame approval does not match the immutable approval handoff');
    for (let index = 0; index < approvedFrames.length; index += 1) {
      ...
      if (
        approved.index !== expected.index ||
        approved.id !== expected.id ||
        approved.path !== expected.path ||
        approved.sha256 !== expected.sha256 ||
        JSON.stringify(approved.landmarkSemantic) !== JSON.stringify(expected.landmarkSemantic)
      )
        throw new Error('signed frame approval ordered outputs do not match the immutable approval handoff');
    }
    const snappedFrames = context.handoff.frames.map((frameRecord) =>
      path.join(path.dirname(receiptArtifact.path), ...frameRecord.path.split('/'))
    );
    const transition = await acquireApprovalTransition(context, {
    ...
    try {
      const result = await finishRun(context, snappedFrames, {
        ...
        approval: {
          path: await portablePath(context.runDir, verified.path),
          sha256: verified.sha256,
          version: options.approvalVersion,
  ...
  if (context.handoff.kind === 'pixel-snapper') {
    ...
    if (supplied.length !== expected || context.handoff.requiredOutputs.length !== expected)
      throw new Error(`exactly ${expected} snapped frames are required in source-frame order`);
    const outputNames = context.handoff.requiredOutputs.map((item) => path.basename(item.path));
    const snappedFrames = await stageBatch({
      inputs: supplied,
      targetDir: path.join(context.runDir, 'snapped'),
      label: 'snapped frame batch',
      outputNames
    });
    const manualReceipt = await writeManualHandoffReceipt({
      projectDir: context.projectDir,
      run: { id: context.handoff.runId, outputDir: context.runDir, manifestSha256: context.handoff.manifestSha256 },
      handoff: context.handoffPath,
      inputs: context.handoff.sourceFrames.map((source) => path.join(context.runDir, ...source.path.split('/'))),
      outputs: snappedFrames
    });
    const snapReceipt = {
      path: await portablePath(context.runDir, manualReceipt.path),
      sha256: manualReceipt.sha256,
      signature: manualReceipt.document.signature
    };
    if (context.animationContract)
      return writeApprovalResume(context, { toolProvenanceVerified: false, receipt: snapReceipt });
    return {
      ...(await finishRun(context, snappedFrames, { toolProvenanceVerified: false, receipt: snapReceipt })),
      snapReceipt
    };
  }
  ...
  if (context.animationContract) {
    ...
    if (supplied.length !== expected)
      throw new Error(`exactly ${expected} generated frames are required in animation-contract order`);
  }
  const generated = await stageBatch({
    inputs: supplied,
    targetDir: path.join(context.runDir, 'generated'),
    label: 'generated frame batch'
  });
  ...
  try {
    const snapped = await runPixelSnapper({
      ...
      receipt: {
        projectDir: context.projectDir,
        run: { id: context.handoff.runId, outputDir: snapStage, manifestSha256: context.handoff.manifestSha256 },
        contract: context.animationContract ?? { sha256: context.handoffSha256 },
        durableReceiptFile: path.join(snappedDir, 'snap-receipt.json')
      }
    });
    ...
    if (snapped.outputs.length !== generated.length)
      throw new Error('Pixel Snapper output count does not match source frame count');
    if (snapped.recoveredExistingReceipt) {
      const provenance = {
        toolProvenanceVerified: true,
        receipt: { ...snapped.receipt, path: await portablePath(context.runDir, snapped.receipt.path) }
      };
      return context.animationContract
        ? writeApprovalResume(context, provenance)
        : finishRun(context, snapped.outputs, provenance);
    }
    ...
    const provenance = {
      ...
      receipt: {
        ...snapped.receipt,
        path: await portablePath(context.runDir, path.join(snappedDir, path.basename(snapped.receipt.path)))
      }
    };
    return context.animationContract
      ? writeApprovalResume(context, provenance)
      : finishRun(context, outputs, provenance);
  } catch (error) {
    await fs.rm(snapStage, { recursive: true, force: true });
    throw error;
  }
...
function correctionRevisionVersion(document) {
  if (!Number.isInteger(document.reportVersion) || document.reportVersion < 2)
    throw new Error('contracted correction requires an explicit reportVersion of at least 2');
  ...
  return {
    ...
    next: {
      ...
      argv: [
        ...
        'correct',
        '--request',
        '<SAME_SIGNED_CORRECTION_REQUEST>',
        '--replacement-snapped-frame',
        '<CORRECTED_SNAPPED_FRAME>',
        '--project-dir',
        '<PROJECT_DIR>'
      ]
    }
...
function exactJsonDocument(actual, expected, label) {
  if (JSON.stringify(stableValue(actual)) !== JSON.stringify(stableValue(expected)))
    throw new Error(`${label} does not match the authenticated correction revision`);
}
...
async function requireCanonicalRevisionDirectory(context, version) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink())
    throw new Error('correction revision directory must be a real directory');
  ...
  if (
    ...
    throw new Error('correction revision directory escaped the authenticated run');
  return revisionDir;
}
...
async function correctionApprovalHandoff({ context, document, version, receipt, definitions }) {
  const frames = receipt.document.payload.outputs.map((output, index) => ({
    index,
    id: definitions[index].id,
    path: output.path,
    sha256: output.sha256,
...
async function stageCorrectionRevision({ context, document, revision, inputs }) {
  ...
  if (inputs.length !== definitions.length)
    throw new Error(`exactly ${definitions.length} corrected snapped frames are required in animation-contract order`);
  const outputNames = definitions.map((_, index) => `frame-${String(index).padStart(2, '0')}-snapped.png`);
  const outputs = await stageBatch({
    inputs,
    targetDir: path.join(revision.revisionDir, 'snapped'),
    label: 'correction revision snapped batch',
    outputNames
  });
  const outputRecords = await Promise.all(
    outputs.map(async (file) => ({
      relative: await portablePath(revision.revisionDir, file),
      sha256: await sha256(file)
    }))
  );
  const replacementHandoff = correctionReplacementHandoff({
    context,
    document,
    version: revision.request.revisionVersion,
    revisionRequestSha256: await sha256(revision.file),
  ...
  const oldInputs = context.ancestry.verifiedApproval.document.payload.frames.map((frameRecord) =>
    path.join(path.dirname(context.ancestry.receiptFile), ...frameRecord.path.split('/'))
  );
  ...
  const approvalHandoff = await correctionApprovalHandoff({
    context,
    document,
    version: revision.request.revisionVersion,
    receipt,
...
async function finishCorrectionRevision({ context, document, revision, approvalFile, approvalVersion }) {
  ...
  if (
    ...
    approvalVersion <= context.provenance.frameApproval.version
  )
    throw new Error('a new numbered frame approval revision is required for corrected pixels');
  const revisionDir = await requireCanonicalRevisionDirectory(context, version);
  const requestArtifact = await revisionArtifact(
    context,
    version,
  ...
  exactJsonDocument(
    actualRequest,
    correctionRevisionRequest(context, document, version),
    'correction revision request'
  );
  ...
  for (const [index] of definitions.entries()) {
    ...
    outputArtifacts.push({
      ...(await revisionArtifact(
        context,
        version,
        relative,
        undefined,
        `correction revision replacement frame ${index}`
      )),
      relative
    });
  }
  const expectedReplacement = correctionReplacementHandoff({
    context,
    document,
    version,
    revisionRequestSha256: requestArtifact.sha256,
    outputs: outputArtifacts
  });
  const replacementArtifact = await revisionArtifact(
    context,
    version,
    'replacement-handoff.json',
    undefined,
    'correction revision replacement handoff'
  );
  ...
  const receiptArtifact = await revisionArtifact(
    context,
    version,
    'manual-handoff-receipt.json',
    undefined,
    'correction revision snap receipt'
  );
  ...
  const handoffArtifact = await revisionArtifact(
    context,
    version,
    'frame-approval-handoff.json',
    undefined,
    'correction revision frame approval handoff'
  );
  ...
  const expectedApprovalPath = path.join(
    revisionDir,
    `frame-approval-${String(approvalVersion).padStart(2, '0')}.json`
  );
  if (!(await sameCanonicalPath(selectedApproval, expectedApprovalPath)))
    throw new Error('new frame approval must be the canonical numbered file in the correction revision');
  const approvalArtifact = await revisionArtifact(
    context,
    version,
    path.basename(expectedApprovalPath),
    undefined,
    'correction revision frame approval'
  );
  const approval = await verifyFrameApproval({
    projectDir: projectDirectory(context),
    file: approvalArtifact.path,
    contract: context.manifest.animationContract,
    snapReceipt: { path: receiptFile, sha256: receipt.sha256 },
    version: approvalVersion
  });
  if (
    approval.document.payload.frames.length !== expectedHandoff.frames.length ||
    approval.document.payload.frames.some(
      (frameRecord, index) =>
        frameRecord.id !== expectedHandoff.frames[index].id ||
        frameRecord.path !== expectedHandoff.frames[index].path ||
        frameRecord.sha256 !== expectedHandoff.frames[index].sha256
    )
  )
    throw new Error('new frame approval does not match the corrected revision outputs');
  const sourceFrames = outputArtifacts.map((output) => output.path);
  const landmarks = approval.document.payload.frames.map((frameRecord, index) => ({
    frameId: frameRecord.id,
    source: frameRecord.landmark,
    target: definitions[index].landmarkSemantic.target
  }));
  ...
  try {
    const staged = await normalizeFrames({
      ...
      scaleFactor: 1,
      landmarks,
      animationContract: context.manifest.animationContract
    });
  ...
  let exported;
  try {
    const staged = await exportContractAnimation({
      normalized,
      contract: context.manifest.animationContract,
      outputDir: runtimeStage,
      config: context.config,
      columns: Math.min(4, normalized.frames.length),
      frameApprovalSha256: approval.sha256
    });
  ...
  const anchorReport = await inspectImage(path.join(context.runDir, context.contract.anchor.path), {
    tolerance: context.config.background.tolerance,
    backgroundColor: context.config.background.mode === 'configured' ? context.config.background.color : undefined
  });
  ...
  const artifacts = await Promise.all(
    ...
    ].map(async (file) => ({ path: await portablePath(context.runDir, file), sha256: await sha256(file) }))
  );
  const humanReviewOutstanding = validation.warnings.some(
    (warning) => warning.code === 'HUMAN_REVIEW_REQUIRED' || warning.requiresUserReview === true
  );
  const report = {
    ...
    frameApprovalSha256: approval.sha256,
    toolProvenanceVerified: false,
    snapReceipt: { path: await portablePath(context.runDir, receiptFile), sha256: receipt.sha256 },
    frameApproval: {
      path: await portablePath(context.runDir, approvalArtifact.path),
    ...
    projectAcceptance: {
      ...
      requiresUserApproval: true,
      reason:
        'Correction replacement was supplied through a manual handoff and cannot inherit verified-tool provenance.'
    }
  };
  const reportFile = path.join(revisionDir, 'report.json');
  await writeJsonIdempotent(reportFile, report, 'correction revision report');
  await writeJsonIdempotent(
    path.join(revisionDir, 'revision-complete.json'),
    { version: 1, report: 'report.json', reportSha256: await sha256(reportFile), frameApprovalSha256: approval.sha256 },
    'correction revision completion'
  );
  setValidationExit(validation);
  return {
    state: 'complete',
...
async function handleContractCorrection({ context, document, options, beforeValidation }) {
  if (!beforeValidation.failures.some((failure) => sameFailure(failure, document.declaredFailure)))
    throw new Error('declared correction failure is not identified by objective validation');
  ...
  if (replacements.length > 0 && options.frameApproval)
    throw new Error('replacement snapped frames and frame approval must be submitted in separate revision steps');
  if (replacements.length > 0)
    return stageCorrectionRevision({
      context,
      document,
      revision,
      inputs: replacements.map((file) => path.resolve(file))
    });
  if (options.frameApproval || options.approvalVersion) {
    if (!options.frameApproval || !options.approvalVersion)
      throw new Error('both --frame-approval and --approval-version are required');
    return finishCorrectionRevision({
      context,
      document,
      revision,
      approvalFile: options.frameApproval,
      approvalVersion: options.approvalVersion
    });
  }
  process.exitCode = EXIT.handoff;
...
export function createProgram({
  setupPixelSnapperImpl = setupPixelSnapper,
  ...
  program
    ...
    .action(async (options) =>
      print(await inspectImage(path.resolve(options.input), { tolerance: options.tolerance }))
    );
  ...
  program
    ...
    .action(async (options) => {
      ...
      const request = validateApprovalRequest(
        JSON.parse(await fs.readFile(path.resolve(options.approvalRequest), 'utf8'))
      );
      await ensureReceiptState(projectDir);
      printImpl(
        await writeFrameApproval({
          projectDir,
          runDir: path.dirname(path.resolve(options.snapReceipt)),
          contract: await loadAnimationContract(path.resolve(options.contract), {
            projectId: config.integration.projectId
          }),
          snapReceipt: { path: path.resolve(options.snapReceipt) },
  ...
  program
    ...
    .action(async (options) => {
      ...
      if (options.snapReceipt) {
        receipt = await verifySnapReceipt({
          projectDir,
          file: path.resolve(options.snapReceipt),
          expectedContract: contract
        });
        snappedOutputs = receipt.document.payload.outputs.map((record) =>
          path.resolve(path.dirname(receipt.path), record.path)
        );
      } else {
        ...
        const snapped = await runPixelSnapper({
          ...
          resolverOptions: await snapperResolverOptions(
            path.resolve(options.snapperProjectDir ?? projectDir),
            manifestPath
          ),
          receipt: {
            projectDir,
            run: { id: null, outputDir: snapDir, manifestSha256: stableHash(selectedInputs.document) },
            contract
          }
        });
        if (snapped.status === 'manual-handoff') {
      ...
      const verifiedApproval = await verifyFrameApproval({
        projectDir,
        file: frameApprovalFile,
        contract,
        snapReceipt: receiptSelection,
        version
      });
      if (
        snappedOutputs.length !==
        contract.document.clips.flatMap((clip) => clip.frames.flatMap((frame) => frame.tracks)).length
      )
        throw new Error('signed snap receipt membership does not match the v2 contract');
      const normalized = await normalizeContractFrames({
        contract,
        frameApproval: verifiedApproval,
        outputDir: path.join(outputRoot, 'normalized')
      });
      const exported = await exportContractAnimation({
        normalized,
        contract,
        outputDir: path.join(outputRoot, 'export'),
        config,
      ...
      const report = await validateRun({
        ...
        frameApproval: frameApprovalSelection
      });
      if (!report.passed) {
        printImpl({
          status: 'objective-failure',
          ...productionBinding,
          receipt: receiptSelection,
      ...
      printImpl({
        ...
        exports: {
          ...
          artifacts: await productionArtifacts(outputRoot, ['normalized', 'export'])
        },
        report
      });
    });
  ...
  program
    ...
    .action(async (options) => {
      ...
      let result = await runPixelSnapper({
        ...
        outputDir,
        config,
        resolverOptions: await snapperResolverOptions(cwd, manifestPath),
        receipt: {
          projectDir: cwd,
          run: { id: null, outputDir, manifestSha256: null },
          contract: { sha256: stableHash({ kind: 'standalone-snap', arguments: config.snapper?.args ?? ['16'] }) }
        }
      });
      if (result.status === 'manual-handoff')
        result = await enhanceStandaloneSnapperHandoff({ result, inputs, outputDir, cwd });
      print(result);
    ...
    .action(async (options) => {
      ...
      print(
        await normalizeFrames({
          inputs: combinedFrames(options),
          outputDir,
          config: await configFor(options),
    ...
    .action(async (options) => {
      ...
      if (options.contract) {
        if (options.duration.length > 0)
          throw new Error('--duration cannot be used with --contract; contract frame durations are authoritative');
        if (!options.normalization) throw new Error('--normalization is required with --contract');
        if ((options.frame?.length ?? 0) > 0 || (options.frames?.length ?? 0) > 0)
          throw new Error(
            '--frame and --frames cannot be used with --contract; the normalization manifest is authoritative'
          );
        ...
        if (
          ...
          Object.keys(normalized).length !== allowed.length ||
          allowed.some((key) => !Object.hasOwn(normalized, key))
        )
          throw new Error('contract export normalization manifest schema is invalid');
        print(
          await exportContractAnimation({
            normalized,
            contract: await loadAnimationContract(path.resolve(options.contract), {
      ...
      if (options.normalization || options.frameApprovalSha256)
        throw new Error('--normalization and --frame-approval-sha256 require --contract');
      ...
      if (durations.length !== frames.length || durations.some((value) => value < 11 || value > 65535))
        throw new Error('durations must provide one integer from 11 to 65535 per frame');
      print(
        await exportAnimation({ frames, outputDir, config, columns: options.columns, durations, name: options.name })
      );
    });
  ...
  program
    ...
    .requiredOption('--project-dir <dir>')
    .option(
      '--replacement-snapped-frame <file>',
      'corrected snapped frame for a contracted immutable revision; repeat in contract order',
      frame,
      []
    )
    ...
    .action(async (options) => {
      ...
      const context = await loadCorrectionContext({
        ...
        receiptSignature: document.receiptSignature,
        declaredFailure: document.declaredFailure
      });
      ...
      if ((options.replacementSnappedFrame?.length ?? 0) > 0 || options.frameApproval || options.approvalVersion)
        throw new Error('contracted correction revision options require an animation-contract run');
      const { runDir, config, request, manifest } = context;
      const beforeValidation = await validateRun({ ...request, config });
      if (!beforeValidation.failures.some((failure) => sameFailure(failure, document.declaredFailure)))
        throw new Error('declared correction failure is not identified by objective validation');
      const candidates = [
        request.anchorReport.path,
        ...request.normalized.frames,
        ...request.normalized.measurements.map((item) => item.input),
        ...request.exported.runtimeFrames,
        request.exported.sheet,
        request.exported.metadata,
        request.exported.preview
      ];
      ...
      if (result.afterValidation.passed && result.correction) {
        const delivered = [
          ...result.exported.runtimeFrames,
          result.exported.sheet,
          result.exported.metadata,
          result.exported.preview
        ];
        const artifacts = await Promise.all(
          delivered.map(async (file) => ({ path: await portablePath(runDir, file), sha256: await sha256(file) }))
        );
        const correctionManifest = {
          path: await portablePath(runDir, result.correction.manifest),
          sha256: await sha256(result.correction.manifest)
        };
        const lessons = result.correction.actions.flatMap((action, actionIndex) =>
          action.approved &&
          LEARNABLE_FAILURES.has(action.code) &&
          action.evidenceVerification?.afterValidation?.passed === true
            ? [
  ...
  program
    ...
    .action(async (options) =>
      print(
        await promoteVerifiedProfile({
          projectDir: path.resolve(options.projectDir),
          runId: safeRunId(options.run),
  ...
  program
    ...
    .action(async (options) => {
      ...
      if (options.resume) {
        ...
        if (options.frameApproval || options.approvalVersion)
          throw new Error('--frame-approval and --approval-version require an authenticated approval resume');
        ...
        result =
          suppliedFrames === 0
            ? started
            : await resumeRun(
                { ...options, resume: started.runId, resumeToken: started.resumeToken },
                { manifestPath }
              );
      }
      print(result);
...
try {
  directExecution = (await fs.realpath(process.argv[1] ?? '')) === (await fs.realpath(CLI_PATH));
} catch {}
if (directExecution) {
  try {
