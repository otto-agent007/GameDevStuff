...
export async function removeCorrectionTree(directory, remove = fs.rm) {
  await remove(directory, CLEANUP_OPTIONS);
}
...
function onlyReviewFailures(report) {
  return (
    ...
    report.failures.every((failure) => GENERATIVE.has(failure.correction) || REVIEW.has(failure.correction))
  );
}
...
function validateRun(run) {
  if (!run || typeof run.runDir !== 'string' || run.runDir.trim() === '')
    throw new Error('run runDir must be a nonempty path');
  ...
  if (run.generativeAttempts !== undefined) {
    ...
    if (
      ...
      throw new Error('run generativeAttempts must be a per-frame object');
  }
}
...
function equivalentKey(failure, runDir) {
  ...
  const before =
    typeof failure.before === 'string'
      ? path.isAbsolute(failure.before) || path.win32.isAbsolute(failure.before)
        ? path.resolve(failure.before)
        : path.resolve(runDir, failure.before)
      : null;
...
async function inputRecords(run, runDir) {
  ...
  for (const value of values.filter((item) => typeof item === 'string')) {
    const resolved =
      path.isAbsolute(value) || path.win32.isAbsolute(value) ? path.resolve(value) : path.resolve(runDir, value);
...
function validateRetryState(failures, attempts = {}) {
  for (const [frame, count] of Object.entries(attempts)) {
    ...
    if (!Number.isSafeInteger(numeric) || numeric < 0 || String(numeric) !== frame)
      throw new Error('retry frame IDs must be canonical nonnegative integers');
    if (!Number.isSafeInteger(count) || count < 0) throw new Error('retry counts must be nonnegative integers');
  }
  for (const failure of failures)
    if (GENERATIVE.has(failure.correction) && (!Number.isSafeInteger(failure.frame) || failure.frame < 0)) {
      throw new Error('generative failures require a nonnegative integer frame ID');
    }
}

function resolvedOutput(output, stagingDir) {
  return path.isAbsolute(output) || path.win32.isAbsolute(output)
    ? path.resolve(output)
    : path.resolve(stagingDir, output);
}
...
async function verifyMetadataArtifact(file, expected) {
  ...
  if (
    !Number.isInteger(document.frameSize?.width) ||
    document.frameSize.width < 1 ||
    !Number.isInteger(document.frameSize?.height) ||
    document.frameSize.height < 1
  )
    return false;
  if (
    ![document.canonicalPivot, document.pivot].every((pivot) => Number.isFinite(pivot?.x) && Number.isFinite(pivot?.y))
  )
    return false;
  if (!Array.isArray(document.frames) || !Array.isArray(document.sources) || !Array.isArray(document.durations))
    return false;
  if (document.frames.length !== document.sources.length || document.frames.length !== document.durations.length)
    return false;
  if (document.durations.some((duration) => !Number.isInteger(duration) || duration < 11 || duration > 65535))
    return false;
  if (document.sources.some((source, index) => source?.index !== index || !/^[a-f0-9]{64}$/.test(source?.sha256 ?? '')))
    return false;
  if (
    document.frames.some(
      (frame, index) =>
        frame?.index !== index ||
        frame?.duration !== document.durations[index] ||
        frame?.width !== document.frameSize.width ||
        frame?.height !== document.frameSize.height ||
  ...
  const normalizeDeliveryPaths = (value) => {
    ...
    if (Array.isArray(normalized.frames))
      for (const frame of normalized.frames)
        if (typeof frame?.file === 'string') frame.file = portableBasename(frame.file);
    return normalized;
  };
...
async function runtimeImages(expected) {
  ...
  if (images.some((image) => image.info.width !== width || image.info.height !== height || image.info.channels !== 4))
    return null;
...
async function verifyPreviewArtifact(file, expected, trustedMetadata) {
  try {
    ...
    if (!runtime || !Array.isArray(expected.durations) || expected.durations.length !== runtime.images.length)
      return false;
    ...
    if (pages !== runtime.images.length || preview.info.width !== runtime.width || pageHeight !== runtime.height)
      return false;
    if (pages === 1) {
      if (!Array.isArray(trustedMetadata?.durations) || !jsonEqual(trustedMetadata.durations, expected.durations))
        return false;
    ...
    for (let index = 0; index < pages; index += 1)
      if (!preview.data.subarray(index * pageBytes, (index + 1) * pageBytes).equals(runtime.images[index].data))
        return false;
    return true;
  } catch {
    return false;
...
async function verifySheetArtifact(file, expected) {
  try {
    ...
    if (sheet.info.width !== expected.columns * runtime.width || sheet.info.height !== rows * runtime.height)
      return false;
    for (let y = 0; y < sheet.info.height; y += 1)
      for (let x = 0; x < sheet.info.width; x += 1) {
        ...
        for (let channel = 0; channel < 4; channel += 1)
          if (sheet.data[sheetOffset + channel] !== runtime.images[cell].data[frameOffset + channel]) return false;
      }
    return true;
  } catch {
    return false;
...
async function verifyArtifactForFailure(failure, file, run) {
  if (failure.correction === 'reexport-metadata' || String(failure.stage ?? '').startsWith('metadata'))
    return verifyMetadataArtifact(file, run.expected?.metadata);
  if (failure.correction === 'reexport-preview' || String(failure.stage ?? '').includes('preview'))
    return verifyPreviewArtifact(file, run.expected?.preview, run.expected?.metadata);
  if (failure.correction === 'reexport-sheet' || String(failure.stage ?? '').includes('sheet'))
    return verifySheetArtifact(file, run.expected?.sheet);
  if (['METADATA_MISMATCH', 'SOURCE_HASH_MISMATCH', 'TIMING_MISMATCH'].includes(failure.code))
    return verifyMetadataArtifact(file, run.expected?.metadata);
  if (failure.code === 'FRAME_COUNT' && (failure.stage === 'metadata' || failure.correction === 'reexport-metadata'))
    return verifyMetadataArtifact(file, run.expected?.metadata);
  if (['FRAME_COUNT', 'PREVIEW_MISMATCH'].includes(failure.code))
    return verifyPreviewArtifact(file, run.expected?.preview, run.expected?.metadata);
...
async function verifyRevalidation(failure, result, outputs, stagingDir, run) {
  ...
  const evidence = Array.isArray(result.revalidations)
    ? result.revalidations.find((item) => sameTarget(item?.target, target))
    : null;
  if (
    !evidence ||
    !Array.isArray(evidence.beforeValidation?.failures) ||
    !Array.isArray(evidence.afterValidation?.failures)
  )
    return invalid;
  if (
    evidence.beforeValidation.passed !== false ||
    (evidence.afterValidation.passed !== true && !onlyReviewFailures(evidence.afterValidation))
  )
    return invalid;
  ...
  if (failure.code === 'CANVAS_SIZE') {
    if (
      ...
      failure.expected.length !== 2 ||
      !Array.isArray(failure.actual) ||
      failure.actual.length !== 2
    )
      return invalid;
  ...
  if (
    failure.code === 'PIVOT_DRIFT' &&
    (failure.expected === undefined || JSON.stringify(afterMeasurements?.pivot) !== JSON.stringify(failure.expected))
  )
    return invalid;
  if (
    failure.code === 'BASELINE_DRIFT' &&
    (failure.expected === undefined || afterMeasurements?.baseline !== failure.expected)
  )
    return invalid;
  if (
    failure.code === 'BACKGROUND_REMAINS' &&
    (afterMeasurements?.background?.opaqueBorderPixels !== 0 ||
      afterMeasurements?.background?.configuredColorPixels !== 0)
  )
    return invalid;
  if (
    failure.code === 'INTERMEDIATE_COLORS' &&
    (afterMeasurements?.scale?.nearestNeighbor !== true || afterMeasurements?.scale?.intermediateColors !== 0)
  )
    return invalid;
  if (
    failure.code === 'NON_INTEGER_SCALE' &&
    (afterMeasurements?.scale?.integer !== true || afterMeasurements?.scale?.uniform !== true)
  )
    return invalid;
  if (
    failure.code === 'GLOBAL_SCALE_DRIFT' &&
...
function translateDeep(value, stagingDir, correctionDir, artifacts, ancestors = new Set()) {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([key, item]) => [key, translateDeep(item, stagingDir, correctionDir, artifacts, next)])
  );
}
...
async function assertStoredReferences(value, correctionDir, ancestors = new Set()) {
  ...
  for (const item of Array.isArray(value) ? value : Object.values(value))
    await assertStoredReferences(item, correctionDir, next);
}
...
async function collectStagingArtifacts(value, stagingDir, artifacts = new Set(), ancestors = new Set()) {
  if (typeof value === 'string') {
    ...
    try {
      ...
      if (path.isAbsolute(value) || path.win32.isAbsolute(value))
        throw new Error(`missing staged artifact reference: ${value}`);
    }
    return artifacts;
  }
  ...
  const next = new Set(ancestors).add(value);
  for (const item of Array.isArray(value) ? value : Object.values(value))
    await collectStagingArtifacts(item, stagingDir, artifacts, next);
  return artifacts;
}
...
async function correctionRunBinding(runDir) {
  ...
  try {
    ...
    return {
      id: manifest.runId ?? path.basename(runDir),
...
export async function applyDeterministicCorrections({ failures, run, config, operations = {} }) {
  ...
  for (const failure of failures) {
    if (!failure || typeof failure.code !== 'string' || typeof failure.correction !== 'string')
      throw new Error('each failure must be classified before correction');
    if (!AUTOMATIC.has(failure.correction) && !GENERATIVE.has(failure.correction) && !REVIEW.has(failure.correction))
      throw new Error(`unknown correction action requires review: ${failure.correction}`);
    if (AUTOMATIC.has(failure.correction) && typeof operations[failure.correction] !== 'function')
      throw new Error(`missing deterministic correction operation: ${failure.correction}`);
  }
  ...
  try {
    for (let index = 0; index < failures.length; index += 1) {
      ...
      if (GENERATIVE.has(failure.correction)) {
        ...
        actions.push({
          ...failure,
          status: 'blocked',
          requires: attempts > 0 ? 'generative-retry' : 'generative-retry-exhausted',
          generativeAttemptsRemaining: attempts
        });
        continue;
      }
      ...
      const result = await operations[failure.correction]({
        failure,
      ...
      if (!result || typeof result !== 'object')
        throw new Error(`deterministic correction ${failure.correction} must return a result`);
      ...
      for (const resolved of artifacts) {
        if (!inside(stagingDir, resolved))
          throw new Error(`correction output is outside correction directory: ${resolved}`);
        ...
        if (!inside(await fs.realpath(stagingDir), physical))
          throw new Error(`correction output is outside correction directory: ${resolved}`);
        if (
          inputs.some(
            (input) =>
              ...
              (input.dev === outputStat.dev && input.ino === outputStat.ino)
          )
        )
          throw new Error(`correction output is the same file as an input: ${resolved}`);
      }
      if (failure.correction !== 'reexport-metadata') {
        if (outputs.length === 0)
          throw new Error(`deterministic correction ${failure.correction} must produce a valid image`);
      ...
      for (const groupIndex of groupIndices)
        verifications.set(groupIndex, await verifyRevalidation(failures[groupIndex], result, outputs, stagingDir, run));
      ...
      executions.set(key, {
        ...
        translatedResult,
        artifacts,
        batchApproved: result.validationPassed === true && result.improved === true
      });
      actions.push({
        ...failure,
    ...
    return {
      ...
      actions,
      generativeAttemptsRemaining: remaining,
      manifest: path.join(correctionDir, 'manifest.json')
    };
  } catch (error) {
    ...
    if (cleanupFailures.length > 0)
      throw new AggregateError([error, ...cleanupFailures], 'correction failed and cleanup did not complete');
    throw error;
  }
}
