...
export function classifyFailures(report) {
  ...
  return report.failures.map((failure) => {
    ...
    if (failure.code === 'FRAME_COUNT')
      correction =
        failure.stage === 'metadata' && failure.trustedArtifact === true ? 'reexport-metadata' : 'stop-for-review';
    if (failure.code === 'SOURCE_HASH_MISMATCH')
      correction =
        failure.stage === 'metadata' && failure.trustedArtifact === true ? 'reexport-metadata' : 'stop-for-review';
...
export function validateIntegerScale({ source, output }) {
  const validSize = (size) =>
    size && Number.isInteger(size.width) && size.width > 0 && Number.isInteger(size.height) && size.height > 0;
...
function paletteCounts(images) {
  ...
  for (const image of images)
    for (const entry of paletteOf(image)) {
...
async function decodeAnimatedPreview(input) {
  ...
  let decoded;
  try {
    decoded = await rawPipeline.toBuffer({ resolveWithObject: true });
  } finally {
    rawPipeline.destroy();
  }
  const metadataPipeline = sharp(input, { animated: true });
  try {
    return { ...decoded, metadata: await metadataPipeline.metadata() };
  } finally {
...
async function retainedForeground(file, image, config, { configuredBackground = true } = {}) {
  return extractPrimaryComponent(file, {
    image,
    isForeground: foregroundPredicate(image, {
      color: configuredBackground ? config.background?.color : null,
...
function normalizedMatchesApprovedSource(source, normalized, scale, left, top) {
  for (let y = 0; y < normalized.height; y += 1) {
    for (let x = 0; x < normalized.width; x += 1) {
      ...
      if (!inside) {
        if (
          normalized.data[outputOffset] !== 0 ||
          normalized.data[outputOffset + 1] !== 0 ||
          normalized.data[outputOffset + 2] !== 0 ||
          normalized.data[outputOffset + 3] !== 0
        )
          return false;
        continue;
      }
      ...
      for (let channel = 0; channel < 4; channel += 1)
        if (normalized.data[outputOffset + channel] !== source.data[sourceOffset + channel]) return false;
    }
  }
  return true;
}
...
function sheetMatches(sheet, runtimeImages, columns, rows, cell) {
  ...
  for (let y = 0; y < sheet.height; y += 1) {
    for (let x = 0; x < sheet.width; x += 1) {
      ...
      for (let channel = 0; channel < 4; channel += 1)
        if (sheet.data[sheetOffset + channel] !== frame.data[frameOffset + channel]) return false;
    }
  }
  return true;
}
...
function validSemanticEvidence(item) {
  return (
    item && SEMANTIC_CODES.has(item.code) && item.failed === true && item.evidence && typeof item.evidence === 'object'
  );
}
...
function snapshotAnimationContract(animationContract) {
  ...
  if (animationContract.sha256 !== stableHash(document))
    throw new Error('validation animation contract hash is invalid');
...
function snapshotFrameApproval(frameApproval, animationContract) {
  ...
  if (
    [
      frameApproval.projectDir,
      frameApproval.file,
      frameApproval.snapReceipt.path,
      frameApproval.snapReceipt.sha256
    ].some((value) => typeof value !== 'string' || value === '') ||
    !Number.isInteger(frameApproval.version) ||
    frameApproval.version < 1
  )
    throw new Error('frame approval selection is invalid');
  return {
    projectDir: frameApproval.projectDir,
    file: frameApproval.file,
    snapReceipt: { path: frameApproval.snapReceipt.path, sha256: frameApproval.snapReceipt.sha256 },
    version: frameApproval.version
  };
}

function flatContractFrames(contract) {
...
async function containedArtifact(root, relative) {
  ...
  if (containment === '..' || containment.startsWith(`..${path.sep}`) || path.isAbsolute(containment))
    throw new Error('contract export artifact escaped its root');
  let current = root;
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink > 1)
    throw new Error('contract export artifact must be a regular single-link file');
  return candidate;
}
...
function colorsOutsidePalette(image, allowed) {
  return paletteOf(image)
    .map(({ rgba }) => rgbaKey(rgba))
...
async function validateContractExportRun(
  { anchorReport, normalized, exported, config, semanticEvidence, animationContract, frameApproval },
  { afterContractCapture } = {}
) {
  ...
  if (
    !jsonEqual(animationContract.document.sizes.canonical, [config.canonical.width, config.canonical.height]) ||
    !jsonEqual(animationContract.document.pivot, config.pivot)
  )
    throw new Error('validation animation contract geometry does not match the selected config');
  if (frameApproval === undefined)
    throw new Error('a signed frame approval is required for animation contract validation');
  ...
  const verifiedApproval = await verifyFrameApproval({
    ...
    snapReceipt: frameApproval.snapReceipt,
    version: frameApproval.version
  });
  const failures = [];
  const warnings = [];
  ...
  try {
    exactKeys(exported, ['clips', 'metadata'], 'contract export selection');
  } catch (error) {
    contractExportFailure(failures, 'METADATA_MISMATCH', { field: 'selectionSchema', reason: error.message });
  }
  if (!Array.isArray(normalized.frames) || !Array.isArray(normalized.measurements))
    throw new Error('contract clip validation requires normalized frames and measurements');
  if (
    !exported.clips ||
    typeof exported.clips !== 'object' ||
    Array.isArray(exported.clips) ||
    typeof exported.metadata !== 'string'
  )
    throw new Error('contract clip validation requires clips and a contract index');
  ...
  try {
    ...
    if (capturedAnchor.sha256 !== animationContract.document.anchor.sha256)
      contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
        field: 'contractAnchor',
        expected: animationContract.document.anchor.sha256,
        actual: capturedAnchor.sha256
      });
    if (anchorReport.sha256 && anchorReport.sha256 !== capturedAnchor.sha256)
      contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
        field: 'anchorReport',
        expected: anchorReport.sha256,
        actual: capturedAnchor.sha256
      });
  ...
  try {
    ...
    if (!indexStat.isFile() || indexStat.isSymbolicLink() || indexStat.nlink > 1)
      throw new Error('contract index must be a regular single-link file');
    root = await fs.realpath(path.dirname(exported.metadata));
    if (path.dirname(await fs.realpath(exported.metadata)) !== root)
      throw new Error('contract index escaped its export root');
  ...
  if (index.version !== 1)
    contractExportFailure(failures, 'METADATA_MISMATCH', { field: 'version', expected: 1, actual: index.version });
  if (path.basename(exported.metadata) !== 'animation-contract-export.json')
    contractExportFailure(failures, 'METADATA_MISMATCH', {
      field: 'indexName',
      expected: 'animation-contract-export.json',
      actual: path.basename(exported.metadata)
    });
  if (
    index.animationContractSha256 !== animationContract.sha256 ||
    !jsonEqual(index.animationContract, animationContract.document) ||
    stableHash(index.animationContract) !== index.animationContractSha256
  )
    contractExportFailure(failures, 'METADATA_MISMATCH', { field: 'animationContract' });
  if (index.frameApprovalSha256 !== verifiedApproval.sha256)
    contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
      field: 'frameApprovalSha256',
      expected: verifiedApproval.sha256,
      actual: index.frameApprovalSha256
    });
  if (
    !jsonEqual(index.palette, animationContract.document.palette) ||
    stableHash(index.palette?.rgba) !== animationContract.document.palette.sha256 ||
    !jsonEqual(index.palette?.snapperPaletteHex, animationContract.document.palette.snapperPaletteHex)
  )
    contractExportFailure(failures, 'PALETTE_DRIFT', { field: 'frozenPalette' });
  ...
  if (!jsonEqual(declaredClipIds, expectedClipIds))
    contractExportFailure(failures, 'FRAME_COUNT', {
      field: 'callerClipSet',
      expected: expectedClipIds,
      actual: declaredClipIds
    });
  if (
    !Array.isArray(index.clips) ||
    index.clips.length !== expectedClipIds.length ||
    !jsonEqual(
      index.clips?.map((clip) => clip?.id),
      expectedClipIds
    )
  )
    contractExportFailure(failures, 'FRAME_COUNT', {
      field: 'indexClipSet',
      expected: expectedClipIds,
      actual: index.clips?.map((clip) => clip?.id) ?? null
    });
  if (
    normalized.frames.length !== contractFrames.length ||
    normalized.measurements.length !== contractFrames.length ||
    !Array.isArray(index.measurements) ||
    index.measurements.length !== contractFrames.length
  )
    contractExportFailure(failures, 'FRAME_COUNT', {
      field: 'normalizedFrameSet',
      expected: contractFrames.length,
      actual: {
        frames: normalized.frames.length,
        measurements: normalized.measurements.length,
        indexMeasurements: index.measurements?.length ?? null
      }
    });
  ...
  for (let frameIndex = 0; frameIndex < contractFrames.length; frameIndex += 1) {
    ...
    try {
      ...
      if (drift.length > 0)
        contractExportFailure(failures, 'PALETTE_DRIFT', {
          field: 'normalizedPixels',
          frame: frameIndex,
          colors: drift
        });
    } catch (error) {
      contractExportFailure(failures, 'FRAME_COUNT', {
        field: 'normalizedArtifact',
        frame: frameIndex,
    ...
    if (!approvedSource || typeof recorded?.input !== 'string' || path.resolve(recorded.input) !== approvedSource)
      contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
        field: 'approvedSource',
        frame: frameIndex,
        expected: approvedSource,
        actual: recorded?.input ?? null
      });
    if (approvedSource) {
      try {
        ...
        if (capturedSource.sha256 !== approved.sha256)
          contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
            field: 'approvedSource',
            frame: frameIndex,
            expected: approved.sha256,
            actual: capturedSource.sha256
          });
      } catch (error) {
        contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
          field: 'approvedSource',
    ...
    if (recorded?.frameId !== definition.id || approved?.id !== definition.id)
      contractExportFailure(failures, 'METADATA_MISMATCH', {
        field: 'orderedFrame',
        frame: frameIndex,
        expected: definition.id,
        actual: recorded?.frameId ?? null
      });
    if (!sameCoordinate(recorded?.sourceLandmark, approved?.landmark))
      contractExportFailure(failures, 'LANDMARK_DRIFT', {
        field: 'sourceLandmark',
        frame: frameIndex,
        expected: approved?.landmark ?? null,
        actual: recorded?.sourceLandmark ?? null
      });
    let derivedRoot = null;
    let derivedDrift = null;
    if (captured && capturedSource && approved) {
      const normalizedBounds = (
        await retainedForeground(normalized.frames[frameIndex], captured.image, config, { configuredBackground: false })
      ).bounds;
      ...
      const landmarkContained =
        approved.landmark.x < capturedSource.image.width && approved.landmark.y < capturedSource.image.height;
      ...
      if (!landmarkContained || !Number.isInteger(scaleX) || scaleX < 1 || scaleX !== scaleY) {
        contractExportFailure(failures, 'GLOBAL_SCALE_DRIFT', {
          field: 'approvedSourceGeometry',
          frame: frameIndex,
          sourceBounds,
          normalizedBounds,
        ...
        derivedDrift = {
          x: derivedRoot.x - definition.landmarkSemantic.target.x,
          y: derivedRoot.y - definition.landmarkSemantic.target.y
        };
        ...
        if (
          !normalizedMatchesApprovedSource(recoveredSource.image, captured.image, scaleX, placementLeft, placementTop)
        )
          contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
            field: 'approvedSource',
            frame: frameIndex,
            reason: 'normalized pixels do not derive from the signed approved source'
          });
      }
    }
    if (
      !sameCoordinate(derivedRoot, definition.landmarkSemantic.target) ||
      !sameCoordinate(derivedDrift, { x: 0, y: 0 }) ||
      !sameCoordinate(recorded?.canonicalLandmark, derivedRoot) ||
      !sameCoordinate(recorded?.landmarkDrift, derivedDrift)
    )
      contractExportFailure(failures, 'LANDMARK_DRIFT', {
        field: 'canonicalLandmark',
        frame: frameIndex,
        expected: definition.landmarkSemantic.target,
        actual: derivedRoot,
        recorded: recorded?.canonicalLandmark ?? null
      });
    const indexedMeasurement = index.measurements?.[frameIndex];
    if (
      !sameCoordinate(indexedMeasurement?.canonicalLandmark, definition.landmarkSemantic.target) ||
      !sameCoordinate(indexedMeasurement?.sourceLandmark, approved?.landmark) ||
      !sameCoordinate(indexedMeasurement?.landmarkDrift, { x: 0, y: 0 })
    )
      contractExportFailure(failures, 'LANDMARK_DRIFT', { field: 'indexedLandmark', frame: frameIndex });
    if (!jsonEqual(indexedMeasurement, expectedMeasurementRecord(recorded, captured?.sha256 ?? null)))
      contractExportFailure(failures, 'METADATA_MISMATCH', { field: 'landmarkMeasurements', frame: frameIndex });
  }
  ...
  for (let clipIndex = 0; clipIndex < animationContract.document.clips.length; clipIndex += 1) {
    ...
    for (const relative of [
      ...canonicalArtifacts.frames,
      canonicalArtifacts.sheet,
      canonicalArtifacts.metadata,
      canonicalArtifacts.preview
    ])
      expectedEntries.add(`file:${relative}`);
    ...
    measurements.contractClips.push({
      id: definition.id,
      frames: expectedIds,
      durations: expectedDurations,
      loopMode: definition.loopMode
    });
    ...
    if (!jsonEqual(actualDurations, expectedDurations))
      contractExportFailure(failures, 'TIMING_MISMATCH', {
        field: 'durations',
        clipId: definition.id,
        expected: expectedDurations,
        actual: actualDurations
      });
    if (record.loopMode !== definition.loopMode)
      contractExportFailure(failures, 'METADATA_MISMATCH', {
        field: 'loopMode',
        clipId: definition.id,
        expected: definition.loopMode,
        actual: record.loopMode
      });
    const recordedArtifacts = {
      frames: record.frames.map((frame) => frame.file),
      sheet: record.sheet,
      metadata: record.metadata,
      preview: record.preview
    };
    if (!jsonEqual(recordedArtifacts, canonicalArtifacts))
      contractExportFailure(failures, 'METADATA_MISMATCH', {
        field: 'canonicalArtifacts',
        clipId: definition.id,
        expected: canonicalArtifacts,
        actual: recordedArtifacts
      });
    ...
    for (const relative of relativeArtifacts) {
      try {
        resolved.push(await containedArtifact(root, relative));
      } catch (error) {
        contractExportFailure(failures, 'FRAME_COUNT', {
          field: 'artifactSet',
          clipId: definition.id,
    ...
    if (resolved.length === relativeArtifacts.length) {
      ...
      const flat = {
        runtimeFrames: frameFiles,
        sheet: resolved.at(-3),
        metadata: resolved.at(-2),
        preview: resolved.at(-1)
      };
      let callerSchema = true;
      try {
        exactKeys(
          caller,
          ['runtimeFrames', 'sheet', 'metadata', 'preview', 'frames', 'durations', 'loopMode'],
          'contract export selected clip'
        );
        if (!Array.isArray(caller.frames) || !Array.isArray(caller.runtimeFrames) || !Array.isArray(caller.durations))
          throw new Error('contract export selected clip arrays are invalid');
        ...
        if (
          [
            ...
            caller.metadata,
            caller.preview,
            ...caller.runtimeFrames,
            ...caller.frames.map((frame) => frame.file)
          ].some((file) => typeof file !== 'string' || file === '')
        )
          throw new Error('contract export selected artifact paths are invalid');
      } catch (error) {
        callerSchema = false;
        contractExportFailure(failures, 'METADATA_MISMATCH', {
          field: 'callerSchema',
        ...
        try {
          const callerArtifacts = await Promise.all(
            [
              ...
              caller.metadata,
              caller.preview
            ].map((file) => canonicalPath(file))
          );
          const expectedArtifacts = await Promise.all(
            [...frameFiles, ...frameFiles, flat.sheet, flat.metadata, flat.preview].map((file) => canonicalPath(file))
          );
          if (
            !jsonEqual(callerIds, expectedIds) ||
            !jsonEqual(caller.durations, expectedDurations) ||
            caller.loopMode !== definition.loopMode ||
            !jsonEqual(callerArtifacts, expectedArtifacts)
          )
            contractExportFailure(failures, 'METADATA_MISMATCH', { field: 'callerArtifacts', clipId: definition.id });
        } catch (error) {
          contractExportFailure(failures, 'METADATA_MISMATCH', {
            field: 'callerArtifacts',
            clipId: definition.id,
            reason: error.message
          });
        }
      ...
      try {
        ...
        const actualMetadataNames = {
          frames: clipMetadata.frames?.map((frame) => frame.file) ?? null,
          sheet: clipMetadata.sheet,
          preview: clipMetadata.preview
        };
        if (!jsonEqual(actualMetadataNames, canonicalMetadataNames))
          contractExportFailure(failures, 'METADATA_MISMATCH', {
            field: 'canonicalMetadataArtifacts',
            clipId: definition.id,
            expected: canonicalMetadataNames,
            actual: actualMetadataNames
          });
        if (
          !jsonEqual(clipMetadata.durations, expectedDurations) ||
          !jsonEqual(
            clipMetadata.frames?.map((frame) => frame.duration),
            expectedDurations
          )
        )
          contractExportFailure(failures, 'TIMING_MISMATCH', {
            field: 'clipMetadataDurations',
            clipId: definition.id,
            expected: expectedDurations,
            actual: {
              durations: clipMetadata.durations,
              frames: clipMetadata.frames?.map((frame) => frame.duration) ?? null
            }
          });
        const previewDelays = capturedPreview.decoded.metadata.delay ?? [];
        if (definition.frames.length > 1 && !jsonEqual(previewDelays, expectedDurations))
          contractExportFailure(failures, 'TIMING_MISMATCH', {
            field: 'previewDelays',
            clipId: definition.id,
            expected: expectedDurations,
            actual: previewDelays
          });
        validationJobs.push({
          clipId: definition.id,
  ...
  try {
    ...
    if (!jsonEqual(actualEntries, expected))
      contractExportFailure(failures, 'FRAME_COUNT', { field: 'artifactSet', expected, actual: actualEntries });
  } catch (error) {
  ...
  for (const job of validationJobs) {
    try {
      ...
      failures.push(
        ...clipReport.failures.map(({ correction: _correction, ...failure }) => ({ ...failure, clipId: job.clipId }))
      );
    } catch (error) {
      contractExportFailure(failures, 'FRAME_COUNT', {
        field: 'artifactValidation',
        clipId: job.clipId,
        reason: error.message
      });
    }
  ...
  for (const code of SEMANTIC_CODES) {
    ...
    if (validSemanticEvidence(evidence))
      addFailure(failures, code, { frame: evidence.frame, evidence: evidence.evidence });
    else
      warnings.push({
        code: 'HUMAN_REVIEW_REQUIRED',
        check: code,
        reason: 'artistic or semantic judgment requires explicit evidence'
      });
  }
  for (const clip of animationContract.document.clips) {
    if (clip.loopMode === 'loop' && !loopApprovedByFrameManifest(verifiedApproval, clip))
      warnings.push({
        code: 'HUMAN_REVIEW_REQUIRED',
        check: 'LOOP_ROOT_TRANSITION',
        clipId: clip.id,
        transition: clip.loopTransition,
        reason:
          'the declared last-to-first root transition requires an authenticated frame approval at its review checkpoint'
      });
  }
...
function loopApprovedByFrameManifest(verifiedApproval, clip) {
  ...
  return [from, to].every(
    (frame) =>
      frame?.approved === true &&
      frame.approvedBy === verifiedApproval.document.payload.approvedBy &&
      frame.checkpoints.includes(clip.loopTransition.reviewCheckpoint)
  );
}

function sameCoordinate(left, right) {
  return validCoordinate(left) && validCoordinate(right) && left.x === right.x && left.y === right.y;
}
...
function nonzeroTravel(value) {
  return value?.x !== 0 || value?.y !== 0;
}

async function compositedFilesMatch(files, expected, width, height) {
  if (!Array.isArray(files) || files.length === 0 || expected.width !== width || expected.height !== height)
    return false;
...
async function validateContractExportRunV2({
  anchorReport,
  normalized,
  exported,
  config,
  ...
  config = structuredClone(config);
  animationContract = snapshotAnimationContract(animationContract);
  if (!animationContract || animationContract.document.version !== 2)
    throw new Error('v2 contract validation requires an animation contract version 2');
  const contract = animationContract.document;
  const canonical = { width: contract.canvas.width, height: contract.canvas.height };
  if (
    !jsonEqual(config?.canonical, canonical) ||
    !jsonEqual(config?.runtime, contract.scale.runtime) ||
    !jsonEqual(config?.pivot, contract.canvas.pivot)
  )
    throw new Error('validation animation contract geometry does not match the selected config');
  if (frameApproval === undefined)
    throw new Error('a signed frame approval is required for animation contract validation');
  ...
  try {
    ...
    measurements.anchorHash = capturedAnchor.sha256;
    if (
      capturedAnchor.sha256 !== contract.character.anchorSha256 ||
      (anchorReport.sha256 && anchorReport.sha256 !== capturedAnchor.sha256)
    )
      contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
        field: 'characterAnchor',
        expected: contract.character.anchorSha256,
        actual: capturedAnchor.sha256
      });
    const drift = colorsOutsidePalette(capturedAnchor.image, allowedPalette);
    if (drift.length > 0) contractExportFailure(failures, 'PALETTE_DRIFT', { field: 'anchorPixels', colors: drift });
  ...
  if (
    ...
    normalized.animationContractSha256 !== animationContract.sha256 ||
    normalized.selectionApprovalSha256 !== contract.selectionApprovalSha256 ||
    normalized.frameApprovalSha256 !== verifiedApproval.sha256 ||
    normalized.snapReceiptSha256 !== verifiedApproval.document.payload.snapReceiptSha256 ||
    !Array.isArray(normalized.frames)
  )
    throw new Error('v2 validation normalized provenance binding is invalid');
  if (exported?.version !== 2 || !exported.clips || !exported.tracks || typeof exported.metadata !== 'string')
    throw new Error('v2 validation requires the complete contract export selection');
  const definitions = contract.clips.flatMap((clip) =>
    clip.frames.map((frame) => ({ ...frame, loopMode: clip.loopMode, clipId: clip.id }))
  );
  if (
    normalized.frames.length !== definitions.length ||
    verifiedApproval.document.payload.frames.length !== definitions.length
  )
    throw new Error('v2 validation requires exact normalized and approved frame coverage');
  ...
  for (const [frameIndex, definition] of definitions.entries()) {
    ...
    const approved = verifiedApproval.document.payload.frames[frameIndex];
    const delta = {
      x: contract.canvas.pivot.x - approved.landmarks.root.x,
      y: contract.canvas.pivot.y - approved.landmarks.root.y
    };
    ...
    if (
      ...
      frame?.semantic !== definition.semantic ||
      frame?.duration !== definition.duration ||
      frame?.loopMode !== definition.loopMode ||
      frame?.scale !== contract.scale.integer
    )
      contractExportFailure(failures, 'METADATA_MISMATCH', { field: 'normalizedFrame', frame: frameIndex });
    if (!sameCoordinate(frame?.root, contract.canvas.pivot))
      contractExportFailure(failures, 'LANDMARK_DRIFT', {
        field: 'root',
        frame: frameIndex,
        expected: contract.canvas.pivot,
        actual: frame?.root ?? null
      });
    if (
      frame?.baseline !== contract.canvas.baseline ||
      approved.landmarks.baseline + delta.y !== contract.canvas.baseline
    )
      contractExportFailure(failures, 'BASELINE_DRIFT', {
        frame: frameIndex,
        expected: contract.canvas.baseline,
        actual: frame?.baseline ?? null
      });
    if (!jsonEqual(frame?.sockets, expectedSockets))
      contractExportFailure(failures, 'LANDMARK_DRIFT', {
        field: 'sockets',
        frame: frameIndex,
        expected: expectedSockets,
        actual: frame?.sockets ?? null
      });
    if (!jsonEqual(frame?.contacts, expectedContacts))
      contractExportFailure(failures, 'LANDMARK_DRIFT', {
        field: 'contacts',
        frame: frameIndex,
        expected: expectedContacts,
        actual: frame?.contacts ?? null
      });
    if (!jsonEqual(frame?.groundTravel, definition.groundTravel))
      contractExportFailure(failures, 'METADATA_MISMATCH', { field: 'groundTravel', frame: frameIndex });
    if (nonzeroTravel(definition.groundTravel) && definition.contacts.length === 0)
      contractExportFailure(failures, 'GROUND_TRAVEL_CONTACT', {
        frame: frameIndex,
        frameId: definition.id,
        groundTravel: definition.groundTravel
      });
    if (!jsonEqual(Object.keys(frame?.tracks ?? {}), definition.tracks))
      contractExportFailure(failures, 'FRAME_COUNT', {
        field: 'normalizedTracks',
        frame: frameIndex,
        expected: definition.tracks,
        actual: Object.keys(frame?.tracks ?? {})
      });
    ...
    for (const [trackIndex, trackId] of definition.tracks.entries()) {
      ...
      if (
        !record ||
        !approvedOutput ||
        approvedOutput.trackId !== trackId ||
        record.sourceSha256 !== approvedOutput.sha256 ||
        record.kind !== track.kind ||
        record.attachTo !== track.attachTo
      ) {
        contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', { field: 'trackBinding', frame: frameIndex, trackId });
        continue;
      }
      if (track.attachTo !== null && !definition.sockets.includes(track.attachTo))
        contractExportFailure(failures, 'SOCKET_ATTACHMENT', { frame: frameIndex, trackId, socket: track.attachTo });
      const expectedSource = path.resolve(path.dirname(frameApproval.snapReceipt.path), approvedOutput.path);
      if (!(await sourcePathsMatch(record.sourcePath, expectedSource)))
        contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
          field: 'trackSourcePath',
          frame: frameIndex,
          trackId
        });
      try {
        ...
        if (source.sha256 !== approvedOutput.sha256 || output.sha256 !== record.normalizedSha256)
          contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', { field: 'trackPixels', frame: frameIndex, trackId });
        if (output.image.width !== canonical.width || output.image.height !== canonical.height)
          contractExportFailure(failures, 'CANVAS_SIZE', {
            frame: frameIndex,
            trackId,
            expected: [canonical.width, canonical.height],
            actual: [output.image.width, output.image.height]
          });
        const drift = colorsOutsidePalette(output.image, allowedPalette);
        if (drift.length > 0)
          contractExportFailure(failures, 'PALETTE_DRIFT', { frame: frameIndex, trackId, colors: drift });
        if (!translatedImageMatches(source.image, output.image, delta))
          contractExportFailure(failures, track.attachTo === null ? 'LANDMARK_DRIFT' : 'SOCKET_ATTACHMENT', {
            frame: frameIndex,
            trackId,
            delta
          });
        if (track.required && record.clippedPixelCount !== 0)
          contractExportFailure(failures, 'CLIPPED_FOREGROUND', {
            frame: frameIndex,
            trackId,
            clippedPixelCount: record.clippedPixelCount ?? null
          });
      } catch (error) {
        contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
          field: 'trackArtifact',
    ...
    try {
      ...
      if (
        combined.sha256 !== frame.combined.sha256 ||
        !(await compositedFilesMatch(normalizedTrackFiles, combined.image, canonical.width, canonical.height))
      )
        contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', { field: 'normalizedCombined', frame: frameIndex });
    } catch (error) {
      contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
        field: 'normalizedCombined',
        frame: frameIndex,
        reason: error.message
      });
    }
  ...
  if (
    index.version !== 2 ||
    index.animationContractSha256 !== animationContract.sha256 ||
    !jsonEqual(index.animationContract, contract) ||
    index.selectionApprovalSha256 !== contract.selectionApprovalSha256 ||
    index.frameApprovalSha256 !== verifiedApproval.sha256 ||
    index.snapReceiptSha256 !== verifiedApproval.document.payload.snapReceiptSha256
  )
    contractExportFailure(failures, 'METADATA_MISMATCH', { field: 'provenanceBindings' });
  for (const field of ['character', 'canvas', 'scale', 'palette', 'tracks', 'sockets', 'contacts'])
    if (!jsonEqual(index[field], contract[field]))
      contractExportFailure(failures, field === 'palette' ? 'PALETTE_DRIFT' : 'METADATA_MISMATCH', { field });
  if (!Array.isArray(index.clips) || index.clips.length !== contract.clips.length)
    contractExportFailure(failures, 'FRAME_COUNT', {
      field: 'clips',
      expected: contract.clips.length,
      actual: index.clips?.length ?? null
    });
  ...
  for (const [clipIndex, clip] of contract.clips.entries()) {
    ...
    try {
      exactKeys(
        record,
        ['id', 'loopMode', 'restart', 'frames', 'sheet', 'contactSheet', 'metadata', 'preview'],
        'v2 export clip'
      );
    } catch (error) {
      contractExportFailure(failures, 'METADATA_MISMATCH', {
    ...
    if (record.id !== clip.id || record.loopMode !== clip.loopMode)
      contractExportFailure(failures, 'METADATA_MISMATCH', { field: 'loopMode', clipId: clip.id });
    const expectedRestart = clip.loopMode === 'loop' ? 'loop' : 'stop';
    if (record.restart !== expectedRestart)
      contractExportFailure(failures, clip.loopMode === 'loop' ? 'METADATA_MISMATCH' : 'NONCYCLIC_RESTART', {
        clipId: clip.id,
        expected: expectedRestart,
        actual: record.restart
      });
    if (!Array.isArray(record.frames) || record.frames.length !== clip.frames.length)
      contractExportFailure(failures, 'FRAME_COUNT', { field: 'clipFrames', clipId: clip.id });
    ...
    for (const [localIndex, definition] of clip.frames.entries()) {
      ...
      if (
        frameRecord.id !== definition.id ||
        frameRecord.semantic !== definition.semantic ||
        frameRecord.duration !== definition.duration ||
        !jsonEqual(frameRecord.tracks, definition.tracks) ||
        !sameCoordinate(frameRecord.root, contract.canvas.pivot) ||
        frameRecord.baseline !== contract.canvas.baseline ||
        !jsonEqual(frameRecord.sockets, normalizedFrame.sockets) ||
        !jsonEqual(frameRecord.contacts, normalizedFrame.contacts) ||
      ...
      if (nonzeroTravel(frameRecord.groundTravel) && Object.keys(frameRecord.contacts ?? {}).length === 0)
        contractExportFailure(failures, 'GROUND_TRAVEL_CONTACT', {
          frameId: definition.id,
          groundTravel: frameRecord.groundTravel
        });
      if (!Array.isArray(frameRecord.outputs) || frameRecord.outputs.length !== definition.tracks.length)
        contractExportFailure(failures, 'FRAME_COUNT', { field: 'trackOutputs', frameId: definition.id });
      ...
      for (const [trackIndex, outputRecord] of (frameRecord.outputs ?? []).entries()) {
        ...
        try {
          exactKeys(
            outputRecord,
            ['trackId', 'kind', 'attachTo', 'sourceSha256', 'normalizedSha256', 'file', 'sha256'],
            'v2 track output'
          );
          expectV2Artifact(expectedEntries, outputRecord.file);
          const artifact = await validateV2ArtifactRecord(
            root,
            { file: outputRecord.file, sha256: outputRecord.sha256 },
            'v2 runtime track artifact'
          );
          ...
          if (
            !artifact.matches ||
            outputRecord.trackId !== trackId ||
            outputRecord.sourceSha256 !== normalizedFrame.tracks[trackId].sourceSha256 ||
            outputRecord.normalizedSha256 !== normalizedFrame.tracks[trackId].normalizedSha256
          )
            contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', {
              field: 'runtimeTrack',
              frameId: definition.id,
          ...
          if (!normalizedImage || !blockMatches(normalizedImage, image, contract.scale.integer))
            contractExportFailure(failures, 'NON_INTEGER_SCALE', { frameId: definition.id, trackId });
        } catch (error) {
          contractExportFailure(failures, 'FRAME_COUNT', {
            field: 'runtimeTrack',
            frameId: definition.id,
      ...
      try {
        ...
        if (
          !combined.matches ||
          image.width !== contract.scale.runtime.width ||
          image.height !== contract.scale.runtime.height ||
          !(await compositedFilesMatch(
            runtimeTrackFiles,
            image,
            contract.scale.runtime.width,
            contract.scale.runtime.height
          ))
        )
          contractExportFailure(failures, 'SOURCE_HASH_MISMATCH', { field: 'combined', frameId: definition.id });
      } catch (error) {
        contractExportFailure(failures, 'FRAME_COUNT', {
          field: 'combined',
          frameId: definition.id,
    ...
    try {
      ...
      if (
        !artifact.matches ||
        !jsonEqual(
          clipMetadata.durations,
          clip.frames.map((frame) => frame.duration)
        ) ||
        clipMetadata.frames?.length !== clip.frames.length ||
        clipMetadata.frameSize?.width !== contract.scale.runtime.width ||
        clipMetadata.frameSize?.height !== contract.scale.runtime.height
      )
        contractExportFailure(failures, 'METADATA_MISMATCH', { field: 'clipMetadata', clipId: clip.id });
    } catch (error) {
    ...
    for (const field of ['sheet', 'contactSheet']) {
      try {
        ...
        const matches =
          clipMetadata &&
          sheetMatches(image, runtimeCombinedImages, clipMetadata.columns, clipMetadata.rows, contract.scale.runtime);
    ...
    try {
      ...
      for (let index = 0; pixelsMatch && index < pages; index += 1)
        pixelsMatch = preview.decoded.data
          .subarray(index * pageBytes, (index + 1) * pageBytes)
          .equals(runtimeCombinedImages[index].data);
      if (
        !artifact.matches ||
        (metadata.pages ?? 1) !== clip.frames.length ||
        metadata.width !== contract.scale.runtime.width ||
        (metadata.pageHeight ?? metadata.height) !== contract.scale.runtime.height ||
        ((metadata.pages ?? 1) > 1 &&
          !jsonEqual(
            metadata.delay,
            clip.frames.map((frame) => frame.duration)
        ...
        contractExportFailure(failures, 'PREVIEW_MISMATCH', { field: 'timingGeometryOrPixels', clipId: clip.id });
    } catch (error) {
  ...
  try {
    const actualEntries = await recursiveEntries(root);
    if (!jsonEqual(actualEntries, [...expectedEntries].sort()))
      contractExportFailure(failures, 'FRAME_COUNT', {
        field: 'artifactSet',
        expected: [...expectedEntries].sort(),
...
function validSourceLandmark(value) {
  return validCoordinate(value) && value.x >= 0 && value.y >= 0;
}

function validCanonicalLandmark(value, canonical) {
  return validSourceLandmark(value) && value.x < canonical.width && value.y < canonical.height;
}

async function validateFlatRun(
  { anchorReport, normalized, exported, config, semanticEvidence = [], animationContract, frameApproval },
  capturedArtifacts = null
) {
  if (!anchorReport || !normalized || !exported || !config)
    throw new Error('anchorReport, normalized, exported, and config are required');
  anchorReport = structuredClone(anchorReport);
  normalized = structuredClone(normalized);
  exported = structuredClone(exported);
  config = structuredClone(config);
  ...
  if (
    ...
    throw new Error('validation animation contract geometry does not match the selected config');
  if (animationContract && frameApproval === undefined)
    throw new Error('a signed frame approval is required for animation contract validation');
  frameApproval = snapshotFrameApproval(frameApproval, animationContract);
  const verifiedFrameApproval = frameApproval
    ? await verifyFrameApproval({
        projectDir: frameApproval.projectDir,
        file: frameApproval.file,
        contract: animationContract,
        snapReceipt: frameApproval.snapReceipt,
  ...
  if (
    contractedFrames &&
    (!Array.isArray(normalized.measurements) || normalized.measurements.length !== normalized.frames?.length)
  )
    throw new Error('animation contract validation requires an exact normalization measurements array for every frame');
  const failures = [];
  const warnings = [];
  ...
  if (!Array.isArray(normalized.frames) || !Array.isArray(exported.runtimeFrames))
    throw new Error('normalized and exported frame lists are required');
  if (normalized.frames.length !== exported.runtimeFrames.length)
    addFailure(failures, 'FRAME_COUNT', {
      stage: 'runtime',
      trustedArtifact: false,
      expected: normalized.frames.length,
      actual: exported.runtimeFrames.length
    });
  if (contractedFrames && normalized.frames.length !== contractedFrames.length)
    addFailure(failures, 'FRAME_COUNT', {
      stage: 'normalization-contract',
      trustedArtifact: false,
      expected: contractedFrames.length,
      actual: normalized.frames.length
    });
  if (normalized.canonicalPivot?.x !== config.pivot.x || normalized.canonicalPivot?.y !== config.pivot.y)
    addFailure(failures, 'PIVOT_DRIFT', { expected: config.pivot, actual: normalized.canonicalPivot });

  if (typeof anchorReport.path !== 'string' || anchorReport.path.trim() === '')
    throw new Error('anchorReport.path is required for artifact-backed validation');
  ...
  if (anchorReport.sha256 && anchorReport.sha256 !== actualAnchorHash)
    addFailure(failures, 'SOURCE_HASH_MISMATCH', {
      stage: 'anchor',
      trustedArtifact: false,
      artifact: 'anchor',
      expected: anchorReport.sha256,
      actual: actualAnchorHash
    });
  ...
  for (let index = 0; index < normalized.frames.length; index += 1) {
    ...
    if (image.width !== canonical.width || image.height !== canonical.height)
      addFailure(failures, 'CANVAS_SIZE', {
        stage: 'canonical',
        frame: index,
        expected: [canonical.width, canonical.height],
    ...
    if (bounds && bounds.bottom !== config.pivot.y - 1)
      addFailure(failures, 'BASELINE_DRIFT', { frame: index, expected: config.pivot.y - 1, actual: bounds.bottom });
    if (
      bounds &&
      (bounds.left < margin ||
        bounds.top < margin ||
        bounds.right >= image.width - margin ||
        bounds.bottom >= image.height - margin)
    )
      addFailure(failures, 'CLIPPED_FOREGROUND', { stage: 'canonical', frame: index, margin, bounds });
    if (
      recorded &&
      bounds &&
      (recorded.left !== bounds.left ||
        recorded.top !== bounds.top ||
        recorded.width !== bounds.width ||
        recorded.height !== bounds.height)
    )
      addFailure(failures, 'METADATA_MISMATCH', { stage: 'normalization', frame: index, recorded, actual: bounds });
    ...
    if (approved) {
      ...
      if (typeof recorded?.input !== 'string' || path.resolve(recorded.input) !== approvedSource)
        addFailure(failures, 'METADATA_MISMATCH', {
          stage: 'normalization-landmark',
          frame: index,
    ...
    if (!sourceArtifact) {
      addFailure(failures, 'GLOBAL_SCALE_DRIFT', {
        stage: 'normalization-provenance',
        frame: index,
      ...
      if (!source) {
        addFailure(failures, 'GLOBAL_SCALE_DRIFT', {
          stage: 'normalization-provenance',
          frame: index,
          reason: 'approved source byte capture did not match its signed hash'
        });
      } else {
    ...
    if (definition) {
      const complete =
        recorded &&
        typeof recorded.frameId === 'string' &&
        validSourceLandmark(recorded.sourceLandmark) &&
        validCanonicalLandmark(recorded.canonicalLandmark, canonical) &&
        validCoordinate(recorded.landmarkDrift);
      if (!complete || recorded.frameId !== definition.id)
        addFailure(failures, 'METADATA_MISMATCH', {
          stage: 'normalization-landmark',
          frame: index,
          field: 'ordered-frame',
      ...
      if (approved) {
        if (!complete || approved.id !== definition.id || !sameCoordinate(recorded.sourceLandmark, approved.landmark))
          addFailure(failures, 'METADATA_MISMATCH', {
            stage: 'normalization-landmark',
            frame: index,
            field: 'sourceLandmark',
            expected: approved.landmark,
            actual: recorded?.sourceLandmark ?? null
          });
        const sourceContained =
          sourceDimensions &&
          approved.landmark.x < sourceDimensions.width &&
          approved.landmark.y < sourceDimensions.height;
        if (!sourceContained)
          addFailure(failures, 'METADATA_MISMATCH', {
            stage: 'normalization-landmark',
            frame: index,
            field: 'sourceLandmarkContainment',
            landmark: approved.landmark,
            sourceDimensions
          });
        if (sourceScale && sourceContained) {
          const canonicalLandmark = {
            x: bounds.left + (approved.landmark.x - sourceBounds.left) * sourceScale,
            y: bounds.top + (approved.landmark.y - sourceBounds.top) * sourceScale
          };
          ...
          if (actual.x !== 0 || actual.y !== 0)
            addFailure(failures, 'LANDMARK_DRIFT', {
              stage: 'canonical',
              frame: index,
              frameId: definition.id,
              expected: { x: 0, y: 0 },
          ...
          if (
            !complete ||
            !sameCoordinate(recorded.canonicalLandmark, canonicalLandmark) ||
            !sameCoordinate(recorded.landmarkDrift, actual)
          )
            addFailure(failures, 'METADATA_MISMATCH', {
              stage: 'normalization-landmark',
              frame: index,
              field: 'derived-landmark',
              canonicalLandmark,
              landmarkDrift: actual
            });
        }
      }
    }
  ...
  if (scales.size > 1)
    addFailure(failures, 'GLOBAL_SCALE_DRIFT', { stage: 'normalization-artifacts', scales: [...scales] });
  ...
  for (let index = 0; index < exported.runtimeFrames.length; index += 1) {
    ...
    if (image.width !== runtime.width || image.height !== runtime.height)
      addFailure(failures, 'CANVAS_SIZE', {
        stage: 'runtime',
        frame: index,
        expected: [runtime.width, runtime.height],
        actual: [image.width, image.height]
      });
    failures.push(
      ...validateIntegerScale({ source: canonical, output: { width: image.width, height: image.height } }).map(
        (failure) => ({ ...failure, frame: index })
      )
    );
    const bounds = alphaBounds(image);
    if (runtimeScale && bounds && bounds.bottom !== config.pivot.y * runtimeScale - 1)
      addFailure(failures, 'BASELINE_DRIFT', {
        stage: 'runtime',
        frame: index,
        expected: config.pivot.y * runtimeScale - 1,
        actual: bounds.bottom
      });
    if (
      bounds &&
      (bounds.left < margin * (runtimeScale ?? 1) ||
        bounds.top < margin * (runtimeScale ?? 1) ||
        bounds.right >= image.width - margin * (runtimeScale ?? 1) ||
        bounds.bottom >= image.height - margin * (runtimeScale ?? 1))
    )
      addFailure(failures, 'CLIPPED_FOREGROUND', {
        stage: 'runtime',
        frame: index,
        margin: margin * (runtimeScale ?? 1),
        bounds
      });
    const background = backgroundEvidence(image, config);
    if (background.opaqueBorderPixels > 0 || background.configuredColorPixels > 0)
      addFailure(failures, 'BACKGROUND_REMAINS', { frame: index, ...background });
    ...
    if (
      ...
      (runtimeScale && normalizedImages[index] && !blockMatches(normalizedImages[index], image, runtimeScale))
    )
      addFailure(failures, 'INTERMEDIATE_COLORS', { frame: index, colors: intermediate, nearestNeighborBlocks: false });
    const paletteDrift = [...runtimeColors].filter((key) => !anchorColors.has(key));
    if (paletteDrift.length > 0)
      addFailure(failures, 'PALETTE_DRIFT', { stage: 'runtime', frame: index, colors: paletteDrift });
  }
  ...
  if (
    ...
    metadata.frames.length !== exported.runtimeFrames.length ||
    !Array.isArray(metadata.sources) ||
    metadata.sources.length !== normalized.frames.length
  )
    addFailure(failures, 'FRAME_COUNT', {
      stage: 'metadata',
      trustedArtifact: normalized.frames.length === exported.runtimeFrames.length,
      expected: normalized.frames.length,
      actual: metadata.frames?.length ?? null
    });
  if (metadata.canonicalPivot?.x !== config.pivot.x || metadata.canonicalPivot?.y !== config.pivot.y)
    addFailure(failures, 'PIVOT_DRIFT', {
      stage: 'metadata-canonical',
      expected: config.pivot,
      actual: metadata.canonicalPivot
    });
  if (
    runtimeScale &&
    (metadata.pivot?.x !== config.pivot.x * runtimeScale || metadata.pivot?.y !== config.pivot.y * runtimeScale)
  )
    addFailure(failures, 'PIVOT_DRIFT', {
      stage: 'metadata',
      expected: { x: config.pivot.x * runtimeScale, y: config.pivot.y * runtimeScale },
      actual: metadata.pivot
    });
  if (metadata.frameSize?.width !== runtime.width || metadata.frameSize?.height !== runtime.height)
    addFailure(failures, 'METADATA_MISMATCH', { field: 'frameSize', expected: runtime, actual: metadata.frameSize });
  if (metadata.sheet !== path.basename(exported.sheet) || metadata.preview !== path.basename(exported.preview))
    addFailure(failures, 'METADATA_MISMATCH', {
      field: 'artifactNames',
      expected: { sheet: path.basename(exported.sheet), preview: path.basename(exported.preview) },
      actual: { sheet: metadata.sheet, preview: metadata.preview }
    });
  if (
    !Array.isArray(metadata.durations) ||
    metadata.durations.length !== normalized.frames.length ||
    metadata.durations.some((duration) => !Number.isInteger(duration) || duration < 11 || duration > 65535)
  )
    addFailure(failures, 'TIMING_MISMATCH', { field: 'durations', actual: metadata.durations });
  if (!samePaletteCounts(metadata.palette?.colors, paletteCounts(normalizedImages)))
    addFailure(failures, 'METADATA_MISMATCH', { field: 'palette' });
  if (!jsonEqual(relevantConfig(metadata.config ?? {}), relevantConfig(config)))
    addFailure(failures, 'METADATA_MISMATCH', {
      field: 'config',
      expected: relevantConfig(config),
      actual: relevantConfig(metadata.config ?? {})
    });
  for (let index = 0; index < normalized.frames.length; index += 1) {
    if (metadata.sources?.[index]?.sha256 !== measurements.sourceHashes[index].sha256)
      addFailure(failures, 'SOURCE_HASH_MISMATCH', {
        stage: 'metadata',
        trustedArtifact: false,
        frame: index,
        expected: metadata.sources?.[index]?.sha256,
        actual: measurements.sourceHashes[index].sha256
      });
    if (metadata.frames?.[index]?.duration !== metadata.durations?.[index])
      addFailure(failures, 'TIMING_MISMATCH', {
        frame: index,
        frameDuration: metadata.frames?.[index]?.duration,
        duration: metadata.durations?.[index]
      });
    ...
    if (
      ...
      (frame.width !== runtime.width ||
        ...
        frame.x !== expectedX ||
        frame.y !== expectedY ||
        frame.file !== path.basename(exported.runtimeFrames[index]))
    )
      addFailure(failures, 'METADATA_MISMATCH', { field: 'frames', frame: index });
  }

  const expectedRows =
    Number.isInteger(metadata.columns) && metadata.columns > 0
      ? Math.ceil(exported.runtimeFrames.length / metadata.columns)
      : null;
  if (!Number.isInteger(metadata.columns) || metadata.columns < 1 || metadata.rows !== expectedRows)
    addFailure(failures, 'METADATA_MISMATCH', {
      field: 'sheetGeometry',
  ...
  if (!expectedRows || !sheetMatches(sheet, runtimeImages, metadata.columns, expectedRows, runtime))
    addFailure(failures, 'FRAME_BLEED', {
      expected: expectedRows ? [metadata.columns * runtime.width, expectedRows * runtime.height] : null,
  ...
  const {
    ...
    info: previewInfo,
    metadata: preview
  } = (capturedArtifacts?.preview ?? (await capturePreview(exported.preview))).decoded;
  measurements.previewPages = preview.pages ?? 1;
  measurements.previewDelays = preview.delay ?? [];
  if ((preview.pages ?? 1) !== exported.runtimeFrames.length)
    addFailure(failures, 'PREVIEW_MISMATCH', {
      field: 'pages',
      expected: exported.runtimeFrames.length,
      actual: preview.pages ?? 1
    });
  const previewPageHeight = preview.pageHeight ?? preview.height;
  if (preview.width !== runtime.width || previewPageHeight !== runtime.height)
    addFailure(failures, 'PREVIEW_MISMATCH', {
      field: 'dimensions',
      expected: runtime,
      actual: { width: preview.width, height: previewPageHeight }
    });
  if ((preview.pages ?? 1) > 1 && JSON.stringify(preview.delay ?? []) !== JSON.stringify(metadata.durations ?? []))
    addFailure(failures, 'PREVIEW_MISMATCH', { field: 'delays', expected: metadata.durations, actual: preview.delay });
  ...
  for (let index = 0; index < Math.min(previewPages, runtimeImages.length); index += 1) {
    ...
    if (!page.equals(expected))
      addFailure(failures, 'PREVIEW_MISMATCH', { field: 'pixels', frame: index, expectedHash, actualHash });
  }
  ...
  if (animationContract) {
    for (const clip of animationContract.document.clips) {
      ...
      if (!loopApprovedByFrameManifest(verifiedFrameApproval, clip))
        warnings.push({
          code: 'HUMAN_REVIEW_REQUIRED',
          check: 'LOOP_ROOT_TRANSITION',
          clipId: clip.id,
...
export async function validateRun(request, options = {}) {
  ...
  return request.exported?.clips !== undefined ? validateContractExportRun(request, options) : validateFlatRun(request);
}
