...
function copyPixel(source, sourceX, sourceY, target, targetX, targetY) {
  source.data.copy(
    target.data,
    pixelOffset(target, targetX, targetY),
    pixelOffset(source, sourceX, sourceY),
    pixelOffset(source, sourceX, sourceY) + 4
  );
}
...
function resolveChroma(configuredColor, detectedBackground) {
  ...
  if (
    detectedBackground.r === 0 &&
    detectedBackground.g === 0 &&
    detectedBackground.b === 0 &&
    detectedBackground.a === 0
  ) {
...
export async function prepareAnchor({ input, outputDir, config }) {
  ...
  const background =
    validatedConfig.background.mode === 'configured' ? validatedConfig.background.color : detectedBackground;
  ...
  if (
    left < 0 ||
    top < 0 ||
    left + bounds.width > validatedConfig.canonical.width ||
    top + bounds.height > validatedConfig.canonical.height
  ) {
  ...
  await createPixelMatrix({
    output: pixelMatrix,
    width: validatedConfig.generation.width,
    height: validatedConfig.generation.height,
    blockSize: generationScaleX
  });

  const files = { input, canonicalChroma, canonicalTransparent, generationPlate, runtimeAnchor, pixelMatrix };
  const hashes = Object.fromEntries(
    await Promise.all(Object.entries(files).map(async ([name, file]) => [name, await sha256(file)]))
  );
  return {
    canonicalChroma,
