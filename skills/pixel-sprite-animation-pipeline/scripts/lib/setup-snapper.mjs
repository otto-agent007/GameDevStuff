...
async function ensureDirectory(directory, getUid) {
  ...
  if (
    !info.isDirectory() ||
    info.isSymbolicLink() ||
    (posix && ((info.mode & 0o022) !== 0 || (Number.isInteger(info.uid) && info.uid !== getUid())))
  ) {
...
function installedExecutable(finalDir, asset) {
  return path.join(finalDir, asset.executable);
}
function archiveFiles(asset) {
  return [asset.executable, ...BUNDLED_FILES];
}
function stagePrefix(releaseTag, target) {
  return `.install-${releaseTag}-${target}-`;
}
...
async function readStage(stage, expected) {
  ...
  if (!markerInfo.isFile() || markerInfo.isSymbolicLink() || markerInfo.nlink !== 1)
    throw new Error('unsafe Pixel Snapper install stage marker');
  const marker = JSON.parse(await fs.readFile(markerPath, 'utf8'));
  if (
    !validStageMarker(marker, { ...expected, name: expected.name ?? path.basename(stage) }) ||
    marker.dev !== info.dev ||
    marker.ino !== info.ino
  )
    throw new Error('Pixel Snapper install stage identity mismatch');
...
async function cleanupInterruptedStages(staging, releaseTag, target) {
  ...
  for (const name of names) {
    ...
    try {
      if (!(await guardedRemoveStage(path.join(staging, name), { releaseTag, target })))
        throw new Error('identity changed');
    } catch (error) {
      throw setupError(
        'PIXEL_SNAPPER_RECOVERY_BLOCKED',
        `Pixel Snapper recovery blocked by untrusted install stage: ${name}`,
        error
      );
    }
  }
...
async function createInstallStage(toolsDir, releaseTag, target, getUid, faults) {
  ...
  try {
    await fs.writeFile(
      path.join(pending, STAGE_MARKER),
      `${JSON.stringify(stageMarker(releaseTag, target, nonce, info))}\n`,
      { flag: 'wx', mode: 0o600 }
    );
...
function receiptIdentity(identity) {
  return Object.fromEntries(
    ['size', 'sha256', 'version', 'helpSha256', 'fixtureRgbaSha256', 'pinnedReleaseTag', 'upstreamCommit'].map(
...
async function installedFileRecords(finalDir, asset, { receipt = false } = {}) {
  ...
  if (JSON.stringify(actualNames) !== JSON.stringify(expectedNames))
    throw new Error('Pixel Snapper installed file inventory mismatch');
  const records = [];
  for (const name of archiveFiles(asset)) {
    const file = path.join(finalDir, name);
    const info = await fs.lstat(file);
    if (!info.isFile() || info.isSymbolicLink())
      throw new Error(`managed Pixel Snapper file must be a regular non-symlink file: ${name}`);
    if (info.nlink !== 1) {
...
async function inspectManaged(finalDir, manifest, asset) {
  return inspectPixelSnapperBinary({
    path: installedExecutable(finalDir, asset),
    origin: 'managed-cache',
    managed: { root: finalDir, asset },
    manifest,
    pinnedAsset: asset
  });
}
...
async function verifyReceipt(finalDir, expected) {
  ...
  let info;
  try {
    info = await fs.lstat(receipt);
  } catch (error) {
    if (error.code === 'ENOENT') throw new Error('Pixel Snapper installation receipt mismatch');
    throw error;
  }
  if (!info.isFile() || info.isSymbolicLink() || info.nlink !== 1)
    throw new Error('Pixel Snapper installation receipt mismatch');
  let actual;
  try {
    actual = JSON.parse(await fs.readFile(receipt, 'utf8'));
  } catch {
    throw new Error('Pixel Snapper installation receipt mismatch');
  }
...
async function verifyInstalledTool({
  finalDir,
  manifest,
  manifestSha256,
  target,
  ...
  const receipt = await verifyReceipt(
    finalDir,
    expectedReceipt({ manifestSha256, manifest, target, asset, identity, installedFiles })
  );
...
async function verifyInstalledRecord(finalDir, context) {
  ...
  if (
    ...
    (await fs.realpath(finalDir)) !== physical
  ) {
...
function classifyExecutable(error) {
  if (/probe|fixture/i.test(error?.message ?? ''))
    return setupError(
      'PIXEL_SNAPPER_PROBE_FAILED',
      'Pixel Snapper compatibility or deterministic fixture verification failed',
      error
    );
  return setupError(
    'PIXEL_SNAPPER_EXECUTABLE_MISMATCH',
    'Pixel Snapper executable identity did not match the pinned asset',
    error
  );
}
...
function validMoveMarker(value, reason, nonce) {
  return (
    ...
    typeof value === 'object' &&
    !Array.isArray(value) &&
    Object.keys(value).sort().join(',') === 'dev,ino,kind,nonce,reason,schemaVersion' &&
    value.schemaVersion === 1 &&
    value.kind === 'pixel-snapper-install-move' &&
    value.reason === reason &&
    value.nonce === nonce &&
    UUID.test(value.nonce ?? '') &&
    Number.isInteger(value.dev) &&
...
async function moveInstallation(finalDir, reason, verified = null, faults = null) {
  ...
  if (!before.isDirectory() || before.isSymbolicLink())
    throw new Error('unsafe managed Pixel Snapper installation move');
  const nonce = crypto.randomUUID();
...
async function deactivateCanonical(finalDir, reason, faults, originalCause) {
  ...
  try {
    ...
    try {
      ...
      if (!sameIdentity(before, current) || (await fs.realpath(finalDir)) !== physical)
        throw new Error('canonical identity changed before invalidation');
      if (typeof faults?.beforeInvalidation === 'function') await faults.beforeInvalidation({ finalDir, reason });
      const marker = {
        schemaVersion: 1,
        kind: 'pixel-snapper-canonical-invalidation',
        reason,
      ...
      const after = await fs.lstat(finalDir);
      if (!sameIdentity(current, after) || (await fs.realpath(finalDir)) !== physical)
        throw new Error('canonical identity changed during invalidation');
      return { invalidated: true, marker };
    } catch (invalidationError) {
      throw setupError(
        'PIXEL_SNAPPER_CANONICAL_DEACTIVATION_FAILED',
        'Pixel Snapper canonical deactivation and invalidation both failed',
        new AggregateError([originalCause, moveError, invalidationError], 'canonical deactivation failed')
      );
    }
  }
}
...
async function readMoved(moved, reason) {
  ...
  const markerInfo = await fs.lstat(markerPath);
  if (!markerInfo.isFile() || markerInfo.isSymbolicLink() || markerInfo.nlink !== 1)
    throw new Error('unsafe Pixel Snapper move marker');
  const marker = JSON.parse(await fs.readFile(markerPath, 'utf8'));
  if (!validMoveMarker(marker, reason, nonceMatch[1])) throw new Error('invalid Pixel Snapper move marker');
  const info = await fs.lstat(moved);
  if (!info.isDirectory() || info.isSymbolicLink() || info.dev !== marker.dev || info.ino !== marker.ino)
    throw new Error('Pixel Snapper moved directory identity mismatch');
...
async function guardedRemoveMoved(moved, reason, faults = null) {
  ...
  if (!alreadyMoved) {
    ...
    if (typeof faults?.afterCleanupDirectoryMove === 'function')
      await faults.afterCleanupDirectoryMove({ moved, cleanup });
  }
  if (record.markerPath !== cleanupMarker) await fs.rename(record.markerPath, cleanupMarker);
  if (typeof faults?.afterCleanupMarkerMove === 'function')
    await faults.afterCleanupMarkerMove({ cleanup, cleanupMarker });
...
async function activationLeftovers(finalDir) {
  ...
  return (await fs.readdir(parent))
    .filter((name) => name.startsWith(prefix) && !name.endsWith(MOVE_MARKER_SUFFIX))
    .sort()
...
async function verifyMovedInstallation(moved, context, expectedVerified) {
  ...
  if (
    expectedVerified &&
    JSON.stringify(receiptIdentity(verified.identity)) !== JSON.stringify(receiptIdentity(expectedVerified.identity))
  )
    throw new Error('Pixel Snapper backup verification changed');
...
async function recoverActivationState(finalDir, context, faults) {
  ...
  if (!(await exists(finalDir))) {
    ...
    try {
      ...
      throw setupError(
        'PIXEL_SNAPPER_RECOVERY_BLOCKED',
        'Pixel Snapper activation recovery found an untrusted backup',
        error
      );
    }
  }
  for (const leftover of leftovers.length ? leftovers : await activationLeftovers(finalDir)) {
    try {
      ...
      if (!(await guardedRemoveMoved(leftover, 'activation')))
        throw new Error('backup identity changed during cleanup');
    } catch (error) {
      throw setupError(
        'PIXEL_SNAPPER_RECOVERY_BLOCKED',
        'Pixel Snapper activation recovery found an untrusted leftover',
        error
      );
    }
  }
...
async function restoreVerifiedBackup({ backup, finalDir, context, faults, activationError }) {
  try {
    ...
    if (!sameIdentity(checked.identity, backup.identity))
      throw new Error('Pixel Snapper backup directory identity changed');
    ...
    throw setupError(
      'PIXEL_SNAPPER_ROLLBACK_FAILED',
      'Pixel Snapper rollback failed verification; canonical installation left inactive',
      new AggregateError([activationError, error], 'activation and rollback failed')
    );
  }
}

function classifyDownload(error) {
  if (error?.code === 'PIXEL_SNAPPER_RELEASE_NOT_FOUND')
    return setupError(
      error.code,
      'Pinned Pixel Snapper release asset was not found; verify the reviewed release exists',
      error
    );
  if (/redirect/i.test(error?.message ?? ''))
    return setupError(
      'PIXEL_SNAPPER_REDIRECT_ERROR',
      'Pixel Snapper download redirect was rejected by the pinned host policy',
      error
    );
  if (/size|checksum|maximum/i.test(error?.message ?? ''))
    return setupError(
      'PIXEL_SNAPPER_ARCHIVE_INTEGRITY',
      'Pixel Snapper archive size or checksum did not match the pinned manifest',
      error
    );
  return setupError(
    'PIXEL_SNAPPER_NETWORK_ERROR',
    'Pixel Snapper download failed; check network access and the pinned release URL',
    error
  );
}
...
async function writeInstallationReceipt({ content, manifestSha256, manifest, target, asset, identity }) {
  ...
  await fs.writeFile(
    receipt,
    `${JSON.stringify(expectedReceipt({ manifestSha256, manifest, target, asset, identity, installedFiles }), null, 2)}\n`,
    { flag: 'wx', mode: 0o600 }
  );
}

export async function setupPixelSnapper({
  projectDir,
  manifestPath,
  fetchImpl = fetch,
  force = false,
  ...
  if (
    ...
    (getUid !== null && typeof getUid !== 'function')
  ) {
  ...
  let target;
  try {
    target = platformKey(platform);
  } catch (error) {
    throw setupError(
      'PIXEL_SNAPPER_UNSUPPORTED_PLATFORM',
      `Pixel Snapper has no pinned asset for ${platform?.platform ?? process.platform}/${platform?.arch ?? process.arch}`,
      error
    );
  }
  ...
  try {
    return await withSetupLock({
      ...
      operation: async () => {
        ...
        if (await exists(finalDir)) {
          try {
            activeRecord = await verifyInstalledRecord(finalDir, context);
            active = activeRecord.verified;
          } catch (error) {
            await moveInstallation(finalDir, 'tampered');
            if (!force)
              throw setupError(
                'PIXEL_SNAPPER_INSTALLATION_TAMPERED',
                'Managed Pixel Snapper installation failed revalidation and was quarantined',
                error
              );
          }
        }
        ...
        try {
          let archive;
          try {
            archive = await downloadPinnedAsset({
              url: asset.url,
              upstreamCommit: manifest.upstream.commit,
              expectedSize: asset.archiveSize,
              expectedSha256: asset.archiveSha256,
              fetchImpl,
              output: path.join(owned.stage, 'download')
            });
          } catch (error) {
            throw classifyDownload(error);
          }

          let inspection;
          try {
            inspection = inspectArchive({
              bytes: await fs.readFile(archive.output),
              format: asset.archiveFormat,
              expectedFiles: archiveFiles(asset)
            });
          } catch (error) {
            throw setupError(
              'PIXEL_SNAPPER_UNSAFE_ARCHIVE',
              'Pixel Snapper archive failed closed safety inspection',
              error
            );
          }
          try {
            ...
            throw setupError(
              'PIXEL_SNAPPER_EXTRACTION_FAILED',
              'Pixel Snapper archive extraction failed before activation',
              error
            );
          }
          ...
          let stagedIdentity;
          try {
            stagedIdentity = await inspectManaged(content, manifest, asset);
          } catch (error) {
            throw classifyExecutable(error);
          }
          ...
          if (active) {
            ...
            if (sourcePhysical !== activeRecord.directory.physical || !sameIdentity(sourceInfo, activeRecord.directory))
              throw new Error('active Pixel Snapper identity changed after verification');
            backup = await moveInstallation(finalDir, 'activation', active, faults);
            if (!sameIdentity(sourceInfo, backup.identity))
              throw new Error('active Pixel Snapper identity changed before backup publication');
          }
          ...
          try {
            ...
            if (typeof faults?.beforeFinalVerification === 'function')
              await faults.beforeFinalVerification({ backup, finalDir });
            ...
            if (backup) {
              ...
              throw setupError(
                'PIXEL_SNAPPER_ACTIVATION_FAILED',
                'Pixel Snapper activation failed; the previous verified installation was restored',
                activationError
              );
            }
            if (await exists(finalDir))
              await deactivateCanonical(finalDir, 'failed-activation', faults, activationError);
            throw setupError(
              'PIXEL_SNAPPER_ACTIVATION_FAILED',
              'Pixel Snapper activation failed before a verified installation became active',
              activationError
            );
          }
          ...
          if (backup) {
            try {
              if (!(await guardedRemoveMoved(backup.path, 'activation', faults)))
                throw new Error('verified backup identity changed during cleanup');
            } catch (error) {
              throw setupError(
                'PIXEL_SNAPPER_RECOVERY_BLOCKED',
                'Verified activation backup cleanup was interrupted or changed',
                error
              );
            }
          }
          ...
          if (await exists(owned.stage)) {
            try {
              if (!(await guardedRemoveStage(owned.stage, owned.expected)))
                throw new Error('stage identity changed during cleanup');
            } catch (error) {
              throw setupError(
                'PIXEL_SNAPPER_RECOVERY_BLOCKED',
                'Pixel Snapper owned stage could not be safely cleaned',
                error
              );
            }
          }
