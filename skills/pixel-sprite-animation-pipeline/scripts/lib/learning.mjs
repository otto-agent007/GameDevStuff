...
function rejectAbsolutePrivatePaths(value, label, ancestors = new Set()) {
  if (typeof value === 'string') {
    if (path.isAbsolute(value) || path.win32.isAbsolute(value))
      throw new Error(`${label} must not persist absolute private paths`);
    return;
  }
  if (!value || typeof value !== 'object' || ancestors.has(value)) return;
  const next = new Set(ancestors).add(value);
  for (const child of Array.isArray(value) ? value : Object.values(value))
    rejectAbsolutePrivatePaths(child, label, next);
}
...
function statePaths(projectDir) {
  ...
  return {
    ...
    lessons: path.join(root, 'lessons.jsonl'),
...
async function reserve(lock) {
  ...
  try {
    ...
    if (error.code === 'EEXIST')
      throw new Error(`${path.basename(lock, '.lock')} is already reserved or already exists`);
    throw error;
  }
}
  ...
  for (const raw of inputs) {
    ...
    for (const segment of resolved.slice(parsed.root.length).split(path.sep).filter(Boolean)) {
      ...
      if (stat.isSymbolicLink())
        throw new Error(`input path must not contain a symlink: ${item.id ?? path.basename(resolved)}`);
    }
...
async function resolveRunArtifact(runDir, value, label) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink())
    throw new Error(`${label} must be a regular non-symlink file under the run directory`);
  ...
  if (containment === '..' || containment.startsWith(`..${path.sep}`) || path.isAbsolute(containment))
    throw new Error(`${label} must be physically contained under the run directory`);
...
function normalizeAnimationContract(animationContract) {
  ...
  if (
    !animationContract ||
    typeof animationContract !== 'object' ||
    Array.isArray(animationContract) ||
    Object.keys(animationContract).sort().join(',') !== 'document,sha256'
  )
    throw new Error('animation contract binding must contain only document and sha256');
...
export async function createRun({
  ...
  if (inspectionSnapshot !== undefined) {
    ...
    if (!snapshot || typeof snapshot !== 'object' || Array.isArray(snapshot) || !HASH.test(snapshot.sha256 ?? ''))
      throw new Error('inspection snapshot must contain a source sha256');
    if (
      ...
      throw new Error('inspection snapshot dimensions must be positive integers');
    if (snapshot.sha256 !== normalizedInputs[0].sha256)
      throw new Error('inspection source hash does not match the immutable input');
    const rawInput = typeof inputs[0] === 'string' ? inputs[0] : inputs[0]?.path;
    const metadata = await sharp(rawInput).metadata();
    if (snapshot.width !== metadata.width || snapshot.height !== metadata.height)
      throw new Error('inspection dimensions do not match the immutable input');
  ...
  try {
    ...
    const manifest = {
      ...
      ...(normalizedAnimationContract ? { animationContract: normalizedAnimationContract } : {})
    };
...
async function loadRun(projectDir, runId) {
  ...
  if (!directory.isDirectory() || directory.isSymbolicLink())
    throw new Error('run directory must be a real immutable directory');
...
async function verifiedArtifactSet(validation, runDir) {
  ...
  for (const artifact of validation.artifacts) {
    ...
    let resolved;
    try {
      resolved = await resolveRunArtifact(runDir, artifact.path, 'validation artifact path');
    } catch {
      return false;
    }
...
async function hasVerifiedArtifact(validation, expectedSha256, runDir) {
  ...
  for (const artifact of validation.artifacts) {
    ...
    const resolved = await resolveRunArtifact(runDir, artifact.path, 'lesson validation artifact path').catch(
      () => null
    );
...
function lessonIdentity(lesson) {
  for (const key of ['failureCode', 'correction', 'proposedRule'])
    if (typeof lesson?.[key] !== 'string' || lesson[key].trim() === '') throw new Error(`lesson ${key} is required`);
...
async function backgroundCounts(file, config) {
  ...
  for (let y = 0; y < info.height; y += 1)
    for (let x = 0; x < info.width; x += 1) {
      ...
      if (data[offset + 3] > 0 && (x === 0 || y === 0 || x === info.width - 1 || y === info.height - 1))
...
async function verifyCorrectionObjective(action, evidence, beforeFile, afterFile, config) {
  ...
  if (action.code === 'BACKGROUND_REMAINS' && action.correction === 'rekey') {
    ...
    return (
      (before.opaqueBorderPixels > 0 || before.configuredColorPixels > 0) &&
      after.opaqueBorderPixels === 0 &&
      after.configuredColorPixels === 0 &&
      measurements?.background?.opaqueBorderPixels === 0 &&
      measurements?.background?.configuredColorPixels === 0
    );
  }
  if (action.code === 'CANVAS_SIZE' && action.correction === 'repad') {
    ...
    return (
      Boolean(expected) &&
      (before.width !== expected.width || before.height !== expected.height) &&
      after.width === expected.width &&
      after.height === expected.height
    );
  }
  return false;
}
...
function correctionTarget(value) {
  return Object.fromEntries(
    ['code', 'frame', 'stage', 'target'].filter((key) => value?.[key] !== undefined).map((key) => [key, value[key]])
  );
}
...
async function verifyCorrectionLesson(run, lesson) {
  ...
  const match =
    typeof lesson.correctionManifest.path === 'string' &&
    /^correction-(\d{2,})\/manifest\.json$/.exec(portable(lesson.correctionManifest.path));
  ...
  if ((await fileSha(absolute)) !== lesson.correctionManifest.sha256)
    throw new Error('correctionManifest hash does not match');
  const document = JSON.parse(await fs.readFile(absolute, 'utf8'));
  if (
    document.version !== 1 ||
    !Number.isInteger(document.correctionVersion) ||
    document.correctionVersion !== Number(match[1]) ||
    document.run?.id !== run.manifest.runId ||
    document.run?.manifestSha256 !== run.manifestSha256
  )
    throw new Error('correction manifest version, directory, or run binding is invalid');
  if (!Number.isInteger(lesson.actionIndex) || lesson.actionIndex < 0)
    throw new Error('lesson actionIndex must be a nonnegative integer');
  const action = document.actions?.[lesson.actionIndex];
  const evidence = action?.evidenceVerification;
  if (
    action?.code !== identity.failureCode ||
    action?.correction !== identity.correction ||
    action?.approved !== true ||
    !['applied', 'deduplicated'].includes(action?.status)
  )
    throw new Error('lesson correction action is not approved evidence');
  if (
    evidence?.valid !== true ||
    evidence.beforeValidation?.passed !== false ||
    evidence.afterValidation?.passed !== true ||
    !HASH.test(evidence.beforeSha256 ?? '') ||
    !HASH.test(evidence.afterSha256 ?? '') ||
    evidence.beforeSha256 === evidence.afterSha256
  )
    throw new Error('lesson requires matching before/after validation evidence and hashes');
  ...
  if (
    !sameCorrectionTarget(evidence.target, target) ||
    !Array.isArray(beforeFailures) ||
    !beforeFailures.some((failure) => sameCorrectionTarget(failure, target)) ||
    !Array.isArray(afterFailures) ||
    afterFailures.some((failure) => sameCorrectionTarget(failure, target))
  )
    throw new Error('lesson before/after validation evidence does not match the failure target');
  if (
    !(await verifiedArtifactSet(evidence.beforeValidation, runDir)) ||
    !(await verifiedArtifactSet(evidence.afterValidation, runDir)) ||
    !(await hasVerifiedArtifact(evidence.beforeValidation, evidence.beforeSha256, runDir)) ||
    !(await hasVerifiedArtifact(evidence.afterValidation, evidence.afterSha256, runDir))
  )
    throw new Error('lesson requires artifact-backed before/after validation evidence');
  const beforeArtifact = evidence.beforeValidation.artifacts.find(
    (artifact) => artifact.sha256 === evidence.beforeSha256
  );
  ...
  if (!(await verifyCorrectionObjective(action, evidence, beforeFile, afterFile, run.manifest.config)))
    throw new Error('lesson lacks artifact-specific objective correction evidence');
  return {
    ...identity,
    correctionManifest: { path: manifestPath, sha256: lesson.correctionManifest.sha256 },
...
async function readLessonRows(file) {
  ...
  try {
    ...
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink > 1)
      throw new Error('lessons.jsonl must be a regular non-linked state file');
  ...
  for (const [index, line] of contents.split(/\r?\n/).entries()) {
    ...
    try {
      ...
      const valid =
        ...
        typeof row.proposedRule === 'string' &&
        row.report &&
        row.correctionManifest;
      if (!valid) throw new Error('invalid lesson schema');
      rows.push(row);
    } catch (error) {
      throw new Error(`malformed lessons.jsonl row ${index + 1}`, { cause: error });
    }
  }
  return rows;
}
...
async function appendLessons(file, rows) {
  ...
  try {
    ...
    const keys = new Set(
      existing.map((row) => `${row.runId}:${row.report?.sha256}:${row.correctionManifest?.sha256}:${row.actionIndex}`)
    );
...
export async function recordRunResult({ projectDir, runId, report, version } = {}) {
  ...
  const rows = lessons.map((lesson) => ({
    ...
    report: { path: fileName, sha256: reportSha256 },
    ...lesson
  }));
...
async function loadVerifiedReport(run, version) {
  const file = (await resolveRunArtifact(run.runDir, path.basename(reportPath(run.runDir, version)), 'run report'))
    .absolute;
  const report = JSON.parse(await fs.readFile(file, 'utf8'));
  if (report.runId !== run.manifest.runId || report.manifestSha256 !== run.manifestSha256)
    throw new Error('report is not tied to the selected run manifest');
  if (!(await verifiedArtifacts(report.validation, run.runDir)))
    throw new Error('profile promotion requires artifact-backed passing validation evidence');
  if (
    report.toolProvenanceVerified !== true ||
    !report.snapReceipt ||
    typeof report.snapReceipt !== 'object' ||
    Array.isArray(report.snapReceipt) ||
    Object.keys(report.snapReceipt).sort().join(',') !== 'path,sha256' ||
    !HASH.test(report.snapReceipt.sha256 ?? '')
  )
    throw new Error('profile promotion requires verified tool provenance');
  const outstandingReview =
    report.profilePromotion?.reviewRequired === true ||
  ...
  try {
    const receipt = await resolveRunArtifact(
      run.runDir,
      validRelative(report.snapReceipt.path, 'snap receipt path'),
      'snap receipt'
    );
    const projectDir = path.dirname(path.dirname(path.dirname(run.runDir)));
    const animationContract = run.manifest.animationContract
      ? normalizeAnimationContract(run.manifest.animationContract)
      : undefined;
    const verified = await verifySnapReceipt({
      projectDir,
      file: receipt.absolute,
      expectedRun: { runId: run.manifest.runId, runDir: run.runDir, manifestSha256: run.manifestSha256 },
      ...(animationContract ? { expectedContract: animationContract } : {})
    });
    if (verified.document.payload.toolProvenanceVerified !== true || verified.sha256 !== report.snapReceipt.sha256)
      throw new Error('receipt binding mismatch');
    if (animationContract) {
      if (
        report.animationContractSha256 !== animationContract.sha256 ||
        report.snapReceiptSha256 !== verified.sha256 ||
        report.toolProvenanceVerified !== true ||
        report.profilePromotion?.eligible !== true
      )
        throw new Error('animation approval chain is not eligible for profile promotion');
      if (
        !report.frameApproval ||
        typeof report.frameApproval !== 'object' ||
        !Number.isInteger(report.frameApproval.version) ||
        !HASH.test(report.frameApproval.sha256 ?? '')
      )
        throw new Error('animation approval chain is not eligible for profile promotion');
      const approval = await resolveRunArtifact(
        run.runDir,
        validRelative(report.frameApproval.path, 'frame approval path'),
        'frame approval'
      );
      const selected = await verifyFrameApproval({
        projectDir,
        file: approval.absolute,
        contract: animationContract,
        snapReceipt: { path: receipt.absolute, sha256: verified.sha256 },
        version: report.frameApproval.version
      });
      if (selected.sha256 !== report.frameApproval.sha256 || selected.sha256 !== report.frameApprovalSha256)
        throw new Error('animation approval chain is not eligible for profile promotion');
    }
  } catch (error) {
...
async function verifyLessonRow(projectDir, row, identity) {
  if (
    !row ||
    row.version !== 1 ||
    row.failureCode !== identity.failureCode ||
    row.correction !== identity.correction ||
    row.proposedRule !== identity.proposedRule
  )
    return null;
  if (
    !HASH.test(row.manifestSha256 ?? '') ||
    !HASH.test(row.report?.sha256 ?? '') ||
    !HASH.test(row.correctionManifest?.sha256 ?? '') ||
    !HASH.test(row.beforeSha256 ?? '') ||
    !HASH.test(row.afterSha256 ?? '')
  )
    return null;
  ...
  const reportFile = (await resolveRunArtifact(run.runDir, relativeReport, 'lesson report').catch(() => null))
    ?.absolute;
  ...
  if (
    ...
    report.manifestSha256 !== row.manifestSha256 ||
    !(await verifiedArtifacts(report.validation, run.runDir))
  )
    return null;
  const verified = await verifyCorrectionLesson(run, {
    ...identity,
    correctionManifest: row.correctionManifest,
  ...
  return {
    ...
    reportSha256: row.report.sha256,
    correctionManifestSha256: row.correctionManifest.sha256,
...
export async function proposeSkillRule({ projectDir, lesson, config, thresholdOverride } = {}) {
  ...
  if (!Number.isInteger(configuredThreshold) || configuredThreshold < 1)
    throw new Error('correction skillProposalEvidence must be a positive integer');
  if (thresholdOverride !== undefined && (!Number.isInteger(thresholdOverride) || thresholdOverride < 1))
    throw new Error('thresholdOverride must be a positive integer');
  ...
  const overrideText =
    thresholdOverride === undefined
      ? ''
      : ` A threshold override of ${thresholdOverride} was requested; the configured threshold is ${configuredThreshold}.`;
  return {
    ...identity,
    eligible,
    applied: false,
