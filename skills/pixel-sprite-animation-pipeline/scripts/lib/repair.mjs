...
function objectivePassed(report) {
  return report.passed || report.failures.every((failure) => REVIEW_ONLY.has(failure.correction));
}
...
function beforeFor(failure, request) {
  ...
  if (['PREVIEW_MISMATCH'].includes(failure.code) || (failure.code === 'FRAME_COUNT' && failure.stage !== 'metadata'))
    return request.exported.preview;
  if (
    ['METADATA_MISMATCH', 'TIMING_MISMATCH', 'SOURCE_HASH_MISMATCH'].includes(failure.code) ||
    failure.stage?.startsWith('metadata')
  )
    return request.exported.metadata;
  if (failure.stage === 'runtime' || failure.code === 'BACKGROUND_REMAINS' || failure.code === 'INTERMEDIATE_COLORS')
    return request.exported.runtimeFrames?.[failure.frame ?? 0];
...
function selectedAfter(failure, normalized, exported) {
  ...
  if (failure.stage === 'runtime' || failure.code === 'BACKGROUND_REMAINS' || failure.code === 'INTERMEDIATE_COLORS')
    return exported.runtimeFrames[failure.frame ?? 0];
...
export async function repairValidationRun({ request, run, config, expected, delivery }) {
  ...
  if (automatic.some((failure) => typeof failure.before !== 'string'))
    throw new Error('automatic correction lacks a trusted source artifact');
  ...
  if (
    ...
    sources.length !== request.normalized.frames.length ||
    sources.some((file) => typeof file !== 'string')
  )
    throw new Error('automatic correction requires trusted normalization source artifacts');
  ...
  const operations = {};
  for (const correction of AUTOMATIC)
    operations[correction] = async ({ failures: grouped, outputDir, correctionVersion }) => {
      const stem = correctionExecutionStem(correction, grouped[0], correctionVersion);
      const normalized = await normalizeFrames({
        inputs: sources,
        outputDir: path.join(outputDir, `${stem}-normalized`),
        config,
        scaleFactor: 1
      });
      ...
      for (const failure of grouped) {
        ...
        revalidations.push({
          ...
          afterValidation: {
            ...afterValidation,
            measurements: measurementProof(failure, afterValidation),
            artifacts: [await artifact(after)]
          }
        });
      }
  ...
  const correction = await applyDeterministicCorrections({
    failures,
    run: {
      ...run,
      expected: {
        metadata,
        preview: { runtimeFrames: request.exported.runtimeFrames, durations: delivery.durations },
        sheet: {
          runtimeFrames: request.exported.runtimeFrames,
          columns: delivery.columns,
          frameSize: delivery.frameSize
        }
      }
    },
    config,
  ...
  const approved = correction.actions.find(
    (action) => action.approved && action.result?.normalized && action.result?.exported
  );
