...
function exact(value, keys, label) {
  const missing =
    value && typeof value === 'object' && !Array.isArray(value) ? keys.find((key) => !Object.hasOwn(value, key)) : null;
  if (
    !value ||
    typeof value !== 'object' ||
    Array.isArray(value) ||
    ...
    throw new Error(`animation contract ${missing ?? label} schema is invalid`);
}

function exactV2(value, keys, label) {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new Error(`animation contract ${label} schema is invalid`);
...
function uniqueStrings(value, label, { nonempty = true } = {}) {
  if (
    ...
    throw new Error(`animation contract ${label} must be a unique ordered string list`);
}

function portableId(value, label) {
  if (typeof value !== 'string' || !PORTABLE_ID.test(value))
    throw new Error(`animation contract ${label} must be a portable ID`);
}
...
function coordinate(value, label, target = false) {
  ...
  if (
    ...
    (target ? !same(value, TARGET) : value.x < 0 || value.y < 0)
  )
    throw new Error(`animation contract ${label} is invalid`);
}
...
function validateAnchor(anchor) {
  ...
  if (
    !Array.isArray(anchor.traitReferenceSha256) ||
    anchor.traitReferenceSha256.length === 0 ||
    new Set(anchor.traitReferenceSha256).size !== anchor.traitReferenceSha256.length
  )
    throw new Error('animation contract anchor trait references are required');
...
function validatePalette(palette) {
  ...
  if (
    !Array.isArray(palette.rgba) ||
    palette.rgba.length < 2 ||
    palette.rgba.some((entry) => !rgba(entry)) ||
    new Set(palette.rgba.map((entry) => entry.join(','))).size !== palette.rgba.length
  )
    throw new Error('animation contract palette RGBA values are invalid');
  if (palette.rgba[0][3] !== 0 || palette.rgba.slice(1).some((entry) => entry[3] !== 255))
    throw new Error('animation contract palette requires one leading transparent RGBA entry and opaque colors');
  if (palette.rgba.length - 1 > 16)
    throw new Error('animation contract palette exceeds the fixed 16-color Snapper limit');
  if (palette.sha256 !== stableHash(palette.rgba))
    throw new Error('animation contract palette hash does not bind its ordered RGBA values');
  if (
    !Array.isArray(palette.snapperPaletteHex) ||
    !same(
      palette.snapperPaletteHex,
      palette.rgba.slice(1).map((entry) => `${hex(entry[0])}${hex(entry[1])}${hex(entry[2])}`)
    )
  )
    throw new Error('animation contract snapperPaletteHex must exactly match ordered opaque palette colors');
}
...
function validateClips(clips, review) {
  ...
  for (const clip of clips) {
    ...
    if (typeof clip.id !== 'string' || clip.id === '' || clipIds.has(clip.id))
      throw new Error('animation contract clip IDs must be unique and ordered');
    clipIds.add(clip.id);
    ...
    if (!Array.isArray(clip.frames) || clip.frames.length === 0)
      throw new Error('animation contract clip frames are required');
    const poses = new Set();
    for (const frame of clip.frames) {
      exact(frame, ['id', 'pose', 'duration', 'landmarkSemantic'], 'frame');
      if (typeof frame.id !== 'string' || frame.id === '' || frameIds.has(frame.id))
        throw new Error('animation contract frame IDs must be unique and ordered');
      frameIds.add(frame.id);
      if (typeof frame.pose !== 'string' || frame.pose === '' || poses.has(frame.pose))
        throw new Error('animation contract frame pose labels must be unique within each clip');
      poses.add(frame.pose);
      if (!Number.isInteger(frame.duration) || frame.duration < 11 || frame.duration > 65535)
        throw new Error('animation contract frame duration must be one integer from 11 to 65535');
      exact(frame.landmarkSemantic, ['name', 'target'], 'landmarkSemantic');
      if (typeof frame.landmarkSemantic.name !== 'string' || frame.landmarkSemantic.name === '')
        throw new Error('animation contract landmarkSemantic name is required');
    ...
    if (clip.loopMode === 'loop') {
      ...
      if (
        clip.loopTransition.fromFrameId !== last ||
        clip.loopTransition.toFrameId !== first ||
        typeof clip.loopTransition.reviewCheckpoint !== 'string' ||
        clip.loopTransition.reviewCheckpoint === '' ||
        !review.checkpoints.includes(clip.loopTransition.reviewCheckpoint)
      )
        throw new Error(
          'animation contract loopTransition must bind the final frame to the first through a declared review checkpoint'
        );
...
export function validateAnimationContractV1(document, { projectId = null } = {}) {
  ...
  if (!same(document.sizes, SIZES))
    throw new Error(
      `animation contract sizes must fix${projectId ? ` ${projectId}` : ''} canonical, generation, runtime, and pixelSize values`
    );
...
function validateCanvasV2(canvas) {
  ...
  if (
    ...
    throw new Error('animation contract canvas dimensions must be positive bounded integers');
  coordinate(canvas.pivot, 'canvas pivot');
  if (canvas.pivot.x >= canvas.width || canvas.pivot.y >= canvas.height)
    throw new Error('animation contract canvas pivot must be inside the stable canvas');
  if (!Number.isInteger(canvas.baseline) || canvas.baseline < 0 || canvas.baseline >= canvas.height)
    throw new Error('animation contract canvas baseline must be inside the stable canvas');
}

function validateScaleV2(scale, canvas) {
  exactV2(scale, ['integer', 'runtime'], 'scale');
  if (!Number.isInteger(scale.integer) || scale.integer < 1 || scale.integer > 64)
    throw new Error('animation contract global scale must be one positive integer');
  ...
  if (scale.runtime.width !== canvas.width * scale.integer || scale.runtime.height !== canvas.height * scale.integer)
    throw new Error('animation contract runtime dimensions must equal the stable canvas times global scale');
}
...
function validateTracksV2(tracks) {
  ...
  for (const track of tracks) {
    ...
    if (track.kind === 'actor' && track.attachTo !== null)
      throw new Error('animation contract actor track cannot attach to a socket');
    if (track.kind !== 'actor') portableId(track.attachTo, 'track attachment socket');
  }
  if (tracks.filter(({ kind }) => kind === 'actor').length !== 1)
    throw new Error('animation contract requires exactly one actor track');
}
...
function validateSocketsV2(sockets, tracks) {
  ...
  for (const track of tracks) {
    if (track.attachTo !== null && !ids.has(track.attachTo))
      throw new Error(`animation contract track references unknown socket: ${track.attachTo}`);
  }
}
...
function validateContactsV2(contacts, tracks) {
  ...
  for (const contact of contacts) {
    ...
    if (!CONTACT_KINDS.has(contact.kind))
      throw new Error(`animation contract contact kind is invalid: ${contact.kind}`);
    ...
    if (contact.kind === 'planted-foot' && track.kind !== 'actor')
      throw new Error('animation contract planted-foot contact must belong to the actor track');
  }
}
...
function validateGroundTravel(value) {
  ...
  if (!Number.isInteger(value.x) || !Number.isInteger(value.y))
    throw new Error('animation contract groundTravel must use integer coordinates');
}
...
function validateClipsV2(clips, tracks, sockets, contacts) {
  ...
  for (const clip of clips) {
    ...
    for (const frame of clip.frames) {
      ...
      frameIds.add(frame.id);
      if (typeof frame.semantic !== 'string' || frame.semantic.trim() === '')
        throw new Error('animation contract frame semantic is required');
      if (!Number.isInteger(frame.duration) || frame.duration < 11 || frame.duration > 65535)
        throw new Error('animation contract frame duration must be one integer from 11 to 65535');
      ...
      for (const id of frame.tracks)
        if (!trackById.has(id)) throw new Error(`animation contract frame references unknown track: ${id}`);
      if (!frame.tracks.some((id) => trackById.get(id).kind === 'actor'))
        throw new Error('animation contract frame must include the actor track');
      for (const track of tracks)
        if (track.required && !frame.tracks.includes(track.id))
          throw new Error(`animation contract frame omits required track: ${track.id}`);
      for (const id of frame.sockets)
        if (!socketIds.has(id)) throw new Error(`animation contract frame references unknown socket: ${id}`);
      for (const id of frame.contacts)
        if (!contactIds.has(id)) throw new Error(`animation contract frame references unknown contact: ${id}`);
      for (const id of frame.tracks) {
        const attachment = trackById.get(id).attachTo;
        if (attachment !== null && !frame.sockets.includes(attachment))
          throw new Error(`animation contract frame omits attachment socket: ${attachment}`);
      }
