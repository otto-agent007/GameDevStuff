...
function exact(value, keys, label) {
  if (
    ...
    throw new Error(`snap receipt ${label} schema is invalid`);
}
...
function runBinding(run) {
  ...
  return {
    id,
    manifestSha256: run.manifestSha256 == null ? null : hash(run.manifestSha256, 'snap receipt manifest hash')
  };
}

function validRun(binding) {
  exact(binding, ['id', 'manifestSha256'], 'run');
  if (binding.id !== null && (typeof binding.id !== 'string' || binding.id === ''))
    throw new Error('snap receipt run schema is invalid');
...
function portableOutputPath(value) {
  if (
    ...
    throw new Error('snap receipt output path must be a portable path contained by the receipt directory');
  return value;
}
...
async function containedOutput(receiptDir, relative) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink > 1)
    throw new Error('snap receipt output artifact must be a regular single-link file');
  ...
  if (containment === '..' || containment.startsWith(`..${path.sep}`) || path.isAbsolute(containment))
    throw new Error('snap receipt output artifact escaped the receipt directory');
  return current;
}
...
async function records(files, receiptDir, { contained = false } = {}) {
  ...
  for (let index = 0; index < files.length; index += 1) {
    ...
    if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink > 1)
      throw new Error('snap receipt artifact must be a regular single-link file');
...
function validBinary(binary) {
  exact(
    binary,
    ['origin', 'sha256', 'size', 'version', 'helpSha256', 'fixtureRgbaSha256', 'pinnedReleaseTag', 'upstreamCommit'],
    'binary'
  );
  if (
    !ORIGINS.has(binary.origin) ||
    !Number.isInteger(binary.size) ||
    binary.size < 1 ||
    typeof binary.version !== 'string' ||
    binary.version === ''
  )
    throw new Error('snap receipt binary identity is invalid');
  ...
  if (pinned !== null) {
    const tag =
      ...
      /^pixel-snapper-v\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?-commit\.([a-f0-9]{7})$/.exec(pinned);
    if (!tag || typeof commit !== 'string' || !/^[a-f0-9]{40}$/.test(commit) || tag[1] !== commit.slice(0, 7))
      throw new Error('snap receipt binary pin is invalid');
  }
}

function validDate(value) {
  if (typeof value !== 'string' || Number.isNaN(Date.parse(value)) || new Date(value).toISOString() !== value)
    throw new Error('snap receipt date is invalid');
}

function validAlignedDerivation(value) {
  const keys =
    value?.kind === 'canonical-palette-normalization'
...
function expectedBinding(payload, expectedRun, expectedContract) {
  ...
  if (expectedContract && payload.animationContractSha256 !== contractHash(expectedContract))
    throw new Error('snap receipt contract binding mismatch');
}
...
async function verifyRecords(recordsValue, receiptDir, label, { contained = false } = {}) {
  if (!Array.isArray(recordsValue) || recordsValue.length === 0)
    throw new Error(`snap receipt ${label} record is invalid`);
  for (const [index, item] of recordsValue.entries()) {
    ...
    if (
      ...
      (contained && (item.path === '.' || item.path === '..' || item.path.startsWith('../')))
    )
      throw new Error(`snap receipt ${label} record is invalid`);
...
async function existingReceipt({
  ...
  if (expectedInputs && !same(payload.inputs, await records(expectedInputs, path.dirname(file))))
    throw new Error('existing snap receipt input binding mismatch');
  if (expectedArgs && !same(payload.arguments, [...expectedArgs]))
    throw new Error('existing snap receipt argument binding mismatch');
  if (expectedIdentity && !same(payload.binary, binaryRecord(expectedIdentity)))
    throw new Error('existing snap receipt binary identity mismatch');
  if (expectedHandoff && payload.handoffSha256 !== (await sha256(expectedHandoff)))
    throw new Error('existing manual handoff receipt binding mismatch');
  if (expectedDerivation && !same(payload.derivation, expectedDerivation))
    throw new Error('existing aligned source receipt derivation mismatch');
  return existing;
}
...
export async function writeSnapReceipt({ projectDir, run, contract, inputs, outputs, args, identity }) {
  ...
  if (!Array.isArray(args) || args.some((argument) => typeof argument !== 'string'))
    throw new Error('snap receipt arguments must be strings');
  ...
  const payload = {
    ...
    arguments: [...args],
    binary: binaryRecord(identity),
    createdAt: new Date().toISOString()
  };
  let signed;
  try {
    signed = await writeSignedState({ projectDir, file, domain: SNAP_DOMAIN, payload, createKey: true });
  } catch (error) {
    if (error.code !== 'EEXIST') throw error;
    const concurrent = await existingReceipt({
      projectDir,
      file,
      domain: SNAP_DOMAIN,
      expectedRun: run,
...
export async function writeManualHandoffReceipt({ projectDir, run, handoff, inputs, outputs }) {
  ...
  let signed;
  try {
    signed = await writeSignedState({ projectDir, file, domain: MANUAL_DOMAIN, payload, createKey: true });
  } catch (error) {
    if (error.code !== 'EEXIST') throw error;
    const concurrent = await existingReceipt({
      projectDir,
...
export async function writeAlignedSourceReceipt({ projectDir, run, contract, inputs, outputs, derivation }) {
  ...
  let signed;
  try {
    signed = await writeSignedState({ projectDir, file, domain: ALIGNED_DOMAIN, payload, createKey: true });
  } catch (error) {
    if (error.code !== 'EEXIST') throw error;
    const concurrent = await existingReceipt({
      projectDir,
...
export async function verifySnapReceipt({ projectDir, file, expectedRun, expectedContract }) {
  ...
  if (!before.isFile() || before.isSymbolicLink() || before.nlink !== 1)
    throw new Error('snap receipt must be a regular non-symlink single-link file');
  const physical = await fs.realpath(selected);
  const physicalInfo = await fs.lstat(physical);
  if (
    !physicalInfo.isFile() ||
    physicalInfo.isSymbolicLink() ||
    physicalInfo.dev !== before.dev ||
    physicalInfo.ino !== before.ino
  )
    throw new Error('snap receipt file identity is unsafe');
  ...
  const domain =
    raw?.payload?.origin === 'manual-handoff'
      ? MANUAL_DOMAIN
      : raw?.payload?.origin === 'verified-aligned-source'
        ? ALIGNED_DOMAIN
        : SNAP_DOMAIN;
  ...
  if (domain === SNAP_DOMAIN) {
    ...
    if (
      payload.version !== 1 ||
      !ORIGINS.has(payload.origin) ||
      payload.toolProvenanceVerified !== true ||
      !Array.isArray(payload.arguments) ||
      payload.arguments.length === 0 ||
      payload.arguments.some((item) => typeof item !== 'string') ||
      !payload.binary ||
      payload.origin !== payload.binary.origin
    )
      throw new Error('verified snap receipt provenance is invalid');
    ...
    if (
      payload.version !== 1 ||
      payload.origin !== 'manual-handoff' ||
      payload.toolProvenanceVerified !== false ||
      payload.arguments !== null ||
      payload.binary !== null
    )
      throw new Error('manual handoff receipt provenance is invalid');
    ...
    exact(
      ...
      [
        ...
        'origin',
        'toolProvenanceVerified',
        'deterministicProvenanceVerified',
        'run',
        'animationContractSha256',
        'inputs',
        'outputs',
    ...
    if (
      payload.version !== 1 ||
      payload.origin !== 'verified-aligned-source' ||
      payload.toolProvenanceVerified !== false ||
      payload.deterministicProvenanceVerified !== true
    )
      throw new Error('aligned source receipt provenance is invalid');
    hash(payload.animationContractSha256, 'snap receipt contract hash');
  ...
  if (after.dev !== before.dev || after.ino !== before.ino || (await fs.realpath(selected)) !== physical)
    throw new Error('snap receipt file identity changed during verification');
...
export async function verifyExistingSnapReceipt(options) {
  return existingReceipt(options);
}
