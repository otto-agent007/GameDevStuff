...
function inferGrid(image, background, tolerance) {
  ...
  for (const [axis, values] of Object.entries(byAxis)) {
    ...
    const significant = [...counts]
      .filter(([, count]) => count >= 4 && count / Math.max(1, values.length) >= 0.15)
      .sort((a, b) => b[1] - a[1]);
    if (significant.length >= 2)
      repeatedModes.push({ axis, modes: significant.slice(0, 3).map(([length, count]) => ({ length, count })) });
  }
  const evidence = {
    runLengths: [...new Set(useful)].sort((a, b) => a - b).slice(0, 32),
    sampleCount: useful.length,
    repeatedIncompatibleModes: repeatedModes
  };
  const confidence =
    blockSize === 1
      ? 0.55
      : blockSize && useful.length >= 4
        ? Math.min(0.75, 0.35 + useful.length / 100)
        : blockSize
          ? 0.25
          : 0;
  ...
  return {
    blockSize,
    confidence: mixedBlockSizes ? Math.min(0.8, 0.55 + repeatedModes.length * 0.1) : confidence,
    evidence,
    mixedBlockSizes,
    method: 'foreground run GCD with conservative same-axis repeated-mode check',
    limitation:
      'Repeated run lengths can reflect intentional geometry; mixed-block diagnostics require two statistically repeated modes on one axis.'
  };
}
...
export async function inspectImage(file, { tolerance = 0, backgroundColor } = {}) {
  ...
  const margins = bounds
    ...
    : null;
  const clipping = margins
    ? { left: margins.left === 0, top: margins.top === 0, right: margins.right === 0, bottom: margins.bottom === 0 }
  ...
  const smoothingEvidence = {
    partialAlphaPixels: alpha.partial,
    paletteSize: palette.length,
    repeatedMixedGridModes: pixelGrid.evidence.repeatedIncompatibleModes.length
  };
  const smoothing = {
  ...
  if (clipping.any)
    diagnostics.push({
      code: 'EDGE_CLIPPING',
      severity: 'warning',
      edges: Object.keys(clipping).filter((key) => key !== 'any' && clipping[key])
    });
  if (smoothing.suspected)
    diagnostics.push({
      code: 'SMOOTHING_SUSPECTED',
      severity: 'warning',
      confidence: smoothing.confidence,
      evidence: smoothingEvidence
    });
  if (pixelGrid.mixedBlockSizes)
    diagnostics.push({
      code: 'MIXED_PIXEL_BLOCKS',
      severity: 'warning',
      confidence: pixelGrid.confidence,
      evidence: pixelGrid.evidence
    });
  return {
    path: file,
    width: image.width,
