...
function assertClosedAllowed(value, allowed, required, label) {
  if (
    ...
    throw new Error(`${label} must use a closed SPDX schema`);
}
...
export function normalizeComplianceSbom(input, { upstreamCommit, cargoLockSha256 }) {
  ...
  assertClosedAllowed(
    input,
    SPDX_KEYS,
    ['SPDXID', 'creationInfo', 'dataLicense', 'documentNamespace', 'packages', 'spdxVersion'],
    'SPDX document'
  );
  if (input.SPDXID !== 'SPDXRef-DOCUMENT' || input.spdxVersion !== 'SPDX-2.3' || input.dataLicense !== 'CC0-1.0')
  ...
  if (!Array.isArray(input.creationInfo.creators) || !input.creationInfo.creators.includes('Tool: cargo-sbom-v0.10.0'))
    throw new Error('cargo-sbom 0.10.0 creator is required');
  ...
  for (const key of [
    ...
    if (Object.hasOwn(value, key)) value[key] = sortByStable(value[key]);
...
async function main() {
  ...
  if (Object.keys(args).some((key) => !required.includes(key)) || required.some((key) => !args[key]))
    throw new Error(
      'usage: normalize-compliance.mjs --input RAW.json --output SPDX.json --upstream-commit SHA --cargo-lock-sha256 SHA256'
    );
  const input = JSON.parse(await fs.readFile(args.input, 'utf8'));
  const output = normalizeComplianceSbom(input, {
    upstreamCommit: args['upstream-commit'],
    cargoLockSha256: args['cargo-lock-sha256']
  });
