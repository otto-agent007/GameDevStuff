...
function validateMetadata(metadata) {
  ...
  if (metadata.schemaVersion !== 1 || !/^pixel-snapper-v\d+\.\d+\.\d+-commit\.[a-f0-9]{7}$/.test(metadata.releaseTag))
    throw new Error('invalid release checksum metadata');
  if (!Array.isArray(metadata.assets) || metadata.assets.length === 0)
    throw new Error('release checksum assets are required');
  const names = new Set();
  for (const asset of metadata.assets) {
    closed(asset, ['name', 'sha256', 'size'], 'release checksum asset');
    portableName(asset.name);
    hashValue(asset.sha256, `${asset.name} hash`);
    safeInteger(asset.size, `${asset.name} size`);
    const folded = asset.name.normalize('NFC').toLowerCase();
    if (names.has(folded)) throw new Error('portable release filename collision');
    names.add(folded);
  }
  if (names.size !== REQUIRED_ASSETS.length || REQUIRED_ASSETS.some((name) => !names.has(name.toLowerCase())))
    throw new Error('public release exact asset set mismatch');
  return metadata;
}
...
async function fetchPublicAsset({ name, root, fetchImpl }) {
  ...
  for (let redirects = 0; redirects <= 3; redirects += 1) {
    ...
    if (
      ...
      next.port ||
      next.hash ||
      (!githubSamePath && !releaseAsset)
    )
      throw new Error(`unsafe public release redirect: ${name}`);
    current = next;
  }
  ...
  if (rawEncoding !== null && rawEncoding !== undefined) {
    ...
    if (!/^[!#$%&'*+.^_`|~0-9a-z-]+$/.test(contentEncoding))
      throw new Error(`public release asset has invalid Content-Encoding: ${name}`);
    if (!ALLOWED_CONTENT_ENCODINGS.has(contentEncoding))
      throw new Error(`public release asset has unsupported Content-Encoding: ${name}`);
  }
  ...
  if (rawLength !== null && rawLength !== undefined) {
    ...
    if (parsed > BigInt(MAX_PUBLIC_ASSET_BYTES))
      throw new Error(`public release asset declared Content-Length exceeded size limit: ${name}`);
...
export async function verifyRelease({ metadata, fetchImpl = globalThis.fetch, baseUrl }) {
  ...
  if (
    ...
    parsedRoot.hostname !== 'github.com' ||
    parsedRoot.username ||
    parsedRoot.password ||
    parsedRoot.port ||
    parsedRoot.search ||
    parsedRoot.hash ||
    parsedRoot.pathname !== expectedPath
  )
    throw new Error('invalid immutable release base URL');
  const publicChecksums = await fetchPublicAsset({ name: 'checksums.json', root: parsedRoot, fetchImpl });
  let publishedMetadata;
  try {
    publishedMetadata = JSON.parse(publicChecksums.bytes.toString('utf8'));
  } catch {
    throw new Error('public checksums metadata is invalid');
  }
  ...
  for (const asset of checked.assets) {
    ...
    if (bytes.length !== asset.size || hashBytes(bytes) !== asset.sha256)
      throw new Error(`public release asset hash mismatch: ${asset.name}`);
    publicBytes.set(asset.name, bytes);
    verified.push({ name: asset.name, size: bytes.length, sha256: asset.sha256, url: url.href });
  }
  let buildMetadata;
  try {
    buildMetadata = JSON.parse(publicBytes.get('build-metadata.json').toString('utf8'));
  } catch {
    throw new Error('public build metadata is invalid');
  }
  closed(buildMetadata, ['schemaVersion', 'releaseTag', 'releaseUrl', 'targets'], 'public build metadata');
  if (
    buildMetadata.schemaVersion !== 1 ||
    buildMetadata.releaseTag !== checked.releaseTag ||
    buildMetadata.releaseUrl !== `https://github.com/otto-agent007/GameDevStuff/releases/tag/${checked.releaseTag}` ||
    !Array.isArray(buildMetadata.targets)
  )
    throw new Error('public build metadata identity mismatch');
  ...
  let publicManifest;
  try {
    publicManifest = JSON.parse(publicBytes.get('pixel-snapper-tool-manifest.json').toString('utf8'));
  } catch {
    throw new Error('public tool manifest is invalid');
  }
  if (stableJson(publicManifest) !== stableJson(expected.manifest))
    throw new Error('public tool manifest does not match build metadata');
  for (const record of records) {
    for (const descriptor of [record.files.license, record.files.notices, record.files.sbom]) {
      ...
      if (!bytes || bytes.length !== descriptor.size || hashBytes(bytes) !== descriptor.sha256)
        throw new Error(`public compliance file mismatch: ${record.target}/${descriptor.name}`);
    }
    const archive = publicBytes.get(record.archive.name);
    if (!archive || archive.length !== record.archive.size || hashBytes(archive) !== record.archive.sha256)
      throw new Error(`public archive identity mismatch: ${record.target}`);
...
async function main() {
  ...
  if (Object.keys(args).some((key) => !['metadata-dir'].includes(key)) || !args['metadata-dir'])
    throw new Error('usage: verify-release.mjs --metadata-dir DIR');
