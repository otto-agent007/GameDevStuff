...
import {
  FIXTURE_HEIGHT,
  FIXTURE_INPUT_RGBA_SHA256,
  FIXTURE_OUTPUT_RGBA_SHA256,
  FIXTURE_PALETTE_SHA256,
  FIXTURE_WIDTH,
  RELEASE_TAG,
  REQUIRED_TARGETS,
  RUST_TARGETS,
...
function validateFile(value, expectedName, label) {
  ...
  hashValue(value.sha256, `${label} hash`);
...
export function validateReleaseRecord(record) {
  ...
  portableName(record.archive.name);
  hashValue(record.archive.sha256, 'archive hash');
  safeInteger(record.archive.size, 'archive size');
  ...
  if (record.archive.format !== expectedFormat || record.archive.name !== expectedArchive)
    throw new Error(`archive identity mismatch: ${record.target}`);
  closed(record.executable, ['name', 'sha256', 'size'], 'executable record');
  const expectedExecutable =
    record.target === 'windows-x64' ? 'spritefusion-pixel-snapper.exe' : 'spritefusion-pixel-snapper';
  if (record.executable.name !== expectedExecutable) throw new Error(`executable name mismatch: ${record.target}`);
  hashValue(record.executable.sha256, 'executable hash');
  safeInteger(record.executable.size, 'executable size');
  closed(record.upstream, ['repository', 'tag', 'version', 'commit'], 'upstream identity');
  if (
    record.upstream.repository !== 'Hugo-Dz/spritefusion-pixel-snapper' ||
    !/^v\d+\.\d+\.\d+$/.test(record.upstream.tag) ||
    record.upstream.version !== record.upstream.tag.slice(1)
  )
  ...
  if (
    record.build.rustVersion !== '1.88.0' ||
    record.build.cargoSbomVersion !== '0.10.0' ||
    record.build.cargoAboutVersion !== '0.8.4' ||
    !/^cargo 1\.88\.0(?: \([^)]+\))?$/.test(record.build.cargoVersion) ||
    record.build.binaryVersion !== `spritefusion-pixel-snapper ${record.upstream.version}`
  )
    throw new Error('invalid locked toolchain identity');
  ...
  for (const key of ['inputRgbaSha256', 'rgbaSha256', 'paletteSha256'])
    hashValue(record.fixture[key], `fixture ${key}`);
  safeInteger(record.fixture.width, 'fixture width');
  safeInteger(record.fixture.height, 'fixture height');
  if (
    record.fixture.inputRgbaSha256 !== FIXTURE_INPUT_RGBA_SHA256 ||
    record.fixture.rgbaSha256 !== FIXTURE_OUTPUT_RGBA_SHA256 ||
    record.fixture.paletteSha256 !== FIXTURE_PALETTE_SHA256 ||
    record.fixture.width !== FIXTURE_WIDTH ||
    record.fixture.height !== FIXTURE_HEIGHT
  )
    throw new Error(`fixture approved identity mismatch: ${record.target}`);
...
export async function inspectReleaseArchive({ record: inputRecord, archiveBytes }) {
  ...
  try {
    ...
    const expectedFiles = [
      ...
      record.files.license.name,
      record.files.notices.name,
      record.files.sbom.name,
      record.files.metadata.name
    ];
    ...
    for (const [name, descriptor] of [
      [record.executable.name, record.executable],
      [record.files.license.name, record.files.license],
      [record.files.notices.name, record.files.notices],
      [record.files.sbom.name, record.files.sbom],
      [record.files.metadata.name, record.files.metadata]
    ]) {
      ...
      if (stat.size !== descriptor.size || (await hashFile(file)) !== descriptor.sha256)
        throw new Error(`archived file hash or size mismatch: ${record.target}/${name}`);
    }
    const expectedMetadata = {
      schemaVersion: 1,
      target: record.target,
      rustTarget: record.rustTarget,
      executable: record.executable,
      upstream: record.upstream,
      build: record.build,
      fixture: record.fixture,
      files: { license: record.files.license, notices: record.files.notices, sbom: record.files.sbom }
    };
    const embedded = JSON.parse(await fs.readFile(path.join(extracted, record.files.metadata.name), 'utf8'));
    if (stableJson(embedded) !== stableJson(expectedMetadata))
      throw new Error(`embedded target metadata mismatch: ${record.target}`);
...
async function loadArtifactDirectory(directory) {
  ...
  for (const [name, descriptor] of [
    [record.archive.name, record.archive],
    [record.executable.name, record.executable],
    [record.files.license.name, record.files.license],
    [record.files.notices.name, record.files.notices],
    [record.files.sbom.name, record.files.sbom],
    ...
    if (stat.size !== descriptor.size || (await hashFile(file)) !== descriptor.sha256)
      throw new Error(`build artifact hash or size mismatch: ${record.target}/${name}`);
  }
...
async function loadInputDirectory(inputDir) {
  ...
  if (entries.some((entry) => entry.isSymbolicLink() || !entry.isDirectory()))
    throw new Error('release input must contain only target artifact directories');
...
function compareRecords(records) {
  ...
  for (const record of records.slice(1)) {
    if (identity(record.upstream) !== identity(first.upstream))
      throw new Error(`upstream identity mismatch: ${record.target}`);
    if (record.build.workflowCommit !== first.build.workflowCommit)
      throw new Error(`workflow identity mismatch: ${record.target}`);
    if (record.build.cargoLockSha256 !== first.build.cargoLockSha256)
      throw new Error(`lockfile identity mismatch: ${record.target}`);
    const tools = ({ rustVersion, cargoVersion, cargoSbomVersion, cargoAboutVersion, binaryVersion }) => ({
      rustVersion,
      cargoVersion,
      cargoSbomVersion,
      cargoAboutVersion,
    ...
    if (identity(tools(record.build)) !== identity(tools(first.build)))
      throw new Error(`toolchain identity mismatch: ${record.target}`);
    if (record.fixture.inputRgbaSha256 !== first.fixture.inputRgbaSha256)
      throw new Error(`fixture input hash mismatch: ${record.target}`);
    if (record.fixture.rgbaSha256 !== first.fixture.rgbaSha256)
      throw new Error(`fixture RGBA hash mismatch: ${record.target}`);
    if (identity(record.fixture) !== identity(first.fixture))
      throw new Error(`fixture identity mismatch: ${record.target}`);
    if (
      record.files.license.sha256 !== first.files.license.sha256 ||
      record.files.license.size !== first.files.license.size
    )
      throw new Error(`upstream license mismatch: ${record.target}`);
    if (
      record.files.notices.sha256 !== first.files.notices.sha256 ||
      record.files.notices.size !== first.files.notices.size
    )
      throw new Error(`third-party notices mismatch: ${record.target}`);
    if (record.files.sbom.sha256 !== first.files.sbom.sha256 || record.files.sbom.size !== first.files.sbom.size)
      throw new Error(`SBOM mismatch: ${record.target}`);
  }
}
...
function validateReleaseTag(releaseTag, upstream) {
  ...
  if (match[1] !== upstream.version || match[2] !== upstream.commit.slice(0, 7))
    throw new Error('release tag commit suffix or version mismatch');
}
...
function createResult(records, releaseTag) {
  ...
  const manifest = validateToolManifest({
    ...
    build: {
      rustVersion: first.build.rustVersion,
      cargoLockSha256: first.build.cargoLockSha256,
      workflowCommit: first.build.workflowCommit
    },
    ...
    assets: Object.fromEntries(
      records.map((record) => [
        record.target,
        {
          ...
          archiveFormat: record.archive.format,
          archiveSize: record.archive.size,
          archiveSha256: record.archive.sha256,
          executable: record.executable.name,
          executableSize: record.executable.size,
          executableSha256: record.executable.sha256
        }
      ])
    )
  });
...
export async function assembleRelease({
  ...
  const loaded = inputDir
    ? await loadInputDirectory(inputDir)
    : inputs.map((record) => ({ record: validateReleaseRecord(structuredClone(record)), directory: null }));
  ...
  for (const target of REQUIRED_TARGETS)
    if (!byTarget.has(target)) throw new Error(`missing release target: ${target}`);
  ...
  if (expectedUpstreamCommit !== undefined) {
    ...
    if (records[0].upstream.commit !== expectedUpstreamCommit)
      throw new Error('upstream commit does not match workflow input');
  }
  if (expectedWorkflowCommit !== undefined) {
    commitValue(expectedWorkflowCommit, 'expected workflow commit');
    if (records[0].build.workflowCommit !== expectedWorkflowCommit)
      throw new Error('workflow commit does not match running workflow');
  }
  ...
  const effectiveReleaseTag =
    releaseTag ?? `pixel-snapper-v${records[0].upstream.version}-commit.${records[0].upstream.commit.slice(0, 7)}`;
  ...
  return atomicDirectory(outputDir, async (stage) => {
    for (const item of ordered)
      await fs.copyFile(
        path.join(item.directory, item.record.archive.name),
        path.join(stage, item.record.archive.name),
        fs.constants.COPYFILE_EXCL
      );
    const first = ordered[0];
    for (const descriptor of [first.record.files.license, first.record.files.notices, first.record.files.sbom])
      await fs.copyFile(
        path.join(first.directory, descriptor.name),
        path.join(stage, descriptor.name),
        fs.constants.COPYFILE_EXCL
      );
    ...
    const releaseAssets = [
      ...records.map((record) => record.archive.name),
...
async function main() {
  ...
  if (Object.keys(args).some((key) => !allowed.includes(key)) || allowed.some((key) => !args[key]))
    throw new Error('usage: assemble-release.mjs --input-dir DIR --output-dir DIR --release-tag TAG');
  const result = await assembleRelease({
    inputDir: args['input-dir'],
    outputDir: args['output-dir'],
    releaseTag: args['release-tag'],
    expectedUpstreamCommit: args['upstream-commit'],
    expectedWorkflowCommit: args['workflow-commit']
  });
